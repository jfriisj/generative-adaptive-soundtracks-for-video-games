...
namespace DemoMPTK
{
    ...
    {
        private const int MAX_LINE_RENDERER = 30;
        ...
        private int lastNoteViewer;
        private MPTKStat midiStat;
        private NoteViewerCircle[] notesViewerCircle;

        private class NoteViewerCircle
        {
            private static Color[] colors;
            private static float widthLine = 0.1f;
            private static float growingPhase;
            private static float fadingPhase = 0.2f;
            private static float distanceShadow;
            ...
            private PHASE phase;
            private Vector3 positionEnd = new(0, 0, 0);

            private Vector3 positionStart = new(0, 0, 0);
            private readonly GameObject ShadowScreen;
            ...
            private float timeSteady;


            public NoteViewerCircle(GameObject goRenderer, GameObject WithLineRenderer, GameObject shadowScreen,
                int pIndex)
            {
                ...
                {
                    colors = new Color[16];
                    // Remplir le tableau avec des couleurs diff�rentes (thanks to CoPilot !)
                    colors[0] = Color.red;
                ...
                var obj = Instantiate(WithLineRenderer);
            ...
            public static float WidthLine
            {
                get => widthLine;
                set => widthLine = Mathf.Clamp(value, 0.001f, 5f);
            }

            public static float GrowingPhase
            {
                get => growingPhase;
                set => growingPhase = Mathf.Clamp(value, 0f, 2f);
            }

            public static float FadingPhase
            {
                get => fadingPhase;
                set => fadingPhase = Mathf.Clamp(value, 0f, 2f);
            }

            public static float DistanceShadow
            {
                get => distanceShadow;
                set => distanceShadow = Mathf.Clamp(value, 0f, 1f);
            }
            ...
            public void AssignMidi(MPTKEvent pmidiEvent, int noteMin, int noteMax)
            {
                ...
                //  Add PI (rotation of 180 degr�) 
                //  Avoid division by zero if min = max: return PI.
                var angleNoteValue = noteMax == noteMin
                    ? Mathf.PI
                    : (midiEvent.Value - noteMin) / (float)(noteMax - noteMin) * Mathf.PI * 2f + MathF.PI;

                // Calculating length from velocity: Build a normalized value [0,1] from the note velocity [0, 127]
                var normalisedNoteVel = midiEvent.Velocity / 127f;
            ...
            public void Update()
            {
                if (midiEvent != null && lineRenderer != null)
                {
                    ...
                    var positionShadow = ((RectTransform)ShadowScreen.transform).anchoredPosition3D;
                    ...
                    switch (phase)
                    {
                        case PHASE.GROWING:
                            ...
                            else
                            {
                                ...
                                color.a = timeStart == timeGrowing
                                    ? 1f
                                    : (Time.time - timeStart) / (timeGrowing - timeStart);
                            }
                        ...
                        case PHASE.STEADY:
                            if (Time.time > timeSteady) phase = PHASE.FADING; // go to fadng phase
                            break;
                        ...
                        case PHASE.FADING:
                            ...
                            else
                            {
                                ...
                                color.a = timeSteady == timeGrowing
                                    ? 0f
                                    : (timeFading - Time.time) / (timeFading - timeSteady);
                            }
