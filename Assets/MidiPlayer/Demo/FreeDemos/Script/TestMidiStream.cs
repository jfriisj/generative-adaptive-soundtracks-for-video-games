...
namespace DemoMPTK
{
    public class TestMidiStream : MonoBehaviour
    {
        ...
        [Range(0.05f, 10f)] public float LoopDelay = 1;

        [Range(-10f, 100f)] public float CurrentDuration;

        [Range(0f, 10f)] public float CurrentDelay;

        [Header("Realtime voice parameters mode")]
        public bool RealtimeRelatif;

        [Header("")] public bool RandomNote, RandomDuration, RandomDelay;
        public bool DrumKit;
        public bool ChordPlay;
        public int ArpeggioPlayChord;
        public int DelayPlayScale = 200;
        public bool ChordLibPlay;
        public bool ScaleLibPlay;
        public int CurrentChord;

        [Range(0, 127)] public int StartLoopingNote = 50;

        [Range(0, 127)] public int EndLoopingNote = 60;

        [Range(0, 127)] public int CurrentVelocity = 100;

        [Range(0, 16)] public int StreamChannel;

        [Range(0, 16)]
        public int
            DrumChannel =
                9; // by convention the channel 10 is used for playing drum (value = 9 because channel start from channel 0 in Maestro)

        [Range(0, 127)] public int CurrentNote;

        [Range(0, 127)] public int StartLoopPreset;

        [Range(0, 127)] public int EndLoopPreset = 127;

        [Range(0, 127)] public int CurrentPreset;

        [Range(0, 127)] public int CurrentBank;

        [Range(0, 127)] public int CurrentBankDrum;

        [Range(0, 127)] public int CurrentPatchDrum;

        [Range(0, 127)] public int PanChange = 64;

        [Range(0, 127)] public int ModulationChange;

        [Range(0, 1)] public float PitchChange = DEFAULT_PITCH;

        [Range(0, 127)] public int ExpressionChange = 127; // default value

        [Range(0, 127)] public int AttenuationChange = 100; // default value

        private const float DEFAULT_PITCH = 0.5f; // 8192;

        [Range(0, 24)] public int PitchSensibility = 2;
        ...
        private readonly int buttonWidth = 250;
        private readonly int buttonSmallWidth = 166;
        private readonly int buttonTinyWidth = 100;
        private readonly float spaceVertical = 0;
        private readonly float widthFirstCol = 100;
        ...
        private void Awake()
        {
            ...
            {
                Debug.LogWarning(
                    "midiStreamPlayer is not defined. Check in Unity editor inspector of this gameComponent");
            }
        }
        ...
        private void Start()
        {
            ...
            PopBankInstrument = new PopupListItem
            {
                Title = "Select A Bank", OnSelect = PopupBankPatchChanged, Tag = "BANK_INST", ColCount = 5,
                ColWidth = 150
            };
            PopPatchInstrument = new PopupListItem
            {
                Title = "Select A Patch", OnSelect = PopupBankPatchChanged, Tag = "PATCH_INST", ColCount = 5,
                ColWidth = 150
            };
            PopBankDrum = new PopupListItem
            {
                Title = "Select A Bank", OnSelect = PopupBankPatchChanged, Tag = "BANK_DRUM", ColCount = 5,
                ColWidth = 150
            };
            PopPatchDrum = new PopupListItem
            {
                Title = "Select A Patch", OnSelect = PopupBankPatchChanged, Tag = "PATCH_DRUM", ColCount = 5,
                ColWidth = 150
            };
            ...
            for (var i = 0; i < nbrGenerator; i++)
            {
                ...
                if (indexGenerator[i] >= 0)
                    valueGenerator[i] = RealtimeRelatif
                        ? 0f
                        : GenModifier.DefaultNormalizedVal((fluid_gen_type)indexGenerator[i]) * 100f;
                PopGenerator[i] = new PopupListItem
                {
                    Title = "Select A Generator", OnSelect = PopupGeneratorChanged, Tag = i, ColCount = 3,
                    ColWidth = 250
                };
            }
        ...
        // disabled
        private void xxxOnApplicationFocus(bool hasFocus)
        {
        ...
        /// <summary>
        ///     @brief
        ///     This method is called when the synthesizer is ready if you have defined OnEventSynthStarted or defined the event
        ///     from the inspector in Unity.
        ...
        public void EndLoadingSynth(string name)
        {
            ...
            {
                Command = MPTKCommand.ControlChange, Controller = MPTKController.BankSelectMsb, Value = CurrentBank,
            ...
            Debug.LogFormat(
                $"   Preset '{midiStreamPlayer.MPTK_Channels[StreamChannel].PresetName}' defined on channel {StreamChannel}");
            //! [ExampleUsingChannelAPI_3]
        ...
        public bool Test_MPTK_ChannelPresetChange;
        ...
        ///     or standard MIDI
        ///     // change bank
        ///     MPTK_PlayEvent(new MPTKEvent() { Command = MPTKCommand.ControlChange, Controller = MPTKController.BankSelectMsb,
        ///     Value = index, Channel = StreamChannel, });
        ...
        private void PopupBankPatchChanged(object tag, int index, int indexList)
        {
            ...
            switch ((string)tag)
            {
                case "BANK_INST":
                    ...
                    Debug.Log(
                        $"Instrument Bank change - channel:{StreamChannel} bank:{midiStreamPlayer.MPTK_Channels[StreamChannel].BankNum} preset:{midiStreamPlayer.MPTK_Channels[StreamChannel].PresetNum}");
                    break;
                ...
                case "PATCH_INST":
                    ...
                    Debug.Log(
                        $"Instrument Preset change - channel:{StreamChannel} bank:{midiStreamPlayer.MPTK_Channels[StreamChannel].BankNum} preset:{midiStreamPlayer.MPTK_Channels[StreamChannel].PresetNum}");
                    break;
                ...
                case "BANK_DRUM":
                    ...
                    if (Test_MPTK_ChannelPresetChange)
                        ...
                        {
                            Command = MPTKCommand.ControlChange, Controller = MPTKController.BankSelectMsb,
                            Value = index, Channel = DrumChannel
                        });

                    Debug.Log(
                        $"Drum Bank change - channel:{DrumChannel} bank:{midiStreamPlayer.MPTK_Channels[DrumChannel].BankNum} preset:{midiStreamPlayer.MPTK_Channels[DrumChannel].PresetNum}");
                    break;
                ...
                case "PATCH_DRUM":
                    ...
                    if (Test_MPTK_ChannelPresetChange)
                        ...
                        midiStreamPlayer.MPTK_PlayEvent(new MPTKEvent
                            { Command = MPTKCommand.PatchChange, Value = index, Channel = DrumChannel });

                    Debug.Log(
                        $"Drum Preset change - channel:{DrumChannel} bank:{midiStreamPlayer.MPTK_Channels[DrumChannel].BankNum} preset:{midiStreamPlayer.MPTK_Channels[DrumChannel].PresetNum}");
                    break;
            }
        }
        ...
        {
            var iGenerator = Convert.ToInt32(tag);
            indexGenerator[iGenerator] = index;
            labelGenerator[iGenerator] = GenModifier.RealTimeGenerator[indexList].Label;
            valueGenerator[iGenerator] = RealtimeRelatif
                ? 0f
                : GenModifier.DefaultNormalizedVal((fluid_gen_type)indexGenerator[iGenerator]) * 100f;
            Debug.Log(
                $"indexList:{indexList} indexGenerator:{indexGenerator[iGenerator]} valueGenerator:{valueGenerator[iGenerator]} {labelGenerator[iGenerator]}");
        }

        private void OnGUI()
        {
            var scale = HelperDemo.GUIScale();
            ...
            if (midiStreamPlayer != null)
            {
                ...
                scrollerWindow = GUILayout.BeginScrollView(scrollerWindow, false, false,
                    GUILayout.Width(Screen.width / scale.x), GUILayout.Height(Screen.height / scale.y));
                ...
                if (midiStreamPlayer.MPTK_SoundFont.SoundFont != null)
                {
                    ...
                    MainMenu.Display("Test MIDI Stream - A very simple Generated Music Stream ", myStyle,
                        Screen.width / scale.x, "https://paxstellar.fr/midi-file-player-detailed-view-2-2/");
                ...
                if (FoldOutLooping)
                    OnGUI_LoopingAndRandom();
                                ...
                                CountNoteToPlay =
 (int)Slider("Play Multiple Notes", CountNoteToPlay, 1, 200, false, 70);
                                HelperDemo.GUI_Horizontal(HelperDemo.Zone.END);
                                GUILayout.Space(spaceVertical);
#endif
                ...
                // ------------------------------
                FoldOutRealTimeMidiChange =
                    GUILayout.Toggle(FoldOutRealTimeMidiChange, "MIDI Controller and Pitch Change");
                ...
                // ---------------------------------
                FoldOutRealTimeVoiceChange =
                    GUILayout.Toggle(FoldOutRealTimeVoiceChange, "Real-time Voice Parameters Change (Pro)");
                ...
                // ------------------------
                FoldOutEffectSoundFontDisplay =
                    GUILayout.Toggle(FoldOutEffectSoundFontDisplay, "Enable SoundFont Effects");
                ...
                if (Application.isEditor)
                {
                    ...
                    if (!string.IsNullOrEmpty(Application.consoleLogPath))
                    {
                        ...
                        if (GUILayout.Button("Open Folder " + Path.GetDirectoryName(Application.consoleLogPath)))
                    ...
                    GUILayout.Label(
                        "Go to your Hierarchy, select GameObject MidiStreamPlayer: inspector contains a lot of parameters to control the sound.",
                        myStyle.TitleLabel2);
                    HelperDemo.GUI_Vertical(HelperDemo.Zone.END);
                }
                else
                {
                    ...
                    GUILayout.Label(
                        "Play blues notes C minor: C, D#, F, F#, G, A# from numeric keys on your top keyboard. Change preset with arrow up/down keys.",
                        myStyle.TitleLabel2);
                    HelperDemo.GUI_Vertical(HelperDemo.Zone.END);
                }
        ...
        private void OnGUI_SelectBankAndPatchForInstrument()
        {
            ...
            if (GUILayout.Button(MidiPlayerGlobal.ImSFCurrent.DefaultBankNumber + " - Bank",
            ...
            if (GUILayout.Button(
                    CurrentPreset + " - " +
                    MidiPlayerGlobal.MPTK_GetPatchName(MidiPlayerGlobal.ImSFCurrent.DefaultBankNumber, CurrentPreset),
                    GUILayout.Width(buttonWidth)))
            {
                PopBankInstrument.Show = false;
                PopBankDrum.Show = false;
            ...
            var channel = (int)HelperDemo.GUI_Slider("Channel", StreamChannel, 0, MAX_CHANNEL - 1, true,
                widthLabelValue: 20, widthCaption: 70, widthSlider: 100);
        ...
        private void OnGUI_SelectBankAndPatchForDrum()
        {
            ...
            if (GUILayout.Button(MidiPlayerGlobal.ImSFCurrent.DrumKitBankNumber + " - Bank",
                    GUILayout.Width(buttonWidth)))
            {
                PopBankInstrument.Show = false;
                PopPatchInstrument.Show = false;
            ...
            if (GUILayout.Button(
                    CurrentPatchDrum + " - " +
                    MidiPlayerGlobal.MPTK_GetPatchName(MidiPlayerGlobal.ImSFCurrent.DrumKitBankNumber,
                        CurrentPatchDrum),
                    GUILayout.Width(buttonWidth)))
            {
                PopBankInstrument.Show = false;
            ...
            var newDrumKit = GUILayout.Toggle(DrumKit, "Activate Drum Mode   ");
        ...
        {
            HelperDemo.GUI_Horizontal(HelperDemo.Zone.BEGIN, null, GUILayout.Width(350));
            ...
            // Select any value in the range 0 and 16383 for the bank 
            var newBank = (int)HelperDemo.GUI_Slider("Free Bank", midiStreamPlayer.MPTK_Channels[StreamChannel].BankNum,
                0, 128 * 128 - 1, widthCaption: 100, widthSlider: 115, widthLabelValue: 30);
            // Select any value in the range 0 and 127 for the preset
            var newPreset = (int)HelperDemo.GUI_Slider("Free Preset",
                midiStreamPlayer.MPTK_Channels[StreamChannel].PresetNum, 0, 127, true, widthCaption: 100,
                widthSlider: 115, widthLabelValue: 30);
            // If user made change for bank or preset ...
        ...
        private void OnGUI_RealTimeVoiceParametersChange()
        {
            ...
            HelperDemo.GUI_Vertical(HelperDemo.Zone.BEGIN, myStyle.BacgDemosLight, GUILayout.Width(widthIndent),
            ...
            if (GUILayout.Button("Stop", myStyle.BtStandard, GUILayout.Width(buttonSmallWidth))) MaestroStopOneNote();
            ...
            float gene;
            for (var i = 0; i < nbrGenerator; i += 2) // 2 generators per line
            {
                HelperDemo.GUI_Horizontal(HelperDemo.Zone.BEGIN, null, GUILayout.Width(650));

                for (var j = 0; j < 2; j++) // 2 generators per line
                {
                    var numGenerator = i + j;
                    // Open the popup to select an instrument
                    if (GUILayout.Button(indexGenerator[numGenerator] + " - " + labelGenerator[numGenerator],
                            GUILayout.Width(buttonWidth)))
                        PopGenerator[numGenerator].Show = !PopGenerator[numGenerator].Show;

                    // Get real time value
                    gene = HelperDemo.GUI_Slider("Value", valueGenerator[numGenerator], RealtimeRelatif ? -100f : 0f,
                        100f, true, true, 50f, 80f);
                    if (indexGenerator[numGenerator] >= 0)
                    {
                        ...
                        {
                            float currentValue =
 NotePlaying.GetSynthParameterCurrentValue((fluid_gen_type)indexGenerator[numGenerator]);
        ...
        private void OnGUI_PlayNote()
        {
            ...
            if (GUILayout.Button("Clear", myStyle.BtStandard, GUILayout.Width(buttonSmallWidth),
                    GUILayout.Height(heightButton))) midiStreamPlayer.MPTK_ClearAllSound(true);

            if (GUILayout.Button("Reset", myStyle.BtStandard, GUILayout.Width(buttonSmallWidth),
                    GUILayout.Height(heightButton)))
            {
                midiStreamPlayer.MPTK_InitSynth();
                StreamChannel = 0;
                ...
                StartLoopPreset = 0;
                EndLoopPreset = 127;
                RealtimeRelatif = FoldOutLooping =
                    FoldOutChord = FoldOutRealTimeMidiChange = FoldOutRealTimeVoiceChange = false;
                ...
                DegreeChord = 1;
                CurrentScale = 0;
                CurrentChord = ArpeggioPlayChord = 0;
                DelayPlayScale = 200;
                PanChange = 64;
                ModulationChange = 0;
                PitchChange = DEFAULT_PITCH;
                ExpressionChange = 127;
                AttenuationChange = 100;
                PitchSensibility = 2;
            }
            ...
            CurrentNote = (int)HelperDemo.GUI_Slider($"Note {HelperNoteLabel.LabelFromMidi(CurrentNote)}", CurrentNote,
                0, 127, widthCaption: 70);
            CurrentVelocity = (int)HelperDemo.GUI_Slider("Velocity", CurrentVelocity, 0f, 127f, true, widthCaption: 70);
            CurrentDuration = HelperDemo.GUI_Slider("Duration", CurrentDuration, -1f, 10f, true, valueButton: 0.1f,
                widthCaption: 70);
            CurrentDelay =
                HelperDemo.GUI_Slider("Delay", CurrentDelay, 0f, 5f, true, valueButton: 0.1f, widthCaption: 50);
        ...
        private void OnGUI_LoopingAndRandom()
        {
            ...
            LoopDelay = HelperDemo.GUI_Slider("Loop Delay (s)", LoopDelay, 0.01f, 10f, valueButton: 0.1f,
                widthCaption: 120, widthSlider: 100);
            GUILayout.Space(20f);
            ...
            if (GUILayout.Button("Start / Stop", IsplayingLoopNotes ? myStyle.BtSelected : myStyle.BtStandard,
                    GUILayout.Width(buttonSmallWidth))) IsplayingLoopNotes = !IsplayingLoopNotes;
            ...
            if (GUILayout.Button("Start / Stop", IsplayingLoopPresets ? myStyle.BtSelected : myStyle.BtStandard,
                    GUILayout.Width(buttonSmallWidth))) IsplayingLoopPresets = !IsplayingLoopPresets;
        ...
        private void OnGUI_MidiControllerChange()
        {
            ...
            var pitchChange = HelperDemo.GUI_Slider("Pitch Change", PitchChange, 0, 1);
            ...
            var pitchSensi = (int)HelperDemo.GUI_Slider("Sensibility", PitchSensibility, 0, 24, true);
            ...
            midiStreamPlayer.MPTK_Transpose =
                (int)HelperDemo.GUI_Slider("Transpose", midiStreamPlayer.MPTK_Transpose, -24, 24, true);
            ...
            // Change volume
            midiStreamPlayer.MPTK_Volume =
                HelperDemo.GUI_Slider("Global Volume", midiStreamPlayer.MPTK_Volume, 0f, Constant.MAX_VOLUME);
            ...
            var panChange = (int)HelperDemo.GUI_Slider("Panoramic", PanChange, 0, 127, true);
            ...
            var sustain = midiStreamPlayer.MPTK_Channels[StreamChannel].Controller(MPTKController.Sustain) < 64
                ? false
                : true;

            // Switch sustain state
            if (GUILayout.Button(sustain ? "Sustain On" : "Sustain Off",
                    sustain ? myStyle.BtSelected : myStyle.BtStandard, GUILayout.Width(buttonTinyWidth)))
            ...
            HelperDemo.GUI_Horizontal(HelperDemo.Zone.BEGIN, null, GUILayout.Width(350));
            var modChange = (int)HelperDemo.GUI_Slider("Modulation", ModulationChange, 0, 127);
            ...
            var expChange = (int)HelperDemo.GUI_Slider("Expression", ExpressionChange, 0, 127, true);
            ...
            // Change modulation. Often applied volume, this effect is defined in the SoundFont 
            var expAttenuation = (int)HelperDemo.GUI_Slider("Attenuation", AttenuationChange, 0, 127, true);
        ...
        {
            HelperDemo.GUI_Horizontal(HelperDemo.Zone.BEGIN, null, GUILayout.Width(600));

            var countNote = (int)HelperDemo.GUI_Slider("Count", CountNoteChord, 2, 17);
            ...
            var degreeChord = (int)HelperDemo.GUI_Slider("Degree", DegreeChord, 1, 7, true, widthSlider: 100,
                widthCaption: 100, widthLabelValue: 30);
            ...
            ArpeggioPlayChord = (int)HelperDemo.GUI_Slider("Arpeggio (ms)", ArpeggioPlayChord, 0, 500, true,
                widthSlider: 100, widthCaption: 100, widthLabelValue: 30);
...
#if MPTK_PRO
            int chord =
 (int)HelperDemo.GUI_Slider("Scale", CurrentChord, 0, MPTKChordLib.Chords.Count - 1, alignCaptionRight: false, widthSlider: 100, widthCaption: 100, widthLabelValue: 30);
...
#if MPTK_PRO
            int scale =
 (int)HelperDemo.GUI_Slider("Scale", CurrentScale, 0, MPTKScaleLib.RangeCount - 1, alignCaptionRight: false, widthSlider: 100, widthCaption: 100, widthLabelValue: 30);
        ...
        private readonly int[] keysToNote = { 60, 63, 65, 66, 67, 70, 72, 75, 77 };
        ...
        private void Update()
        {
            ...
            for (var key = 0; key < 9; key++)
            {
                if (Input.GetKeyDown(KeyCode.Alpha1 + key))
                {
                    ...
                    eventsMidi[key] = new MPTKEvent
                    {
                        ...
                        Value = keysToNote[
                            key], // blues en C minor: C,D#,F,F#,G,A# http://patrick.murris.com/musique/gammes_piano.htm?base=3&scale=0%2C3%2C5%2C6%2C7%2C10&octaves=1
                        Velocity = 100
                    };
            ...
            if (PitchChange != DEFAULT_PITCH)
            {
                ...
                {
                    PitchChange =
 Mathf.SmoothDamp(PitchChange, DEFAULT_PITCH, ref currentVelocityPitch, 0.5f, 10000, Time.unscaledDeltaTime);
            ...
            if (midiStreamPlayer != null && (IsplayingLoopPresets || IsplayingLoopNotes))
            {
                ...
                if (time > LoopDelay)
                {
                    ...
                    for (var indexNote = 0; indexNote < CountNoteToPlay; indexNote++)
                    {
                        // Loop on preset
        ...
        private void MaestroPlay(bool stopCurrent)
        {
            if (RandomNote)
            {
                if (StartLoopingNote >= EndLoopingNote)
                    ...
                    CurrentNote = Random.Range(StartLoopingNote, EndLoopingNote);
            }

            if (RandomDuration)
            {
                CurrentDuration = Random.Range(0.1f, 2f);
                if (!RandomDelay)
                    LoopDelay = CurrentDuration;
            }

            if (RandomDelay)
                LoopDelay = Random.Range(0.01f, 2f);
        ...
        private void MaestroPlayScale()
        {
            ...
            for (int ecart = 0; ecart < scale.Count; ecart++)
            {
                NotePlaying = new MPTKEvent()
                {
                    ...
                    Duration =
 DelayPlayScale, // note duration in millisecond, -1 to play indefinitely, MPTK_StopEvent to stop
                    Velocity = CurrentVelocity, // from 0 to 127, sound can vary depending on the velocity
                    Delay =
 ecart * DelayPlayScale, // delay in millisecond before playing the note. Use it yo play chord with Arpeggio
                };
                midiStreamPlayer.MPTK_PlayEvent(NotePlaying);
            }
        }
