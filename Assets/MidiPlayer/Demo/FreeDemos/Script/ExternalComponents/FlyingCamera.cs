...
namespace DemoMPTK
{
    public class FlyingCamera : MonoBehaviour
    {
        ...
        private bool isRotating; // Angryboy: Can be called by other things (e.g. UI) to see if camera is rotating
        ...
        private PhysicsMaterial myMaterial;
#else
        ...
        private void Update()
        {
            ...
            var newPosition = transform.position; //If player wants to move on X and Z axis only
            transform.Translate(p);
            newPosition.x = transform.position.x;
            newPosition.z = transform.position.z;

            // Angryboy: Manipulate Y plane by using Q/E keys
            if (Input.GetKey(KeyCode.Q)) newPosition.y += -speedMultiplier;
            if (Input.GetKey(KeyCode.E)) newPosition.y += speedMultiplier;
        ...
        private Vector3 GetBaseInput()
        {
            ...
            if (Input.GetKey(KeyCode.W) || Input.GetKey(KeyCode.UpArrow)) p_Velocity += new Vector3(0, 0, 1);
            if (Input.GetKey(KeyCode.S) || Input.GetKey(KeyCode.DownArrow)) p_Velocity += new Vector3(0, 0, -1);
            if (Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.LeftArrow)) p_Velocity += new Vector3(-1, 0, 0);
            if (Input.GetKey(KeyCode.D) || Input.GetKey(KeyCode.RightArrow)) p_Velocity += new Vector3(1, 0, 0);
            return p_Velocity;
        }
    }
