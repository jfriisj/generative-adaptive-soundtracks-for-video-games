...
namespace DemoMPTK
{
    /// <summary>
    ///     @brief
    ///     MPTK Demo component able to load a MIDI file from your list of MIDI file or from a folder and displaying the list
    ///     of events.
    /// </summary>
    public class TestMidiFileLoad : MonoBehaviour
    {
        private const int MAXLINEPAGE = 100;
        ...
        private void Awake()
        {
            ...
            {
                Debug.LogError(
                    "Can't find a MidiFilePlayer Prefab in the current Scene Hierarchy. Add it with the Maestro menu.");
                return;
            }
        ...
        private void OnGUI()
        {
            ...
            if (MidiLoader != null)
            {
                ...
                MainMenu.Display("Test MIDI File Loader - Demonstrate how to use the MPTK API to load a MIDI file",
                    myStyle, Screen.width);
                ...
                GUILayout.BeginHorizontal();
                GUILayout.Label(
                    "Select and load a MIDI File from the internal MidiDB. Add you MIDI from the Unity editor menu MPTK (ALT-M):",
                    GUILayout.Width(350));
                ...
                GUILayout.Space(20);
                GUILayout.BeginHorizontal();
                if (GUILayout.Button($"Read MIDI Events from ticks {StartTicks} to ticks {EndTicks}",
                        GUILayout.Height(40)))
                {
                    // Read MIDI events between two value of ticks
                    // -------------------------------------------
                    infoEvents = new List<string>();
                    var mptkEvents = MidiLoader.MPTK_ReadMidiEvents(StartTicks, EndTicks);
                ...
                StartTicks = (long)GUILayout.HorizontalSlider(StartTicks, 0f, MidiLoader.MPTK_TickLast,
                ...
                EndTicks = (long)GUILayout.HorizontalSlider(EndTicks, 0f, MidiLoader.MPTK_TickLast,
                    GUILayout.Width(buttonWidth));
                GUILayout.EndHorizontal();
                ...
                GUILayout.Label(Convert.ToInt32(MidiLoader.MPTK_MidiLoaded.MPTK_InitialTempo).ToString(),
                    myStyle.TitleLabel3);
                GUILayout.EndHorizontal();
                ...
                GUILayout.Label(
                    "The simplest MIDI file loader. See C# script TestMidiFileLoad.cs and TheMostSimpleDemoForMidiLoader() method:",
                    GUILayout.Width(380));
                if (GUILayout.Button(new GUIContent($"Load MIDI File index {MidiIndex}", ""), GUILayout.Width(160),
                        GUILayout.Height(40))) TheMostSimpleDemoForMidiLoader();
                GUILayout.EndHorizontal();
                GUILayout.Space(20);


                GUILayout.Label(
                    "This class can be used only to load a MIDI file and read events. There is no MIDI sequencer and no MIDI Synthesizer. Rather, used the prefab MidiFilePlayer to play a MIDI file.",
                    myStyle.TitleLabel3);
                ...
                if (infoEvents != null && infoEvents.Count > 0)
                {
                    ...
                    if (GUILayout.Button("<<", butCentered, GUILayout.Width(40), GUILayout.Height(40)))
                        PageToDisplay = 0;
                    if (GUILayout.Button("<", butCentered, GUILayout.Width(40), GUILayout.Height(40))) PageToDisplay--;
                    GUILayout.Label("page " + (PageToDisplay + 1) + " / " + (infoEvents.Count / MAXLINEPAGE + 1),
                        labCentered, GUILayout.Width(150), GUILayout.Height(40));
                    if (GUILayout.Button(">", butCentered, GUILayout.Width(40), GUILayout.Height(40))) PageToDisplay++;
                    if (GUILayout.Button(">>", butCentered, GUILayout.Width(40), GUILayout.Height(40)))
                    ...
                    var infoToDisplay = "";
                    for (var i = PageToDisplay * MAXLINEPAGE; i < (PageToDisplay + 1) * MAXLINEPAGE; i++)
        ...
        private void TheMostSimpleDemoForMidiLoader()
        {
            ...
            var mfp = FindFirstObjectByType<MidiFilePlayer>();
            if (mfp == null)
            {
                Debug.LogWarning(
                    "Can't find a MidiFilePlayer Prefab in the current Scene Hierarchy. Add it with the Maestro menu.");
            ...
            foreach (var mptkEvent in mfp.MPTK_MidiEvents)
                // Log if event is a note on
                if (mptkEvent.Command == MPTKCommand.NoteOn)
                    Debug.Log(
                        $"Note On at {mptkEvent.RealTime} millisecond  Channel:{mptkEvent.Channel} Note:{mptkEvent.Value}  Duration:{mptkEvent.Duration} millisecond  Velocity:{mptkEvent.Velocity}");
                else if (mptkEvent.Command == MPTKCommand.PatchChange)
                    Debug.Log(
                        $"Preset Change at {mptkEvent.RealTime} millisecond  Channel:{mptkEvent.Channel}  Preset:{mptkEvent.Value}");
                else if (mptkEvent.Command == MPTKCommand.ControlChange)
                    if (mptkEvent.Controller == MPTKController.BankSelectMsb)
                        Debug.Log(
                            $"Bank Change at {mptkEvent.RealTime} millisecond  Channel:{mptkEvent.Channel}  Bank:{mptkEvent.Value}");
