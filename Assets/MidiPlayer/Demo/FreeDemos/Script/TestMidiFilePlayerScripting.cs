...
namespace DemoMPTK
{
    public partial class TestMidiFilePlayerScripting : MonoBehaviour
    {
        ...
        [Header("[Pro] Delay to ramp up volume at startup or down at stop")] [Range(0f, 5000f)]
        public float DelayRampMillisecond;

        [Header("Start position in Midi defined in pourcentaage of the whole druration of the Midi")] [Range(0f, 100f)]
        public float StartPositionPct;

        [Header("Stop position in Midi defined in pourcentaage of the whole druration of the Midi")] [Range(0f, 100f)]
        public float StopPositionPct;

        [Header("Delay to apply random change")] [Range(0f, 10f)]
        ...
        public bool FoldOutViewer;
        public bool FoldOutStartStopRamp;
        public bool FoldOutAlterMidi;
        public bool FoldOutChannelDisplay;
        public bool FoldOutRealTimeChange;
        public bool FoldOutSetStartStopPosition;
        public bool FoldOutInnerLoop;
        public bool FoldOutGeneralSettings;
        ...
        private static readonly Color ButtonColor = new(.7f, .9f, .7f, 1f);
        ...
        private long tickPositionToInsert;
        private int channelToInsert;

        //DateTime localStartTimeMidi;
        private TimeSpan realTimeMidi;
        ...
        private void Start()
        {
            ...
            for (var i = 0; i < MAX_LINE_RENDERER; i++)
            ...
            if (midiFilePlayer == null)
            {
                ...
                var fp = FindFirstObjectByType<MidiFilePlayer>();
                if (fp == null)
                    Debug.LogWarning(
                        "Can't find a MidiFilePlayer Prefab in the current Scene Hierarchy. Add it with the MPTK menu.");
                else
                    midiFilePlayer = fp;
            }
        ...
        private void Update()
        {
            ...
            foreach (var noteViewer in notesViewerCircle)
                if (noteViewer != null)
                    noteViewer.Update();
            ...
            if (midiFilePlayer != null && midiFilePlayer.MPTK_IsPlaying)
            {
                ...
                // 
                var time = Time.realtimeSinceStartup - LastTimeChange;
                if (DelayRandomSecond > 0f && time > DelayRandomSecond)
                {
                    ...
                    // Random position
                    if (IsRandomPosition)
                        midiFilePlayer.MPTK_Position =
                            Random.Range(0f, (float)midiFilePlayer.MPTK_Duration.TotalMilliseconds);

                    // Random Speed
                    if (IsRandomSpeed) midiFilePlayer.MPTK_Speed = Random.Range(0.1f, 5f);

                    // Random transpose
                    if (IsRandomTranspose) midiFilePlayer.MPTK_Transpose = Random.Range(-12, 13);
                }
            }
        }
        ...
        ///     - Accuracy is garantee (see midiFilePlayer.MPTK_Pulse which is the minimum time in millisecond between two MIDI
        ///     events).
        ...
        private bool MaestroOnMidiEvent(MPTKEvent midiEvent)
        {
            var playEvent = true;
                        ...
                        switch (midiEvent.Meta)
                        {
                            case MPTKMeta.SetTempo:
                                if (toggleChangeTempo)
                                {
                                    ...
                                    Debug.Log(
                                        $"Detected MIDI event Set Tempo {midiEvent.Value}, forced to a random value {midiFilePlayer.MPTK_Tempo}");
                                }
        ...
        ///     Warning: avoid to define this event by script because the initial loading could be not trigger in the case of
        ///     MidiPlayerGlobal id load before any other gamecomponent
        /// </summary>
        public void MaestroOnEventPresetLoaded()
        {
            ...
            Debug.Log(
                $"   Time To Download SF:     {Math.Round(MidiPlayerGlobal.MPTK_TimeToDownloadSoundFont.TotalSeconds, 3)} second");
            Debug.Log(
                $"   Time To Load SoundFont:  {Math.Round(MidiPlayerGlobal.MPTK_TimeToLoadSoundFont.TotalSeconds, 3)} second");
            Debug.Log(
                $"   Time To Load Samples:    {Math.Round(MidiPlayerGlobal.MPTK_TimeToLoadWave.TotalSeconds, 3).ToString()} second");
        ...
        /// <summary>
        ///     @brief
        ///     Event fired by MidiFilePlayer when a midi is started (set by Unity Editor in MidiFilePlayer Inspector or by script
        ///     see above)
        /// </summary>
        public void MaestroOnEventStartPlayMidi(string name)
        {
            ...
            if (midiFilePlayer != null)
            {
                ...
                infoMidi +=
                    $"Full Duration: {midiFilePlayer.MPTK_Duration} {midiFilePlayer.MPTK_DurationMS / 1000f:F2} seconds {midiFilePlayer.MPTK_TickLast} ticks\n";
                infoMidi +=
                    $"First note-on: {TimeSpan.FromMilliseconds(midiFilePlayer.MPTK_PositionFirstNote)} {midiFilePlayer.MPTK_PositionFirstNote / 1000f:F2} seconds {midiFilePlayer.MPTK_TickFirstNote} ticks\n";
                infoMidi +=
                    $"Last note-on : {TimeSpan.FromMilliseconds(midiFilePlayer.MPTK_PositionLastNote)} {midiFilePlayer.MPTK_PositionLastNote / 1000f:F2} seconds  {midiFilePlayer.MPTK_TickLastNote} ticks\n";
                ...
                infoMidi +=
                    $"Delta Ticks  : {midiFilePlayer.MPTK_MidiLoaded.MPTK_DeltaTicksPerQuarterNote} Ticks Per Quarter\n";
                ...
                foreach (var tempo in midiFilePlayer.MPTK_MidiLoaded.MPTK_TempoMap)
                {
                    var sEndTick = tempo.ToTick == long.MaxValue ? "    End" : $"{tempo.ToTick,-7:000000}";
                    infoMidi +=
                        $"   Tick:{tempo.FromTick,-7:000000} to {sEndTick}  BPM:{MPTKEvent.QuarterPerMicroSecond2BeatPerMinute(tempo.MicrosecondsPerQuarterNote):F1} \n";
                }

                if (FoldOutSetStartStopPosition && StartPositionPct > 0f)
                    midiFilePlayer.MPTK_TickCurrent = (long)(midiFilePlayer.MPTK_TickLast * (StartPositionPct / 100f));
            }

            Debug.Log(
                $"Start Play MIDI '{name}' '{midiFilePlayer.MPTK_MidiName}' Duration: {midiFilePlayer.MPTK_DurationMS / 1000f:F2} seconds  Load time: {midiFilePlayer.MPTK_MidiLoaded.MPTK_LoadTime:F2} milliseconds");
        }
        ...
        public void MaestroOnEventNotesMidi(List<MPTKEvent> midiEvents)
        {
            ...
            if (FoldOutSetStartStopPosition)
            {
                if (StopPositionPct < StartPositionPct)
                    Debug.LogWarning(
                        $"StopPosition ({StopPositionPct} %) is defined before StartPosition ({StartPositionPct} %)");

                if (StartPositionPct > 0f)
                {
                    // Convert percentage start position to tick position
                    var tickStart = (long)(midiFilePlayer.MPTK_TickLast * (StartPositionPct / 100f));
                ...
                if (StopPositionPct < 100f)
                {
                    ...
                    var tickStop = (long)(midiFilePlayer.MPTK_TickLast * (StopPositionPct / 100f));
                    if (midiFilePlayer.MPTK_TickCurrent > tickStop)
                    {
                        ...
                        else if (randomPlay)
                        {
                            ...
                            var index = Random.Range(0, MidiPlayerGlobal.MPTK_ListMidi.Count);
                    ...
                    case MPTKCommand.MetaEvent:
                        switch (midiEvent.Meta)
                        {
                            ...
                            case MPTKMeta.Marker:
                                ...
                                var text = midiEvent.Info.Replace("\\", "\n");
                                text = text.Replace("/", "\n");
        ...
        {
            var midiplayer = FindFirstObjectByType<MidiFilePlayer>();
            if (midiplayer == null)
            {
                Debug.LogWarning(
                    "Can't find a MidiFilePlayer Prefab in the current Scene Hierarchy. Add it with the MPTK menu.");
            ...
            if (midiplayer.MPTK_Load() != null)
            {
                ...
                var mptkEvents = midiplayer.MPTK_ReadMidiEvents(1000, 10000);
                ...
                foreach (var mptkEvent in mptkEvents)
                    ...
                    if (mptkEvent.Command == MPTKCommand.NoteOn)
                        Debug.Log(
                            $"Note on Time:{mptkEvent.RealTime} millisecond  Note:{mptkEvent.Value}  Duration:{mptkEvent.Duration} millisecond  Velocity:{mptkEvent.Velocity}");
        ...
        private void OnGUI()
        {
            ...
            if (midiFilePlayer != null)
            {
                ...
                scrollerWindow = GUILayout.BeginScrollView(scrollerWindow, false, false, GUILayout.Width(widthGUI + 25),
                    GUILayout.Height(Screen.height / scale.y));
                ...
                MainMenu.Display("Test MIDI File Player Scripting - Demonstrate how to use the MPTK API to Play Midi",
                    myStyle, widthGUI,
                ...
                if (GUILayout.Button($"{midiFilePlayer.MPTK_MidiIndex} - '{midiFilePlayer.MPTK_MidiName}'",
                        GUILayout.Height(40), GUILayout.Width(300)))
                    PopMidi.Show = !PopMidi.Show;
                if (Application.isEditor)
                {
                    GUILayout.Space(10);
                    GUILayout.Label(
                        "This demo is built with IMGUI, unfortunately the garbage collector causes wait times in editor mode.\nSolved by building with IL2CPP.",
                        myStyle.TitleLabel3);
                }
                ...
                if (Application.isEditor)
                {
                    ...
                    GUILayout.Label(
                        "Go to your Hierarchy, select GameObject MidiFilePlayer: inspector contains a lot of parameters to control the sound.",
                        myStyle.TitleLabel2);
                    HelperDemo.GUI_Horizontal(HelperDemo.Zone.END);
                }
        ...
        private void OnGUI_LeftPanel()
        {
            HelperDemo.GUI_Vertical(HelperDemo.Zone.BEGIN, myStyle.BacgDemosMedium,
                GUILayout.Width(widthScaledLeftPanel));
            ...
            if (midiFilePlayer != null && midiFilePlayer.MPTK_MidiLoaded != null)
            {
                ...
                FoldOutGeneralSettings = HelperDemo.FoldOut(FoldOutGeneralSettings,
                    "General Settings (volume, speed, ...)",
                    "https://mptkapi.paxstellar.com/d7/deb/class_midi_player_t_k_1_1_midi_file_player.html#details");
                if (FoldOutGeneralSettings)
                    OnGUI_MidiGeneralSettings();

                FoldOutSetStartStopPosition = HelperDemo.FoldOut(FoldOutSetStartStopPosition,
                    "Set MIDI Start & Stop Position",
                    "https://mptkapi.paxstellar.com/d7/deb/class_midi_player_t_k_1_1_midi_file_player.html#aabd956ce6d15dca46ca0d53d9be40fa7");
                if (FoldOutSetStartStopPosition)
                    OnGUI_MidiStartStopPosition();

                FoldOutViewer = HelperDemo.FoldOut(FoldOutViewer, "MIDI Viewer",
                    "https://mptkapi.paxstellar.com/d7/deb/class_midi_player_t_k_1_1_midi_file_player.html#af15c52b3fef54a544db289fe18b0ffe5");
                if (FoldOutViewer)
                    OnGUI_MidiViewer();

                FoldOutInnerLoop = HelperDemo.FoldOut(FoldOutInnerLoop, "MIDI Inner Loop [Pro]",
                    "https://mptkapi.paxstellar.com/d7/deb/class_midi_player_t_k_1_1_midi_file_player.html#a872c1de207fde3f38ce7e93cf13955be");
                if (FoldOutInnerLoop)
                    OnGUI_InnerLoop();

                // Channel setting display
                FoldOutChannelDisplay = HelperDemo.FoldOut(FoldOutChannelDisplay,
                    "Display Channels and Change Properties",
                    "https://mptkapi.paxstellar.com/d0/d99/class_midi_player_t_k_1_1_m_p_t_k_channels.html");
                if (FoldOutChannelDisplay)
                    OnGUI_ChannelChange();

                FoldOutStartStopRamp = HelperDemo.FoldOut(FoldOutStartStopRamp,
                    "Play with Crescendo and Stop with Diminuendo [Pro]",
                    "https://mptkapi.paxstellar.com/d7/deb/class_midi_player_t_k_1_1_midi_file_player.html#a5677c5289df9c77777422c65ea31af58");
                if (FoldOutStartStopRamp)
                    OnGUI_StartStopCrescendo();

                FoldOutAlterMidi = HelperDemo.FoldOut(FoldOutAlterMidi, "Modify MIDI and Play [Pro]",
                    "https://mptkapi.paxstellar.com/d7/deb/class_midi_player_t_k_1_1_midi_file_player.html#a2d46ba8d8bcd260315d57f9ae9ff4dba");
                if (FoldOutAlterMidi)
                    OnGUI_ModifyMidiAndPlay();

                FoldOutMetronome = HelperDemo.FoldOut(FoldOutMetronome, "Enable Metronome [Pro]",
                    "https://mptkapi.paxstellar.com/d3/d1d/class_midi_player_t_k_1_1_midi_synth.html#a7f4f01651da795149866f38cdd068c2c");
                if (FoldOutMetronome)
                    OnGUI_EnableMetronome();

                FoldOutRealTimeChange = HelperDemo.FoldOut(FoldOutRealTimeChange, "Real-time MIDI Change [Pro]",
                    "https://mptkapi.paxstellar.com/d3/d1d/class_midi_player_t_k_1_1_midi_synth.html#a45b22cbcee07d6012d6652b60bfce269");
                if (FoldOutRealTimeChange)
                    OnGUI_RealTimeMIDIChange();

                FoldOutEffectSoundFontDisplay = HelperDemo.FoldOut(FoldOutEffectSoundFontDisplay,
                    "Enable SoundFont Effects [Pro]",
                    "https://mptkapi.paxstellar.com/df/da3/class_midi_player_t_k_1_1_m_p_t_k_effect_sound_font.html#details");
                if (FoldOutEffectSoundFontDisplay)
                    HelperDemo.GUI_EffectSoundFont(widthIndent, midiFilePlayer.MPTK_EffectSoundFont);

                FoldOutEffectUnityDisplay = HelperDemo.FoldOut(FoldOutEffectUnityDisplay, "Enable Unity Effects [Pro]",
                    "https://mptkapi.paxstellar.com/d7/d64/class_midi_player_t_k_1_1_m_p_t_k_effect_unity.html#details");
        ...
        private void OnGUI_PlayPauseStopMIDI()
        {
            ...
            HelperDemo.LinkTo(
                "https://mptkapi.paxstellar.com/d3/d1d/class_midi_player_t_k_1_1_midi_synth.html#ae03bc14d19f3c8c9d390eaa825db08a7");
        ...
        private void OnGUI_MIDIOptions()
        {
            ...
            midiFilePlayer.MPTK_StartPlayAtFirstNote = GUILayout.Toggle(midiFilePlayer.MPTK_StartPlayAtFirstNote,
                "Start Playing on first note");
            HelperDemo.LinkTo(
                "https://mptkapi.paxstellar.com/d7/deb/class_midi_player_t_k_1_1_midi_file_player.html#ab58505a6dd72b26477f2ee206f92cf45");
            GUILayout.FlexibleSpace();

            midiFilePlayer.MPTK_StopPlayOnLastNote =
                GUILayout.Toggle(midiFilePlayer.MPTK_StopPlayOnLastNote, "Stop playing on last note");
            HelperDemo.LinkTo(
                "https://mptkapi.paxstellar.com/d7/deb/class_midi_player_t_k_1_1_midi_file_player.html#a3dec6265d9e5ef1830a88e5dacd73cbd");
            GUILayout.FlexibleSpace();
        ...
        private void OnGui_MidiTimePosition()
        {
            ...
            var timePosition = TimeSpan.FromMilliseconds(midiFilePlayer.MPTK_Position);
            ...
            var realTime =
                $"{realTimeMidi.Hours:00}:{realTimeMidi.Minutes:00}:{realTimeMidi.Seconds:00}:{realTimeMidi.Milliseconds:000} ";
            GUILayout.Label(realTime, myStyle.TitleLabel3, GUILayout.Width(90));
            HelperDemo.LinkTo(
                "https://mptkapi.paxstellar.com/d7/deb/class_midi_player_t_k_1_1_midi_file_player.html#a154e08aaba43883d20c016b2b388d3f5");

            // Delta time with the last MIDI events
            var deltaTime =
                $"Delta time with the last MIDI event: {(timePosition - realTimeMidi).TotalSeconds:F3} second";
            ...
            var sPlayTime =
                $"{timePosition.Hours:00}:{timePosition.Minutes:00}:{timePosition.Seconds:00}:{timePosition.Milliseconds:000}";
            ...
            var sRealDuration =
                $"{tsDuration.Hours:00}:{tsDuration.Minutes:00}:{tsDuration.Seconds:00}:{tsDuration.Milliseconds:000}";

            // Build slider title
            var sPosition = $"{sPlayTime} / {sRealDuration}";

            // Calculate current position as a double for using in the slider
            var currentPosition = Math.Round(midiFilePlayer.MPTK_Position / 1000d, 2);

            // Change current position with a slider
            var newPosition = Math.Round(HelperDemo.GUI_Slider(sPosition, (float)currentPosition, 0f,
                (float)tsDuration.TotalSeconds,
                false, true, 1f, 250, 140, 0), 2);
            ...
            HelperDemo.LinkTo(
                "https://mptkapi.paxstellar.com/d7/deb/class_midi_player_t_k_1_1_midi_file_player.html#aa9814f63623a1f8d67dcb0630e033a0a");

            // When seeking, replayed from the beginning of the MIDI to the new position or not
            var rawseek = GUILayout.Toggle(midiFilePlayer.MPTK_RawSeek, "Raw Seek", GUILayout.Width(75));
            if (midiFilePlayer.MPTK_RawSeek != rawseek)
                midiFilePlayer.MPTK_RawSeek = rawseek;
            HelperDemo.LinkTo(
                "https://mptkapi.paxstellar.com/d7/deb/class_midi_player_t_k_1_1_midi_file_player.html#a2ebd6f7b1b4b4effedb3b66fcfbdf851");
            ...
            var sTickPosition = $"{midiFilePlayer.MPTK_TickCurrent} / {tickLast}";

            // Change current tick with a slider
            var tick = (long)HelperDemo.GUI_Slider(sTickPosition, midiFilePlayer.MPTK_TickCurrent, 0f, tickLast,
                false, true, midiFilePlayer.MPTK_DeltaTicksPerQuarterNote, 250, 140, 0);
        ...
        private void OnGUI_MidiGeneralSettings()
        {
            ...
            midiFilePlayer.MPTK_Volume = HelperDemo.GUI_Slider("Global Volume:", midiFilePlayer.MPTK_Volume, 0f,
                Constant.MAX_VOLUME,
                false, true, dynamicDelta(midiFilePlayer.MPTK_Volume), 170, 250, 80);

            //// Transpose each note
            midiFilePlayer.MPTK_Transpose = (int)HelperDemo.GUI_Slider("Note Transpose:", midiFilePlayer.MPTK_Transpose,
                -24, 24,
                false, true, 1f, 170, 250, 80);

            // Change speed
            midiFilePlayer.MPTK_Speed = HelperDemo.GUI_Slider("MIDI Reading Speed:", midiFilePlayer.MPTK_Speed,
                Constant.MIN_SPEED, Constant.MAX_SPEED,
                false, true, dynamicDelta(midiFilePlayer.MPTK_Speed), 170, 250, 80);
        ...
        private void OnGUI_MidiStartStopPosition()
        {
            ...
            GUILayout.Label("Set the start and stop points in the MIDI based on tick values. For ease, " +
                            ...
                            "For precise looping in the MIDI, it's better using the Inner Loop features (Pro version).",
                myStyle.TitleLabel3);

            HelperDemo.GUI_Horizontal(HelperDemo.Zone.BEGIN);

            // Calculate tick position from the percentage
            var tickStartPosition = (long)(midiFilePlayer.MPTK_TickLast * (StartPositionPct / 100f));
            var tickStopPosition = (long)(midiFilePlayer.MPTK_TickLast * (StopPositionPct / 100f));
            var label =
                $"Start from tick: {tickStartPosition} to {tickStopPosition} {midiFilePlayer.MPTK_TickCurrent} / {midiFilePlayer.MPTK_TickLast}";
            ...
            if (GUILayout.Button(MidiFilePlayer.ModeStopPlayLabel[(int)ModeLoop]))
            {
                ...
                foreach (MidiFilePlayer.ModeStopPlay mode in Enum.GetValues(typeof(MidiFilePlayer.ModeStopPlay)))
                    ...
                    (
                        ...
                        () =>
                        {
                            midiFilePlayer.MPTK_ModeStopVoice = mode;
                            ModeLoop = mode;
                        }
                    );
        ...
        private void OnGUI_MidiViewer()
        {
            ...
            GUILayout.Label(
                "Display a simple MIDI viewver. Simple line are created for each note plays from the MIDI.\n" +
            ...
            if (ViewerEnabled)
            {
                NoteViewerCircle.DistanceShadow = HelperDemo.GUI_Slider("   Shadow:", NoteViewerCircle.DistanceShadow,
                    0f, 1f, false, true, 0.01f, 200, 400, 100);
                NoteViewerCircle.WidthLine = HelperDemo.GUI_Slider("   Width line:", NoteViewerCircle.WidthLine, 0.01f,
                    5f, false, true, 0.01f, 200, 400, 100);
                NoteViewerCircle.GrowingPhase = HelperDemo.GUI_Slider("   Growing phase time:",
                    NoteViewerCircle.GrowingPhase, 0, 2f, false, true, 0.01f, 200, 400, 100);
                NoteViewerCircle.FadingPhase = HelperDemo.GUI_Slider("   Fading phase time:",
                    NoteViewerCircle.FadingPhase, 0, 2f, false, true, 0.01f, 200, 400, 100);
            }
        ...
        private void OnGUI_InnerLoop()
        {
            ...
            if (InnerLoopEnabled)
            {
                ...
                GUILayout.Label($"Current Tick: {midiFilePlayer.MPTK_TickCurrent,-5}", myStyle.TitleLabel3,
                    GUILayout.Width(160));
                GUILayout.Label($"Current Time: {Math.Round(midiFilePlayer.MPTK_Position / 1000d, 2),-5}",
                    myStyle.TitleLabel3, GUILayout.Width(160));
                ...
                switch (InnerSlider)
                {
                    case 1:
                        deltaTicks = 1;
                        sliderText = "Slider by one tick";
                        break;
                    case 2:
                        deltaTicks = midiFilePlayer.MPTK_DeltaTicksPerQuarterNote / 4;
                        sliderText = $"Slider by Sixteenth ({deltaTicks} ticks):";
                        break;
                    case 3:
                        deltaTicks = midiFilePlayer.MPTK_DeltaTicksPerQuarterNote / 2;
                        sliderText = $"Slider by Eighth ({deltaTicks} ticks):";
                        break;
                    case 4:
                        deltaTicks = midiFilePlayer.MPTK_DeltaTicksPerQuarterNote;
                        sliderText = $"Slider by Quarter ({deltaTicks} ticks):";
                        break;
                    case 5:
                        deltaTicks = midiFilePlayer.MPTK_DeltaTicksPerQuarterNote * 2;
                        sliderText = $"Slider by Half ({deltaTicks} ticks):";
                        break;
                    case 6:
                        deltaTicks = midiFilePlayer.MPTK_DeltaTicksPerQuarterNote * 4;
                        sliderText = $"Slider by Whole ({deltaTicks} ticks):";
                        break;
                }

                InnerSlider = (int)HelperDemo.GUI_Slider(sliderText, InnerSlider, 1, 6, false, true, 1, 200, 50, 0);
                HelperDemo.GUI_Horizontal(HelperDemo.Zone.END);

                // Change start playing resume loop, end loop. Add a deltaticks between Resume and End ticks position
                InnerLoopStart = (long)HelperDemo.GUI_Slider("Start from:", InnerLoopStart, 0,
                    midiFilePlayer.MPTK_TickLast, false, true, deltaTicks, 200, 400, 100);

                if (InnerLoopResume < InnerLoopStart)
                    InnerLoopResume = InnerLoopStart;
                InnerLoopResume = (long)HelperDemo.GUI_Slider("Resume loop at:", InnerLoopResume, 0,
                    midiFilePlayer.MPTK_TickLast, false, true, deltaTicks, 200, 400, 100);

                if (InnerLoopEnd < InnerLoopResume)
                    InnerLoopEnd = InnerLoopResume + deltaTicks;
                InnerLoopEnd = (long)HelperDemo.GUI_Slider("Loop end at:", InnerLoopEnd, 0,
                    midiFilePlayer.MPTK_TickLast, false, true, deltaTicks, 200, 400, 100);
            }
        ...
        private void OnGUI_ChannelChange()
        {
            ...
            if (GUILayout.Button("Random!", GUILayout.Width(100)))
                ...
                foreach (var mptkChannel in midiFilePlayer.MPTK_Channels)
                    mptkChannel.ForcedPreset = Random.Range(0, 127);
            //! [ExampleUsingChannelAPI_1]
            midiFilePlayer.MPTK_Channels.EnableResetChannel =
                GUILayout.Toggle(midiFilePlayer.MPTK_Channels.EnableResetChannel, "Reset when MIDI start");
            ...
            // Also available for MidiStreamPlayer, MidiInReader, MidiExternalPlayer.
            for (var channel = 0; channel < midiFilePlayer.MPTK_Channels.Length; channel++)
            {
                HelperDemo.GUI_Horizontal(HelperDemo.Zone.BEGIN);
                ...
                // Display preset name
                GUILayout.Label(midiFilePlayer.MPTK_Channels[channel].PresetName ?? "not set", myStyle.TitleLabel3,
                    GUILayout.Width(140));

                // Display preset and bank index
                var presetNum = midiFilePlayer.MPTK_Channels[channel].PresetNum;
                var bankNum = midiFilePlayer.MPTK_Channels[channel].BankNum;
                var presetForced = midiFilePlayer.MPTK_Channels[channel].ForcedPreset;
                //! [ExampleUsingChannelAPI_One]
                ...
                var forcePreset = (int)HelperDemo.GUI_Slider(sPreset, presetNum, -1f, 127f, true, widthCaption: 120,
                    widthSlider: 100, widthLabelValue: -1);
                ...
                GUILayout.Label($"{midiFilePlayer.MPTK_Channels[channel].NoteCount,-5}", myStyle.LabelRight,
                    GUILayout.Width(100));
                ...
                var state = GUILayout.Toggle(midiFilePlayer.MPTK_Channels[channel].Enable, "", GUILayout.MaxWidth(20));
                if (state != midiFilePlayer.MPTK_Channels[channel].Enable)
                {
                    midiFilePlayer.MPTK_Channels[channel].Enable = state;
                    Debug.LogFormat("Channel {0} state:{1}, preset:{2}", channel, state,
                        midiFilePlayer.MPTK_Channels[channel].PresetName ?? "not set"); /*2.84*/
                }
                //! [ExampleUsingChannelAPI_7]
                ...
                var currentVolume = midiFilePlayer.MPTK_Channels[channel].Volume;
                var volume = HelperDemo.GUI_Slider(null, currentVolume, 0f, 1f, true, false, widthCaption: -1,
                    widthSlider: 100, widthLabelValue: 40);
        ...
        private void OnGUI_ModifyMidiAndPlay()
        {
            ...
            GUILayout.Label(
                "It's possible to change the MIDI events before playing. This demo loads the selected MIDI, adds some notes and plays the MIDI without reloading it. Result not guaranteed!",
                myStyle.TitleLabel3);

            countNoteToInsert = (int)HelperDemo.GUI_Slider("Count notes to insert:", countNoteToInsert, 1, 100,
                widthCaption: 170, widthSlider: 250, widthLabelValue: 50);

            tickPositionToInsert = (long)HelperDemo.GUI_Slider("Tick position to insert:", tickPositionToInsert, 0,
                midiFilePlayer.MPTK_TickLast, widthCaption: 170, widthSlider: 250, widthLabelValue: 50);

            var quarterPosition = tickPositionToInsert / midiFilePlayer.MPTK_DeltaTicksPerQuarterNote;

            var newQuarter = (long)HelperDemo.GUI_Slider("Tick position by quarter:", quarterPosition, 0,
                midiFilePlayer.MPTK_TickLast / midiFilePlayer.MPTK_DeltaTicksPerQuarterNote, widthCaption: 170,
                widthSlider: 250, widthLabelValue: 50);
                    ...
                    for (int insertNote = 0; insertNote < countNoteToInsert; insertNote++)
                    {
                        ...
                        midiFilePlayer.MPTK_MidiEvents.Insert(0,
                            new MPTKEvent()
                            {
                                ...
                                Length = tickDuration,
                                Duration =
 (long)(tickDuration * midiFilePlayer.MPTK_Pulse), // Transform ticks to millisecond
                                Tick = tickToInsert,
                            });
            ...
            HelperDemo.LinkTo(
                "https://mptkapi.paxstellar.com/d7/deb/class_midi_player_t_k_1_1_midi_file_player.html#a7c1b1b1efab0022731f69e5161952c59");
...
#else
            GUILayout.Label("Plays an extra drum sound with each beat is available with Maestro MPTK Pro.",
                myStyle.TitleLabel3);
#endif

            volumeMetronome = (int)HelperDemo.GUI_Slider("Beat Volume", volumeMetronome, 0, 127, widthCaption: 170,
                widthSlider: 250, widthLabelValue: 50);
        ...
        private void OnGUI_RealTimeMIDIChange()
        {
            ...
            HelperDemo.GUI_Horizontal(HelperDemo.Zone.BEGIN);
            GUILayout.Label("Transpose one octave depending on the channel value and disable drum.",
                myStyle.TitleLabel3, GUILayout.Width(400));
        ...
        {
            if (!string.IsNullOrEmpty(infoMidi) || !string.IsNullOrEmpty(infoLyrics) ||
                !string.IsNullOrEmpty(infoCopyright) || !string.IsNullOrEmpty(infoSeqTrackName))
            {
                //
                ...
                var savedBackColor = GUI.backgroundColor;
                ...
                {
                    scrollPos1 =
                        GUILayout.BeginScrollView(scrollPos1, false, true); //, GUILayout.Height(heightLyrics));
                ...
                {
                    scrollPos2 =
                        GUILayout.BeginScrollView(scrollPos2, false, true); //, GUILayout.Height(heightLyrics));
