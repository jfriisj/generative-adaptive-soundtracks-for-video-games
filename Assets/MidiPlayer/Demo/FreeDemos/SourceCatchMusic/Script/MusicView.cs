...
namespace MPTKDemoCatchMusic
{
    ...
    public class MusicView : MonoBehaviour
    {
        ...
        /// <summary>
        ///     A prefab MidiFilePlayer must exists in the Unity Editor Hierarchy and must be associated in the MusicView
        ///     GameObject inspector.
        ///     In ths demo, the MidiFilePlayer is muted (Send MIDI ti the Synth disabled).
        ///     It is only used to receive MIDI events thanks to the MIDI sequencer and the event defined with OnEventNotesMidi
        ///     (see bellow).
        /// </summary>
        public MidiFilePlayer midiFilePlayer;

        /// <summary>
        ///     A prefab MidiStreamPlayer must exists in the Unity Editor Hierarchy and must be associated in the MusicView
        ///     GameObject inspector.
        ///     A reference to the MidiStreamPlayer is associated to each NoteView and ControlView GameObject created for each MIDI
        ///     event.
        ...
        /// <summary>
        ///     Hold a GameObject created for each MIDI note received from the MidiFilePlayer sequencer. Created with Instantiate
        ///     <NoteView>
        ///         ().
        ///         with:
        ///         - note associated
        ...
        /// <summary>
        ///     Hold a GameObject created for each MIDI controller received from the MidiFilePlayer sequencer. Created with
        ///     Instantiate
        ///     <NoteView>
        ///         ().
        ///         with:
        ...
        private void Start()
        {
            ...
            if (midiFilePlayer != null)
            {
                ...
                Debug.Log(
                    "MusicView: Maestro Event MidiFilePlayer.OnEventNotesMidi set by script (see MusicView.cs). Setting with the inspector is also possible.");
                midiFilePlayer.OnEventNotesMidi.AddListener(NotesToPlay);
            }
            else
            {
                Debug.Log(
                    "No MidiFilePlayer prefab detected. Add it to your Hierarchy and defined it in MusicView inspector.");
            }
        }
        ...
        private void Update()
        {
            if (midiFilePlayer != null && midiFilePlayer.MPTK_IsPlaying)
            {
                ...
                var time = Time.realtimeSinceStartup - LastTimeCollider;
                if (time > DelayCollider + FirstDelayCollider)
                {
                    ...
                    float zone = 10;
                    var position = new Vector3(Random.Range(minX + zone, maxX - zone), -5,
                        Random.Range(minZ + zone, maxZ - zone));
                    // Instantiate collider (sphere) to interact with note and patch change.
                    var n = Instantiate(Collider, position, Quaternion.identity);
        ...
        private void OnGUI()
        {
            ...
            if (midiFilePlayer != null)
            {
                ...
                GUILayout.Label("MIDI '" + midiFilePlayer.MPTK_MidiName +
                                (midiFilePlayer.MPTK_IsPlaying ? "' is playing" : " is not playing"));
                GUILayout.BeginHorizontal();
                GUILayout.Label("MIDI Position :", GUILayout.Width(100));
                var currentposition = Math.Round(midiFilePlayer.MPTK_Position / 1000d, 2);
                var newposition =
                    Math.Round(
                        GUILayout.HorizontalSlider((float)currentposition, 0f, midiFilePlayer.MPTK_DurationMS / 1000f,
                            GUILayout.Width(200)), 2);
                ...
                var speed = GUILayout.HorizontalSlider(midiFilePlayer.MPTK_Speed, 0.1f, 5f, GUILayout.Width(200));
                ...
                var y = GUILayout.HorizontalSlider(Cam.transform.position.y, 50f, 150f, GUILayout.Width(200));
                ...
                var x = GUILayout.HorizontalSlider(Cam.transform.position.x, -50f, 50f, GUILayout.Width(200));
        ...
        public void NotesToPlay(List<MPTKEvent> notes)
        {
            ...
            foreach (var mptkEvent in notes)
                switch (mptkEvent.Command)
                {
                    ...
                    case MPTKCommand.PatchChange:
                    {
                        ...
                        // Z position is set depending the note value:mptkEvent.Value
                        var z = Mathf.Lerp(minZ, maxZ, mptkEvent.Value / 127f);
                        // Y position is set depending the count of objects at the z position
                        countZ[Convert.ToInt32(z - minZ)]++;
                        var position = new Vector3(maxX, 8f + countZ[Convert.ToInt32(z - minZ)] * 4f, z);
                        // Instantiate a GameObject to represent this midi event in the 3D world
                        var patchview = Instantiate(ControlDisplay, position, Quaternion.identity);
        ...
        public void Clear()
        {
            var components = FindObjectsByType<NoteView>(FindObjectsSortMode.None);
            foreach (var noteview in components)
                if (noteview.enabled)
