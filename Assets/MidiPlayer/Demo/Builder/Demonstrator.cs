...
namespace MidiPlayerTK
{
    ...
    {
        private static readonly Color ColorBack = new(0.5f, 0.5f, 0.5f);
        private static readonly Color ColorContentList = new(0.7f, 0.7f, 0.7f);
        private static readonly Color ColorHover = new(0.75f, 0.75f, 0.75f);
        private static readonly Color ColorHeaderList = new(0.5f, 0.5f, 0.5f);
        ...
        public VisualTreeAsset RowTemplate { get; set; }
        ...
        public void LoadCSV()
        {
            try
            {
                ...
                var mytxtData = Resources.Load<TextAsset>("DemosListCommon");
                var text = Encoding.UTF8.GetString(mytxtData.bytes);
                text = text.Replace("\n", "");
                text = text.Replace("\r", "");
                text = text.Replace("<br>", "\n");
                var listDemos = text.Split("ENDLINE");
                if (listDemos != null)
                    foreach (var demo in listDemos)
                    {
                        var colmuns = demo.Split(';');
                        if (colmuns.Length >= 5)
        ...
        {
            var gbHeader = Root.Q<GroupBox>("gbHeader");
            ...
            Root.Q<Label>("labTitle").text = "Midi Player Tool Kit Demonstration Loader - V" + Constant.version +
                                             " - " + Constant.releaseDate;
#if UNITY_WEBGL
            string title =
 "Welcome to the first WebGL demo of Maestro MPTK! Please note that due to WebGL constraints,\n";
            ...
            Root.Q<Button>("btWebSite").RegisterCallback<ClickEvent>(evt =>
            {
        ...
        */
        public void AddRow(Demonstrator demo, int index, int selected)
        {
            ...
            var gbOneRow = row.Q<GroupBox>("gbOneRow");
            ...
            else
            {
                ...
                gbOneRow.RegisterCallback<MouseLeaveEvent>(evt =>
                {
                    gbOneRow.style.backgroundColor = backgroundcolorSaved;
                });
                ...
                gbOneRow.RegisterCallback<ClickEvent>(evt =>
                {
                    ...
                    {
                        SceneManager.LoadScene(SceneUtility.GetBuildIndexByScenePath(demo.SceneName),
                            LoadSceneMode.Single);
                    }
                    else
                    {
#if UNITY_EDITOR //NOT_AVAILABLE_OUT_OF_EDITOR_FOLDER
                        var freepro = demo.Version == "Free" ? "FreeDemos" : "ProDemos";
                        var scenePath = $"Assets/MidiPlayer/Demo/{freepro}/{demo.SceneName}.unity";
                        try
                        {
