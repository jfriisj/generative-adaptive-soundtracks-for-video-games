...
namespace DemoMVP
{
    ...
    ///     Demonstration:      Play random note with random instrument when the space key is pressed, stop the note when the
    ///     key is released.
    ///     Implementation:     Add an empty gameobject in your Unity Scene then add this script to this gameobject.
    ///     Running and using:  Play and stroke the space key, a random note will be played. Release the key, the note is
    ///     stopped.
    /// </summary>
    public class TheSimplestRandomPlayer : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (midiStreamPlayer == null)
            {
                ...
                var go = new GameObject("HoldsMaestroPrefab");
