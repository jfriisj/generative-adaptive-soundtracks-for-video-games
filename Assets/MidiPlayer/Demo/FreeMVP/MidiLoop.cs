...
namespace DemoMVP
{
    ...
    ///     As usual with a MVP demo, focus is on the essentials: no value check, no error catch, no optimization, limited
    ///     functions ...
    /// </summary>
    public class MidiLoop : MonoBehaviour
    {
        ...
        /// </summary>
        [Header("Play only this channel if greater or equal to 0")] [Range(-1, 15)]
        ...
        ///     warning: when the events are reveived it's too late to stop playing them, they are already in the pipeline of the
        ///     synth.
        ...
        public void MidiReadEvents(List<MPTKEvent> midiEvents)
        {
            ...
            Debug.Log(
                $"OnEventNotesMidi: midiEvents.Count:{midiEvents.Count}  MPTK_TickCurrent:{midiFilePlayer.MPTK_TickCurrent}");
        ...
        private long ConvertBarToTick(int bar)
        {
            // was MPTK_NumberQuarterBeat, replaced by MPTK_NumberBeatsMeasure 
            return bar * midiFilePlayer.MPTK_MidiLoaded.MPTK_NumberBeatsMeasure *
                   midiFilePlayer.MPTK_MidiLoaded.MPTK_DeltaTicksPerQuarterNote;
        }
    }
}