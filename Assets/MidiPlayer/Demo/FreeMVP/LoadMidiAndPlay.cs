...
namespace DemoMVP
{
    ...
    public class LoadMidiAndPlay : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            {
                Debug.LogWarning(
                    "No MidiFilePlayer Prefab found in the current Scene Hierarchy. See 'Maestro / Add Prefab' in the menu.");
            }
            else
            {
                ...
                var midiloaded = midiFilePlayer.MPTK_Load();

                if (midiloaded != null)
                {
                    Debug.Log(
                        $"Duration: {midiloaded.MPTK_Duration.TotalSeconds} seconds, Initial Tempo: {midiloaded.MPTK_InitialTempo}, MIDI Event Count: {midiloaded.MPTK_ReadMidiEvents().Count}");
                    ...
                    foreach (var mptkEvent in midiloaded.MPTK_MidiEvents)
                        if (mptkEvent.Command == MPTKCommand.MetaEvent && mptkEvent.Meta == MPTKMeta.SetTempo)
                        {
                            ...
                            var bpm = MPTKEvent.QuarterPerMicroSecond2BeatPerMinute(mptkEvent.Value);
                            ...
                            Debug.Log(
                                $"   Tempo doubled at tick position {mptkEvent.Tick} and {mptkEvent.RealTime / 1000f:F2} seconds. New tempo: {MPTKEvent.QuarterPerMicroSecond2BeatPerMinute(mptkEvent.Value)} BPM");
                        }
