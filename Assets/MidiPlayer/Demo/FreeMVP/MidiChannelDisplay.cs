...
namespace DemoMVP
{
    ...
    ///     Therefore, error tests are absent, the user interface is almost non-existent and manipulations in Unity are
    ///     reduced.
    ///     The goal is rather to learn how to use the Maestro API and then progress by building more complex applications.
    ///     Maestro is based on the use of prefabs (MidiFilePlayer, MidiStreamPlayer, �) which must be added in the Unity
    ///     editor in the hierarchy of your project.
    ///     In these demos, we prefer to create the prefabs by script to avoid manipulations in the editor.
    ///     It is rather recommended to create Prefabs in Unity Editor to take advantage of the Inspectors and its many
    ...
    public class MidiChannelDisplay : MonoBehaviour
    {
        ...
        public void BuildInfoChannels()
        {
            ...
            for (var channel = 0; channel < midiPlayer.MPTK_Channels.Length; channel++)
                ...
                if (midiPlayer.MPTK_Channels[channel].NoteCount > 0)
                {
                    ...
                    var presetNum = midiPlayer.MPTK_Channels[channel].PresetNum;
                    logChannelInfo.Append(
                        $" Preset: {presetNum: 000} {midiPlayer.MPTK_Channels[channel].PresetName ?? "not set",-20}");

                    // Sustain enabled?
                    var sustain = midiPlayer.MPTK_Channels[channel].Controller(MPTKController.Sustain) < 64
                        ? false
                        : true;
