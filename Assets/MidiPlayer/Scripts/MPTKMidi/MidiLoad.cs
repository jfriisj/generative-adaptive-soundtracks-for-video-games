...
{
    /// <summary>
    ///     Base class for loading a MIDI file and reading MIDI event in a MIDI sequencer.
    ///     <b>It's not possible to instanciate directly this class.</b>\n
    ///     Rather, use MidiFilePlayer to load a MIDI
    ///     This class is used by MidiFilePlayer, MidiListPlayer, MidiFileWrite2, MidiFileLoader (see members MPTK_MidiLoaded
    ///     of these classes).\n
    /// </summary>
    public class MidiLoad
    {
            ...
            try
            {
                    ...
                    MidiPlayerGlobal.CurrentMidiSet.MidiFiles.Count > 0)
                {
                    if (index >= 0 && index < MidiPlayerGlobal.CurrentMidiSet.MidiFiles.Count)
                    {
                        ...
                        var midiname = MidiPlayerGlobal.CurrentMidiSet.MidiFiles[index];
                        var mididata = Resources.Load<TextAsset>(Path.Combine(MidiPlayerGlobal.MidiFilesDB, midiname));
        ...
        public bool MPTK_Load(byte[] datamidi, bool strict = false)
        {
            ...
            {
                timeStartLoad = DateTime.Now;
                midifile = new MidiFile(datamidi, strict, MPTK_ExtendedText);
                if (midifile != null)
                {
        ...
        {
            var midievents = new List<MPTKEvent>();
            try
            {
                if (MPTK_MidiEvents != null)
                    foreach (var trackEvent in MPTK_MidiEvents)
                    {
                        //if (Quantization != 0)
        ...
        public MPTKEvent MPTK_SearchEventFromTime(double time)
        {
            ...
            //    }
            var lowIndex = 0;
            var highIndex = MPTK_MidiEvents.Count - 1;
            var found = -1;
            var lTime = (long)time;
            while (found < 0)
            {
                var middleIndex = (lowIndex + highIndex) / 2; // bug corrected in v2.9.0 Was low + high / 2 // ;-)
                var middleTime = (long)MPTK_MidiEvents[middleIndex].RealTime;
                if (lTime < middleTime)
            ...
            if (found >= 0)
            {
                var mptkEvent = MPTK_MidiEvents[found];
                if (mptkEvent != null)
                    return mptkEvent;
            }
        ...
        /// <summary>
        ///     @deprecated MidiLoad.BeatPerMinute2QuarterPerMicroSecond is deprecated and will be removed. Please investigate
        ///     MPTKEvent.BeatPerMinute2QuarterPerMicroSecond.
        /// </summary>
        public static int MPTK_BPM2MPQN(int bpm)
        {
            Debug.LogWarning(
                "MidiLoad.BeatPerMinute2QuarterPerMicroSecond is deprecated and will be removed. Please investigate MPTKEvent.BeatPerMinute2QuarterPerMicroSecond in place");
            return 60000000 / bpm;
        }

        /// <summary>
        ///     @deprecated MidiLoad.QuarterPerMicroSecond2BeatPerMinute is deprecated and will be removed. Please investigate
        ///     MPTKEvent.QuarterPerMicroSecond2BeatPerMinute.
        /// </summary>
        public static int MPTK_MPQN2BPM(int microsecondsPerQuaterNote)
        {
            Debug.LogWarning(
                "MidiLoad.QuarterPerMicroSecond2BeatPerMinute is deprecated and will be removed. Please investigate MPTKEvent.QuarterPerMicroSecond2BeatPerMinute in place");
        ...
        public MPTKEvent.EnumLength NoteLength(MPTKEvent mptkEvent)
        {
            ...
            if (mptkEvent.Length >= MPTK_DeltaTicksPerQuarterNote * 2)
                return MPTKEvent.EnumLength.Half;
            if (mptkEvent.Length >= MPTK_DeltaTicksPerQuarterNote)
        ...
        public void MPTK_DisplayMidiAttributes()
        {
            ...
            else
            {
                ...
                Debug.Log(
                    $"MPTK_TickFirstNote:\t\t{MPTK_TickFirstNote} ticks \t\tMPTK_PositionFirstNote:\t{MPTK_PositionFirstNote / 1000f:F2} second {TimeSpan.FromMilliseconds(MPTK_PositionFirstNote)} ");
                Debug.Log(
                    $"MPTK_TickLastNote:\t\t{MPTK_TickLastNote} ticks \t\tMPTK_PositionLastNote:\t{MPTK_PositionLastNote / 1000f:F2} second {TimeSpan.FromMilliseconds(MPTK_PositionLastNote)}");
        ...
        private void calculateTiming()
        {
            ...
            else
            {
                ...
                if (MPTK_LogLoadEvents)
                    MPTK_TempoMap.ForEach(tempo =>
                        Debug.Log(
                            $"Tempo change From:{tempo.FromTick} To:{tempo.ToTick} MPQN:{tempo.MicrosecondsPerQuarterNote,10} BPM:{MPTKEvent.QuarterPerMicroSecond2BeatPerMinute(tempo.MicrosecondsPerQuarterNote),3} Pulse:{tempo.Pulse,2:F2} Cumul:{tempo.FromTime,6:F2}"));
                ...
                MPTK_MidiEvents.ForEach(midievent =>
                {
                    ...
                    var tempoMap = MPTK_TempoMap[indexTempo];
                    var newTime = tempoMap.FromTime + (midievent.Tick - tempoMap.FromTick) * tempoMap.Pulse;
                    midievent.RealTime = (float)newTime;

                    indexSign = MPTKSignature.FindSegment(MPTK_SignMap, midievent.Tick, indexSign);
                    var signMap = MPTK_SignMap[indexSign];
        ...
        {
            if (MPTK_MidiEvents != null)
                foreach (var midievent in MPTK_MidiEvents)
                    if (midievent.Command == MPTKCommand.NoteOn)
                        return midievent;
        ...
        ///     @details
        ///     This is done automatically when loading a MIDI file but not when MIDI events are added by script. In this case you
        ///     will have to call this method.
        /// </summary>
        public void MPTK_ComputeDuration()
        {
            if (MPTK_MidiEvents != null && MPTK_MidiEvents.Count > 0)
            {
                var lastEvent = MPTK_MidiEvents[MPTK_MidiEvents.Count - 1];
                ...
                if (MPTK_EventLastNote != null)
                {
                    ...
                    var endLastNote =
                        MPTK_EventLastNote.RealTime +
                        MPTK_EventLastNote
                            .Duration; // v2.12.1 now take into account the duration of the last note-on event. thx Izanol.
        ...
        public static string BuildOSPath(string filename)
        {
            try
            {
                var pathMidiFolder = Path.Combine(Application.dataPath, MidiPlayerGlobal.PathToMidiFile);
                var pathfilename = Path.Combine(pathMidiFolder, filename + MidiPlayerGlobal.ExtensionMidiFile);
                return pathfilename;
            }
            catch (Exception ex)
            {
        ...
        private List<MPTKEvent> ConvertNAudioEventsToMPTKEvents()
        {
            try
            {
                ...
                indexLoadingTrack = 0;
                var mptkEvents = new List<MPTKEvent>();
                foreach (var nAudioTrack in midifile.Events)
                {
                    foreach (var nAudioMidievent in nAudioTrack)
                        // V2.89.0 able to remove End Track event
                        if (nAudioMidievent.CommandCode == MidiCommandCode.MetaEvent &&
                            ((MetaEvent)nAudioMidievent).MetaEventType == MetaEventType.EndTrack && !MPTK_KeepEndTrack)
                        {
                        ...
                        {
                            var mptkEvent = ConvertNAudioEventToMPTKEvent(nAudioMidievent);
                            if (mptkEvent != null)
            ...
            switch (midiEvent.CommandCode)
            {
                    ...
                    //if (((NoteOnEvent)mptkEvent.Event).OffEvent != null)
                {
                        ...
                        // V2.88   if (noteon.Velocity != 0)
                    {
                        ...
                        if (MPTK_LogLoadEvents)
                            Debug.Log(BuildInfoTrack(mptkEvent) +
                                      $"NoteOn {mptkEvent.Value:000} {noteon.NoteName,-4}\tDuration:{noteon.NoteLength,5} ticks {mptkEvent.Duration:000} ms {NoteLength(mptkEvent)}\tVelocity:{noteon.Velocity,3}");
                    }
                    else // It's a noteon which means a noteoff
                    {
                        ...
                        if (MPTK_KeepNoteOff) // && noteon.OffEvent == null && noteon.NoteLength!=0)
                        {
                            ...
                            if (MPTK_LogLoadEvents)
                                Debug.Log(BuildInfoTrack(mptkEvent) +
                                          $"NoteOff {mptkEvent.Value:000}\t{noteon.NoteName,-4}\tFrom NoteOn");
                        }
                    }
                }
                    break;
                ...
                case MidiCommandCode.NoteOff:
                    if (MPTK_KeepNoteOff)
                    {
                        ...
                        if (MPTK_LogLoadEvents)
                            Debug.Log(BuildInfoTrack(mptkEvent) +
                                      $"NoteOff {mptkEvent.Value:000}\t{noteoff.NoteName,-4}\tFrom file");
                    }
                ...
                case MidiCommandCode.PitchWheelChange:
                    ...
                    mptkEvent = new MPTKEvent
                    {
                        ...
                        Channel = midiEvent.Channel - 1,
                        Value = pitch
                            .Pitch // Pitch Wheel Value 0 is minimum, 0x2000 (8192) is default, 0x3FFF (16383) is maximum
                    };
                    if (MPTK_LogLoadEvents)
                        Debug.Log(BuildInfoTrack(mptkEvent) + string.Format("PitchWheelChange {0}", pitch.Pitch));
                    break;

                case MidiCommandCode.ControlChange:
                    var controlchange = (ControlChangeEvent)midiEvent;
                    mptkEvent = new MPTKEvent
                    {
                        Track = indexLoadingTrack,
                        Tick = midiEvent.AbsoluteTime,
                    ...
                    if (MPTK_LogLoadEvents)
                        Debug.Log(BuildInfoTrack(mptkEvent) +
                                  $"Control 0x{mptkEvent.Controller:X}/{mptkEvent.Controller} {mptkEvent.Value}");

                    break;

                case MidiCommandCode.PatchChange:
                    var change = (PatchChangeEvent)midiEvent;
                    mptkEvent = new MPTKEvent
                    {
                        Track = indexLoadingTrack,
                        Tick = midiEvent.AbsoluteTime,
                    ...
                    if (MPTK_LogLoadEvents)
                        Debug.Log(BuildInfoTrack(mptkEvent) + string.Format("Preset   {0,3:000} {1}", change.Patch,
                            PatchChangeEvent.GetPatchName(change.Patch)));
                    break;
                ...
                case MidiCommandCode.MetaEvent:
                    ...
                    switch (meta.MetaEventType)
                    {
                        ...
                        case MetaEventType.KeySignature:
                            var keysig = (KeySignatureEvent)meta;
                            ...
                            mptkEvent.Value = MPTKEvent.BuildIntFromBytes((byte)MPTK_KeySigSharpsFlats,
                                (byte)MPTK_KeySigMajorMinor, 0, 0);
                            mptkEvent.Info = $"SharpsFlats:{MPTK_KeySigSharpsFlats} MajorMinor:{MPTK_KeySigMajorMinor}";
                            break;

                        case MetaEventType.TimeSignature:
                            var timesig = (TimeSignatureEvent)meta;
                            ...
                            if (MPTK_TicksInMetronomeClick == 0)
                                MPTK_TicksInMetronomeClick = timesig.TicksInMetronomeClick;
                            if (MPTK_No32ndNotesInQuarterNote == 0)
                                MPTK_No32ndNotesInQuarterNote = timesig.No32ndNotesInQuarterNote;
                            mptkEvent.Value = MPTKEvent.BuildIntFromBytes((byte)timesig.Numerator,
                                (byte)timesig.Denominator, (byte)timesig.TicksInMetronomeClick,
                                (byte)timesig.No32ndNotesInQuarterNote);
                            ...
                            // if (MPTK_EnableChangeTempo)
                        {
                            ...
                            var microsecondsPerQuarterNote = ((TempoEvent)meta).MicrosecondsPerQuarterNote;
                            if (microsecondsPerQuarterNote <= 0 /* || tempo <= 0*/)
                                // tempo = 120;
                                microsecondsPerQuarterNote = MPTKEvent.BeatPerMinute2QuarterPerMicroSecond(120);
                            ...
                            if (MPTK_LogLoadEvents)
                                Debug.Log(BuildInfoTrack(mptkEvent) + string.Format(
                                    "Meta     {0,-15} 'Tempo:{1:F2} MicrosecondsPerQuarterNote:{2}'",
                                    meta.MetaEventType, tempoEvent.Tempo, tempoEvent.MicrosecondsPerQuarterNote));
                        }
                            break;
                        ...
                        default:
                            ...
                            if (MPTK_LogLoadEvents)
                                Debug.LogWarning($"This Meta MIDI event is not processed by Maestro: {midiEvent}");
                            break;
                    }

                    if (MPTK_LogLoadEvents && mptkEvent != null && !string.IsNullOrEmpty(mptkEvent.Info))
                        Debug.Log(BuildInfoTrack(mptkEvent) +
                                  string.Format("Meta     {0,-15} '{1}'", mptkEvent.Meta, mptkEvent.Info));
                    break;

                default:
                    // Other midi event
                    if (MPTK_LogLoadEvents)
                        Debug.LogWarning($"This MIDI event is not processed by Maestro: {midiEvent}");
                    break;
            }
        ...
        public static void Sort<T>(IList<T> list, int lowIndex, int highIndex, IComparer<T> comparer)
        {
            ...
            while (lowIndex <= endLow && startHigh <= highIndex)
                ...
                else
                {
                    ...
                    for (var k = startHigh - 1; k >= lowIndex; k--) list[k + 1] = list[k];
        ...
        {
            var beatChange = false;
            try
            {
                // Calculate tick player. All MIDI with tick bellow will be read
                cur_msec = msec;
                MPTK_TickPlayer = TickFromTempoChange + (long)((cur_msec - start_msec) / MPTK_Pulse + 0.5d);
                    ...
                    MPTK_TickPlayer > MPTK_CurrentTempoMap.ToTick)
                {
                    ...
                    MPTK_CurrentIndexTempoMap =
                        MPTKTempo.FindSegment(MPTK_TempoMap, MPTK_TickPlayer, MPTK_CurrentIndexTempoMap);
                    ...
                    MPTK_TickPlayer > MPTK_CurrentSignMap.ToTick)
                {
                    ...
                    MPTK_CurrentIndexSignMap =
                        MPTKSignature.FindSegment(MPTK_SignMap, MPTK_TickPlayer, MPTK_CurrentIndexSignMap);
        ...
        private void fluid_player_set_midi_tempo(int MPQN)
        {
            ...
            {
                Debug.LogWarning(
                    "MicrosecondsPerQuaterNote is not correct. Probably a weird tempo event has been found");
            }
            else
            {
                miditempo = MPQN;
                ...
                if (next_event >= 0 && MPTK_MidiEvents != null /*v2.9.0*/)
                {
                    ...
                    else
                    {
                        ...
                        while (true)
                        {
                            ...
                            var mptkEvent = MPTK_MidiEvents[next_event];
                            // 2.9.0 remove stupid shift half quanta

                            var quantizedTick = Quantization != 0
                                ? mptkEvent.Tick / Quantization * Quantization
                                : mptkEvent.Tick;
                            ...
                            if (TickSeek < 0
                                || (!MPTK_RawSeek &&
                                    mptkEvent.Command !=
                                    MPTKCommand.NoteOn /*&& mptkEvent.Command != MPTKCommand.NoteOff*/ &&
                                    !(mptkEvent.Command == MPTKCommand.MetaEvent &&
                                      mptkEvent.Meta != MPTKMeta.SetTempo)))
                            {
                        ...
                        if (next_event < 0) EndMidiEvent = true;
        ...
        {
            return
                $"[I:{e.Index:00000} A:{e.Tick:00000} R:{e.RealTime / 1000f:F2}] [Track:{e.Track} Channel:{e.Channel,00}] ";
        }
        ...
        public void DebugTrack()
        {
            ...
            foreach (var track in midifile.Events)
            {
                ...
                {
                    var info = string.Format("Track:{0} Channel:{1,2:00} Command:{2} AbsoluteTime:{3:0000000} ", itrck,
                        midievent.Channel, midievent.CommandCode, midievent.AbsoluteTime);
        ...
        /// <summary>
        ///     @brief
        ///     Get or change the current tempo played by the internal MIDI sequencer. Available from MPTK_MidiLoaded attribut from
        ///     MidiFilePLayer or MidiExternal.
        ///     @code
        ///     if (midiPlayer.MPTK_MidiLoaded != null)
        ...
        /// <summary>
        ///     @brief
        ///     Should enable or disable change tempo from MIDI Events ? If disabled, the first tempo defined (or default to 120
        ///     BPM) will be used for all the MIDI played.
        /// </summary>
        ...
        ///     Unlike MPTK_TickPlayer, which is continuously updated, this property is only updated when a MIDI event is read from
        ///     the sequencer.
        ///     @details
        ///     A MIDI tick is an easy way to identify a position in a song, independent of time, which can vary with tempo change
        ///     events.\n
        ///     The number of ticks per quarter note is constant throughout a MIDI, it's a property of the whole MIDI. see
        ...
        ///     @li It's not possible to set the current tick when MIDI is not playing. Instead, use the OnEventStartPlayMidi event
        ...
        ///     By default (false), all events except note-on are played from the beginning of the MIDI to the new position, to put
        ///     the synthesizer back in the right context (tempo, selected instruments, controller, ...).\n
        ///     If set to true, the current playback position is set, but the previous context is retained. This can have
        ///     undesirable (or funny!) effects on some MIDI, but it also makes it possible to change position much more quickly.\n
        ///     It's a choice to be made according to your needs.\n
        ///     @note
        ///     - Value returns to false (default) each time a MIDI is loaded. You can load the MIDI with #MPTK_Load(), change this
        ///     parameter and play the MIDI with #MPTK_Play().        ///
        /// </summary>
        ...
        ///     Unlike MPTK_TickCurrent which is only updated when a MIDI event is read, this property is updated continuously
        ///     while the MIDI thread is playing.
        ///     @details
        ...
        ///     See also #MPTK_DurationMS which gives the full duration of all MIDI events including track end, start and end
        ///     control, ....\n
        ...
        ///     In MIDI the denominator value is stored in a special format. i.e. the real denominator = 2 ^
        ...
        /// <summary>
        ///     @brief
        ///     Updated from TimeSignature event when playing MIDI: The denominator specifies the number of quarter notes in a
        ///     beat.
        ...
        /// <summary>
        ///     @brief
        ///     Updated from TimeSignature event when playing MIDI: describe of what note value a beat is (ie, how many quarter
        ///     notes there are in a beat).
        ///     @note
        ///     This value is updated at each time signature meta event played.\n
        ...
        /// <summary>
        ///     @brief
        ///     Updated from TimeSignature event when playing MIDI: The standard MIDI clock ticks every 24 times every quarter note
        ///     (crotchet)\n
        ///     So a #MPTK_TicksInMetronomeClick value of 24 would mean that the metronome clicks once every quarter note.\n
        ///     A #MPTK_TicksInMetronomeClick value of 6 would mean that the metronome clicks once every 1/8th of a note
        ///     (quaver).\n
        ///     https://paxstellar.fr/2020/09/11/midi-timing/
        /// </summary>
        ...
        /// <summary>
        ///     @brief
        ///     Updated from TimeSignature event when playing MIDI: This value specifies the number of 1/32nds of a note happen
        ///     every MIDI quarter note.\n
        ...
        /// <summary>
        ///     @brief
        ///     Updated from KeySignature event when playing MIDI: values between -7 and 7 and specifies the key signature in terms
        ///     of number of flats (if negative) or sharps (if positive)
        ///     https://www.recordingblogs.com/wiki/midi-key-signature-meta-message
        /// </summary>
