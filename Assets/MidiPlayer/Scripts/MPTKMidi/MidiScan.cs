...
namespace MidiPlayerTK
{
    ...
    public class MidiScan
    {
        ...
        /// <param name="pathfilename"></param>
        /// <param name="Info"></param>
        public static List<string> GeneralInfoMPTKEvent(string pathfilename, bool withNoteOn, bool withNoteOff,
            bool withControlChange, bool withPatchChange, bool withAfterTouch, bool withMeta, bool withOthers)
        {
            var Info = new List<string>();
            try
            {
                var midifile = new MidiLoad();
                ...
                if (midifile != null)
                {
                    ...
                    Info.Add(string.Format("Duration: {0} ({1} seconds) {2} Ticks", midifile.MPTK_Duration,
                        midifile.MPTK_Duration.TotalSeconds, midifile.MPTK_TickLast));
                    Info.Add($"Track Count: {midifile.MPTK_TrackCount}");
                    Info.Add("Initial Value");
                    ...
                    Info.Add(
                        string.Format("   Ticks per Quarter Note: {0}", midifile.midifile.DeltaTicksPerQuarterNote));
                    Info.Add("");

                    if (withNoteOn || withNoteOff || withControlChange || withPatchChange || withAfterTouch ||
                        withMeta || withOthers)
                    {
                        ...
                        foreach (var mptkEvent in midifile.MPTK_MidiEvents)
                            switch (mptkEvent.Command)
                            {
                                case MPTKCommand.NoteOn:
                                    if (withNoteOn)
                                        Info.Add(BuildInfoTrack(mptkEvent) +
                                                 string.Format("NoteOn {0,3} ({1,3}) Len:{2,3} Vel:{3,3}",
                                                     HelperNoteLabel.LabelFromMidi(mptkEvent.Value), mptkEvent.Value,
                                                     mptkEvent.Length, mptkEvent.Velocity));
                                    break;
                                case MPTKCommand.NoteOff:
                                    if (withNoteOff)
                                        Info.Add(BuildInfoTrack(mptkEvent) +
                                                 string.Format("NoteOff {0,3} ({1,3}) Vel:{2,3}",
                                                     HelperNoteLabel.LabelFromMidi(mptkEvent.Value), mptkEvent.Value,
                                                     mptkEvent.Velocity));
                                    break;
                                    ...
                                    if (withMeta)
                                        switch (mptkEvent.Meta)
                                        {
                                            case MPTKMeta.SetTempo:
                                                Info.Add(BuildInfoTrack(mptkEvent) +
                                                         $"SetTempo MicrosecondsPerQuarterNote:{mptkEvent.Value} BPM:{60000000 / mptkEvent.Value:F2}");
                                                break;
                                            ...
                                            default:
                                                Info.Add(BuildInfoTrack(mptkEvent) + mptkEvent.Meta + mptkEvent.Info);
                                                break;
                                        }
        ...
        public static List<List<MidiEvent>> GetEventFromRawMIDI(string pathfilename, bool withNoteOn, bool withNoteOff,
            bool withPitchWheelChange, bool withControlChange, bool withPatchChange, bool withAfterTouch, bool withMeta,
            bool withOthers)
        {
            var midiEvents = new List<List<MidiEvent>>();
            CountMidiEvents = 0;
            try
            {
                var mididata = Resources.Load<TextAsset>(Path.Combine(MidiPlayerGlobal.MidiFilesDB, pathfilename));
                if (mididata != null && mididata.bytes != null && mididata.bytes.Length > 0)
                {
                    var midifile = new MidiFile(mididata.bytes, false);
                    var indexTrack = 0;
                    foreach (var track in midifile.Events)
                    {
                        midiEvents.Add(new List<MidiEvent>());
                        //midiEvents[indexTrack].AddRange(track);
                        foreach (var nAudioMidievent in track)
                            if ((withNoteOn && nAudioMidievent.CommandCode == MidiCommandCode.NoteOn) ||
                                ...
                                (withPitchWheelChange &&
                                 nAudioMidievent.CommandCode == MidiCommandCode.PitchWheelChange) ||
                                ...
                                (withOthers && (uint)nAudioMidievent.CommandCode >= 0xF0 &&
                                 (uint)nAudioMidievent.CommandCode < 0xFF)) // Sysex, Eox, TimingClock, ... and not Meta
                            {
                ...
                {
                    Debug.LogWarningFormat("Midi {0} not loaded from folder {1}", pathfilename,
                        MidiPlayerGlobal.MidiFilesDB);
                }
            }
            catch (Exception ex)
            {
        ...
        public static string ConvertnAudioEventToString(MidiEvent nAudioMidievent, int track)
        {
            ...
            switch (nAudioMidievent.CommandCode)
            {
                case MidiCommandCode.NoteOn:
                    var noteon = (NoteOnEvent)nAudioMidievent;
                    if (noteon.OffEvent != null)
                        infoevent = BuildInfoTrack(nAudioMidievent, track) + string.Format(
                            "NoteOn  {0,3} ({1,03}) Len:{2,3} Vel:{3,3}", noteon.NoteName, noteon.NoteNumber,
                            noteon.NoteLength, noteon.Velocity);
                    else
                        // It's a noteoff
                        infoevent = BuildInfoTrack(nAudioMidievent, track) + string.Format(
                            "NoteOff {0,3} ({1,03}) Len:{2,3} Vel:{3,3} (from noteon)", noteon.NoteName,
                            noteon.NoteNumber, noteon.NoteLength, noteon.Velocity);
                    break;
                case MidiCommandCode.NoteOff:
                    var noteoff = (NoteEvent)nAudioMidievent;
                    infoevent = BuildInfoTrack(nAudioMidievent, track) + string.Format(
                        "NoteOff {0,3} ({1,03}) Vel:{2,3}", noteoff.NoteName, noteoff.NoteNumber, noteoff.Velocity);
                    break;
                case MidiCommandCode.PitchWheelChange:
                    var aftertouch = (PitchWheelChangeEvent)nAudioMidievent;
                    infoevent = BuildInfoTrack(nAudioMidievent, track) +
                                string.Format("PitchWheelChange {0,03}", aftertouch.Pitch);
                    break;
                case MidiCommandCode.KeyAfterTouch:
                    var keyaftertouch = (NoteEvent)nAudioMidievent;
                    infoevent = BuildInfoTrack(nAudioMidievent, track) +
                                $"KeyAfterTouch {keyaftertouch.NoteName} ({keyaftertouch.NoteNumber}) Pressure:{keyaftertouch.Velocity}";
                    break;
                case MidiCommandCode.ChannelAfterTouch:
                    var channelaftertouch = (ChannelAfterTouchEvent)nAudioMidievent;
                    infoevent = BuildInfoTrack(nAudioMidievent, track) +
                                $"ChannelAfterTouch Pressure:{channelaftertouch.AfterTouchPressure}";
                    break;
                case MidiCommandCode.ControlChange:
                    var controlchange = (ControlChangeEvent)nAudioMidievent;
                    infoevent = BuildInfoTrack(nAudioMidievent, track) +
                                $"ControlChange 0x{(MPTKController)controlchange.Controller:X}/{(MPTKController)controlchange.Controller} {controlchange.ControllerValue}";
                    break;
                case MidiCommandCode.PatchChange:
                    var change = (PatchChangeEvent)nAudioMidievent;
                    infoevent = BuildInfoTrack(nAudioMidievent, track) +
                                $"PresetChange {change.Patch} {PatchChangeEvent.GetPatchName(change.Patch)}";
                    break;
                case MidiCommandCode.MetaEvent:
                    var meta = (MetaEvent)nAudioMidievent;
                    switch (meta.MetaEventType)
                    {
                        case MetaEventType.SetTempo:
                            var tempo = (TempoEvent)meta;
                            infoevent = BuildInfoTrack(nAudioMidievent, track) +
                                        string.Format("SetTempo Tempo:{0} MicrosecondsPerQuarterNote:{1}",
                                            Math.Round(tempo.Tempo, 0), tempo.MicrosecondsPerQuarterNote);
                            //tempo.Tempo
                            break;
                        ...
                        default:
                            var text = meta is TextEvent ? " '" + ((TextEvent)meta).Text + "'" : "";
                            infoevent = BuildInfoTrack(nAudioMidievent, track) + meta.MetaEventType + text;
                            break;
                    }
                ...
                default:
                    // Other midi event
                    infoevent = BuildInfoTrack(nAudioMidievent, track) + string.Format("{0} ({1})",
                        nAudioMidievent.CommandCode, (int)nAudioMidievent.CommandCode);
                    break;
            }
