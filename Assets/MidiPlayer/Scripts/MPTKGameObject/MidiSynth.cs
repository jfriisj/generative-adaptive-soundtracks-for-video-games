...
namespace MidiPlayerTK
{
    ...
    ///         and use attributs and methods from an instance of them in your script.
    ///     </b>
    ///     Example:
    ...
    public partial class MidiSynth : MonoBehaviour
    {
        ...
        public const float FLUID_REVERB_DEFAULT_ROOMSIZE = 0.5f;

        /**
         * < Default reverb room size
         */
        public const float FLUID_REVERB_DEFAULT_DAMP = 0.3f;

        /**
         * < Default reverb damping
         */
        public const float FLUID_REVERB_DEFAULT_WIDTH = 0.8f;

        /**
         * < Default reverb width
         */
        public const float FLUID_REVERB_DEFAULT_LEVEL = 0.7f;

        /**
         * < Default reverb level
         */
        public const int FLUID_CHORUS_DEFAULT_N = 3;

        /**
         * < Default chorus voice count
         */
        public const float FLUID_CHORUS_DEFAULT_LEVEL = 0.6f;

        /**
         * < Default chorus level
         */
        public const float FLUID_CHORUS_DEFAULT_SPEED = 0.2f;

        /**
         * < Default chorus speed
         */
        public const float FLUID_CHORUS_DEFAULT_DEPTH = 4.25f;

        /**
         * < Default chorus depth
         */
        public const float FLUID_CHORUS_DEFAULT_WIDTH = 10f;

        /**< Default chorus width gradually stereo effect*/
        [NonSerialized] public AudioSource CoreAudioSource;
        ...
        [HideInInspector] public int priorityThreadMidi;
        ...
        public int ThreadMidiPriorityReal
        {
            get
            {
                ...
                if (midiThread != null)
                    return midiThread.Priority - ThreadPriority.Normal;
                return -1;
            }
        }
        ...
        [NonSerialized] public int IdSynth;
        [NonSerialized] public int IdSession;


        [NonSerialized] public int FLUID_BUFSIZE = 64;
#if MPTK_PRO && UNITY_ANDROID && UNITY_OBOE
...
#else
        [NonSerialized] public int FLUID_MAX_BUFSIZE = 64;
        ...
        [NonSerialized] public int DspBufferSize;
        ...
        /// <summary>
        ///     @brief
        ///     Preset are often composed with 2 or more samples, classically for left and right channel. Check this to play only
        ...
        */
        ...
        /// <summary>
        ///     @brief
        ///     When your application is running, SoundFonts can be dynamically loaded either from a local file system or directly
        ///     from the web.\n
        ///     This means you don't need to include a SoundFont in your build, making it ideal for scenarios like in-app purchases
        ///     or downloadable content. \n
        ///     For compatibility, the legacy mode still allows loading SoundFonts from the internal MPTK database. \n
        ///     Additionally, Maestro MPTK supports assigning different SoundFonts to different MIDI players (MidiFilePlayer,
        ///     MidiStreamPlayer, ...), enabling flexible and customized audio rendering across multiple instruments or scenes.\n
        ///     @version 2.15
        ///     An instance of this class is automatically created when the MPTK prefab is loaded in the scene. See MPTKSoundFont
        ///     class for more details.\n
        ///     @note
        ///     @code
        ///     @endcode
        /// </summary>
        [NonSerialized] public MPTKSoundFont MPTK_SoundFont;


        /// <summary>
        ///     @brief
        ///     Accept preset changes for Drum Channel 9 (drum).\n
        ///     If set, this may sometimes produce unexpected results with MIDI files that are not compliant with the MIDI
        ///     standard.\n
        ...
        ///     - Set to true in MidiStreamPlayer (all MIDI events are generated by your script, so you are aware of what is
        ///     happening).
        /// </summary>
        [HideInInspector] public bool MPTK_EnablePresetDrum;
        ...
        /// </summary>
        [HideInInspector] public bool MPTK_ReleaseSameNote = true;
        ...
        /// </summary>
        [HideInInspector] public bool MPTK_KillByExclusiveClass = true;

        /// <summary>
        ///     @brief
        ///     When the value is true, NoteOff and Duration for non-looped samples are ignored and the samples play through to the
        ///     end.
        /// </summary>
        public bool MPTK_KeepPlayingNonLooped
        {
            get => keepPlayingNonLooped;
            set => keepPlayingNonLooped = value;
        }

        /// <summary>
        ///     @brief
        ///     When a note is stopped with a noteoff or when the duration is over, note continue to play for a short time
        ///     depending the instrument.\n
        ///     This parameter is a multiplier to increase or decrease the default release time defined in the SoundFont for each
        ///     instrument.\n
        ///     Recommended values between 0.1 and 10. Default is 1 (no modification of the release time).\n
        ///     Performance issue: the longer it lasts the more CPU is used after the noteon.  With a long release time, a lot of
        ///     samples will be played simultaneously.
        /// </summary>
        [Tooltip("Modify the default value of the release time")] [Range(0.1f, 10f)]
        public float MPTK_ReleaseTimeMod = 1f;

        /// <summary>
        ///     @brief
        ///     When amplitude of a sample is below this value the playing of sample is stopped.\n
        ///     Can be increase for better performance (when a lot of samples are played concurrently) but with degraded quality
        ///     because sample could be stopped too early.
        ///     Remember: Amplitude can varying between 0 and 1.\n
        ///     @version 2.9.1 // was 0.03
        /// </summary>
        [Range(0.000001f, 0.5f)] [Tooltip("Sample is stopped when amplitude is below this value")]
        public float MPTK_CutOffVolume = 0.0001f; // replace amplitude_that_reaches_noise_floor 

        /// <summary>
        ///     @brief
        ///     Experimental feature: modify the Fluidsynth constant FLUID_PEAK_ATTENUATION to change how velocity affects the
        ///     voice attenuation.
        ...
        {
            get => fluid_conv.FLUID_PEAK_ATTENUATION;
            set
            {
                if (value >= 100f && value <= 2440f)
                {
                    fluid_conv.FLUID_PEAK_ATTENUATION = value;
                    if (MPTK_SynthState == fluid_synth_status.FLUID_SYNTH_PLAYING)
                        MPTK_InitModulators();
                }
                else
                {
        ...
        /// <summary>
        ///     @brief
        ///     A lean startup of the volume of the synth is useful to avoid weird sound at the beginning of the application (in
        ///     some cases).\n
        ...
        [Range(0.001f, 1f)]
        [Tooltip(
            "Lean startup of the volume of the synth is useful to avoid weird sound at the beginning of the application. Set to 1 for an immediate full volume at startup.")]
        ...
        /// </summary>
        [Tooltip("Auto Clean Voice Greater Than")] [Range(0, 1000)]
        public int MPTK_AutoCleanVoiceLimit;

        [Tooltip("Auto Clean Voice Older Than (millisecond)")] [Range(1000, 100000)]
        ...
        ///     Furthermore, to get more liberty from SoundFont, Maestro Pro can increase or decrease the impact of effects (from
        ///     the inspector or by script).
        ///     To summarize:
        ...
        ///     - These adjustments will be applied across the entire prefab, but the effect will depend on the initial settings
        ///     defined in the SoundFont preset.
        ...
        ///     - To enable them, you�ll need to adjust the settings from the prefab inspector (Synth Parameters / SoundFont
        ///     Effect) or by script!
        ///     - For enhanced sound quality, it�s beneficial to add a low-filter effect, it is enabled by default..
        ///     @code
        ///     // Find a MPTK Prefab, will works also for MidiStreamPlayer, MidiExternalPlayer ... all classes which inherit from
        ///     MidiSynth.
        ///     MidiFilePlayer fp = FindFirstObjectByType
        ///     <MidiFilePlayer>
        ///         ();
        ...
        ///         @endcode
        /// </summary>
        [SerializeField] public MPTKEffectSoundFont MPTK_EffectSoundFont;
        ...
        ///     @note: A higher priority thread might consume all the CPU time. It's recommended to set MPTK_ThreadMidiWait higher
        ///     than 5 ms.
        /// </summary>
        public int MPTK_ThreadMidiPriority
        {
            get => priorityThreadMidi;
            set
            {
                priorityThreadMidi = value;
                // Try setting value
                if (midiThread != null)
                    midiThread.Priority = priorityThreadMidi + ThreadPriority.Normal;
        ...
        [Header("DSP Statistics")] public float StatDspLoadPCT;
        ...
        public double StatDeltaTimeMidi;
...
#if DEBUG_PERF_AUDIO
        private System.Diagnostics.Stopwatch watchPerfAudio =
 new System.Diagnostics.Stopwatch(); // High resolution time
#endif
...
#endif
        public SynthInfo synthInfo = new();
        ...
        private readonly Stopwatch watchSynth = new();

        protected Stopwatch watchMidi = new();

        protected Stopwatch pauseMidi = new();
        ...
        private System.Diagnostics.Stopwatch watchPerfNoteOn =
 new System.Diagnostics.Stopwatch(); // High resolution time
#endif
        ...
        [HideInInspector] public bool mPTK_CorePlayer;

        /// <summary>
        ///     @brief
        ///     If true then rate synth and buffer size will be automatically defined by Unity in accordance of the capacity of the
        ///     hardware. -  V2.89.0 - \n
        ///     Look at Unity menu "Edit / Project Settings..." and select between best latency and best performance.\n
        ///     If false, then rate and buffer size can be defined manually ... but with the risk of bad audio quality. It's more
        ///     an experimental capacities!
        /// </summary>
        [HideInInspector] public bool MPTK_AudioSettingFromUnity;
        ...
        public int MPTK_SynthRate
        {
            ...
            set
            {
                var valueClamped = Mathf.Clamp(value, 12000, 96000);
                if (OutputRate != value)
                {
        ...
        /// </summary>
        [HideInInspector] public bool MPTK_EnableFreeSynthRate;

        /// <summary>
        ///     @brief
        ///     Set or Get sample rate output of the synth. -1:default, 0:24000, 1:36000, 2:48000, 3:60000, 4:72000, 5:84000,
        ///     6:96000.\n
        ...
        [SerializeField] [HideInInspector] private int indexSynthRate = -1;

        private readonly int[] tabDspBufferSize = { 64, 128, 256, 512, 1024, 2048 };

        /// <summary>
        ///     @brief
        ///     Set or Get synth buffer size  -1:default,  0:64, 1;128, 2:256, 3:512, 4:1024, 5:2048.\n
        ///     The change is global for all prefab. It's better to stop playing for all prefab before changing on fly to avoid bad
        ///     noise or crash.
        /// </summary>
        ...
        [SerializeField] [HideInInspector] private int indexBuffSize = -1;
        ...
        /// </summary>
        [HideInInspector] public bool MPTK_DirectSendToPlayer;
        ...
        /// </summary>
        [HideInInspector] public bool MPTK_EnableChangeTempo;

        /// <summary>
        ///     @brief
        ///     If MPTK_Spatialize is enabled, the volume of the audio source depends on the distance between the audio source and
        ///     the listener.
        ///     Beyong this distance, the volume is set to 0 and the midi player is paused. No effect if MPTK_Spatialize is
        ///     disabled.
        /// </summary>
        ...
        protected void SetSpatialization()
        {
            ...
            else
            {
                ...
                if (FreeVoices != null)
                    for (var i = 0; i < FreeVoices.Count; i++)
                    {
                        var voice = FreeVoices[i];
                        if (voice.VoiceAudio != null)
                            voice.VoiceAudio.Audiosource.maxDistance = maxDistance;
                    }
            }
        ...
        ///     See here how to setup spatialization with Unity
        ...
        /// <summary>
        ///     @brief
        ///     Contains each Midi Synth for each channel or track when the prefab MidiSpatializer is used and
        ...
        /// </summary>
        public int MPTK_SpatialSynthIndex { get; } = -1;
        ...
        /// </summary>
        [HideInInspector] public bool MPTK_EnablePanChange;

        /// <summary>
        ///     @brief
        ///     Set the global volume between 0 and 1 for the current MPTK Synth.
        ///     @note:
        ///     - Unlock the maximum value to 10 (could be useful with Oboe) when the Unity script symbol MPTK_UNLOCK_VOLUME is
        ...
        public float MPTK_Volume
        {
            ...
            set
            {
                if (value >= 0f && value <= Constant.MAX_VOLUME)
                    ...
                    Debug.LogWarning(
                        $"Set Volume value {value} not valid, must be between 0 and {Constant.MAX_VOLUME}");
            }
        }

        [SerializeField] [HideInInspector] private float volumeGlobal = 0.5f;
        ...
        public int MPTK_Transpose
        {
            ...
            set
            {
                if (value >= -24 && value <= 24) transpose = value;
                else Debug.LogWarning("Set Transpose value not valid : " + value);
            }
        }
        ...
        {
            get => transExcludedChannel;
            set
            {
                if (value < 16 && value >= -1) transExcludedChannel = value;
                else Debug.LogWarning("Set Transpose Excluded Channel value not valid : " + value);
            }
        }
        ...
        [HideInInspector] public bool dsp_64;

        /// <summary>
        ///     @brief
        ///     Log MIDI events (v2.9.0 moved from MidiFilePlayer)
        /// </summary>
        [HideInInspector] public bool MPTK_LogEvents;

        /// <summary>
        ///     @brief
        ///     Log for each sample to be played
        /// </summary>
        [HideInInspector] public bool MPTK_LogWave;
        ...
        [SerializeField] [HideInInspector] protected bool playPause;
        ...
        /// </summary>
        [HideInInspector] public float distanceToListener;

        [SerializeField] [HideInInspector] public int transpose, transExcludedChannel = 9;
        ...
        [HideInInspector] public fluid_interp InterpolationMethod = fluid_interp.Linear;

        [HideInInspector] public float gain = 1f;
        ...
        public fluid_synth_status MPTK_SynthState { get; set; } = fluid_synth_status.FLUID_SYNTH_CLEAN;
        ...
        ///     Playing MIDI files with WeakDevice activated could cause some bad interpretation of MIDI Event, consequently bad
        ///     sound.
        /// </summary>
        [Tooltip("Apply only with AudioSource mode (no Core Audio)")]
        ...
        /// </summary>
        [Range(0, 5000000)] [Tooltip("Apply only with AudioSource mode (no Core Audio)")]
        public uint MPTK_ReleaseTimeMin = 500000;

        [Range(0, 100)] [Tooltip("Smooth Volume Change")]
        public int DampVolume; // default value=5
        ...
        private static readonly HiMod default_vel2att_mod = new(); /* SF2.01 section 8.4.1  */
        public static HiMod default_vel2filter_mod = new(); /* SF2.01 section 8.4.2  */
        private static readonly HiMod default_at2viblfo_mod = new(); /* SF2.01 section 8.4.3  */
        private static readonly HiMod default_mod2viblfo_mod = new(); /* SF2.01 section 8.4.4  */
        private static readonly HiMod default_att_mod = new(); /* SF2.01 section 8.4.5  */
        private static readonly HiMod default_pan_mod = new(); /* SF2.01 section 8.4.6  */
        private static readonly HiMod default_expr_mod = new(); /* SF2.01 section 8.4.7  */
        private static readonly HiMod default_reverb_mod = new(); /* SF2.01 section 8.4.8  */
        private static readonly HiMod default_chorus_mod = new(); /* SF2.01 section 8.4.9  */
        private static readonly HiMod default_pitch_bend_mod = new(); /* SF2.01 section 8.4.10 */

        // @cond NODOC

        [HideInInspector] public bool showMidiInfo;
        [HideInInspector] public bool showSynthParameter;
        [HideInInspector] public bool showSpatialization;
        [HideInInspector] public bool showUnitySynthParameter;
        [HideInInspector] public bool showUnityPerformanceParameter;
        [HideInInspector] public bool showExperimentalFeature;
        [HideInInspector] public bool showMidiPerformanceParameter;
        [HideInInspector] public bool showSoundFontEffect;
        [HideInInspector] public bool showVerbose;
        [HideInInspector] public bool showUnitySynthEffect;
        [HideInInspector] public bool showMidiParameter;
        [HideInInspector] public bool showMidiPlaying;
        [HideInInspector] public bool showSynthEvents;
        [HideInInspector] public bool showEvents;
        [HideInInspector] public bool showDefault;

        [HideInInspector] public bool spatialize;
        ...
        public void Awake()
        {
            ...
            {
                if (VerboseSynth)
                    Debug.LogWarningFormat("AudiosourceTemplate not defined in the {0} inspector, search one", name);
            ...
            if (MPTK_CorePlayer)
            {
                if (CoreAudioSource == null)
                {
                    ...
                    if (CoreAudioSource == null)
                        Debug.LogErrorFormat("No AudioSource defined in the MPTK prefab '{0}'", name);
                }
            }
            else
            {
                ...
                CoreAudioSource = GetComponent<AudioSource>();
                if (CoreAudioSource != null) CoreAudioSource.enabled = false;
            }
        }
        ...
        public void Start()
        {
            ...
            try
            {
                ...
                if (Application.isPlaying)
                    Routine.RunCoroutine(ThreadLeanStartAudio(CoreAudioSource).CancelWith(gameObject),
                        Segment.FixedUpdate);
                else
                ...
                try
                {
                    if (OnEventSynthStarted != null)
                        OnEventSynthStarted.Invoke(name);
                }
                catch (Exception ex)
                {
        ...
        {
            var increment = Mathf.Clamp(MPTK_LeanSynthStarting, 0.001f, 1f);
            if (increment >= 1f)
            {
        ...
        private void OnAudioConfigurationChanged(bool deviceWasChanged)
        {
            ...
            // see InitOboe method
#else
            var GetConfiguration = AudioSettings.GetConfiguration();
            ...
            {
                Debug.Log("OnAudioConfigurationChanged - " +
                          (deviceWasChanged ? "Device was changed" : "Reset was called"));
        ...
        private void GetInfoAudio()
        {
            ...
            {
                Debug.LogError(
                    $"DspBufferSize length is {ac.dspBufferSize}. It must be a multiple of {FLUID_BUFSIZE}.");
                Debug.LogError(
                    "Try to change the 'DSP Buffer Size' in the Unity Editor menu 'Edit / Project Settings / Audio'");
            }
        ...
        ///     But in some cases, you could have to call MPTK_InitSynth to restaure initial condition of the synth (mainly useful
        ///     with MidiStreamPlayer)\n
        /// </summary>
        /// <param name="channelCount">Number of channel to create. Default is 16. Any other values are experimental!</param>
        /// <param name="preserveChannelInfo">
        ///     if true, the channel information will not be reset, Default is false: reinit also
        ///     channel information
        /// </param>
            ...
            bool preserveActivVoice = true)
        {
            if (VerboseSynth)
                Debug.Log(
                    $"MPTK_InitSynth. IdSynth:{IdSynth}, Name:{name}, Channels:{channelCount}, preserveChannelInfo:{preserveChannelInfo}, state was {MPTK_SynthState}");
            ...
            if (Channels == null || Channels.EnableResetChannel || Channels.Length != channelCount ||
                !preserveChannelInfo) Channels = new MPTKChannels(this, channelCount);
            ...
            MPTK_ResetStat();
            MPTK_SynthState = fluid_synth_status.FLUID_SYNTH_PLAYING;
        ...
        public void MPTK_InitModulators()
        {
            if (VerboseSynth)
                Debug.Log(
                    $"MPTK_InitModulators. IdSynth:{IdSynth}, Channels:{Channels.Length}, state was {MPTK_SynthState} {fluid_conv.FLUID_PEAK_ATTENUATION}");
            ...
            fluid_mod_set_dest(default_vel2att_mod,
                (int)fluid_gen_type.GEN_ATTENUATION); /* Target: Initial attenuation */
            ...
            fluid_mod_set_dest(default_vel2filter_mod,
                (int)fluid_gen_type.GEN_FILTERFC); /* Target: Initial filter cutoff */
            ...
            fluid_mod_set_dest(default_at2viblfo_mod,
                (int)fluid_gen_type.GEN_VIBLFOTOPITCH); /* Target: Vib. LFO => pitch */
            ...
            fluid_mod_set_dest(default_pitch_bend_mod, /* (int)fluid_gen_type.GEN_PITCH replace with FS 2.3*/
                (int)fluid_gen_type.GEN_FINETUNE); /* Destination: Initial pitch */
        ...
        public void MPTK_StartSequencerMidi()
        {
            if (MPTK_CorePlayer)
            {
                if (!AudioThreadMidi)
                {
                    if (VerboseSynth)
                        Debug.Log(
                            $"MPTK_StartSequencerMidi {name} {(midiThread == null ? "Null" : "Alive:" + midiThread.IsAlive)}");
                    if (midiThread != null)
                    {
                    ...
                    {
                        if (midiThread == null) midiThread = new Thread(ThreadMidiPlayer);

                        if (midiThread != null && !midiThread.IsAlive)
                        {
                            midiThread.Name = "MidiThread_" + IdSynth;
                            midiThread.Priority = MPTK_ThreadMidiPriority + ThreadPriority.Normal;
                            midiThread.Start();
                            if (VerboseSynth)
                                Debug.Log(
                                    $"MPTK_StartSequencerMidi Started {name} {IdSynth} ManagedThreadId:{midiThread.ManagedThreadId}");
                        }
                        else if (VerboseSynth)
                        {
                            Debug.Log(
                                $"MPTK_StartSequencerMidi: Alive  {name} {IdSynth} ManagedThreadId:{midiThread.ManagedThreadId} Priority:{midiThread.Priority}");
                        }
                    }
                }
                else if (VerboseSynth)
                {
                    Debug.Log("MPTK_StartSequencerMidi - MIDI MPTK Thread disabled, MIDI reader from Audio Thread");
                }
            }
        }
        ...
        {
            if (VerboseSynth) Debug.Log($"MPTK_StopSynth. IdSynth:{IdSynth}, state was {MPTK_SynthState}");
            MPTK_SynthState = fluid_synth_status.FLUID_SYNTH_STOPPED;
        }
                        ...
                        if (MPTK_ApplyRealTimeModulator)
                            ...
                            Channels[midiEvent.Channel]
                                .Controller(midiEvent.Controller,
                                    midiEvent
                                        .Value); // replace of fluid_synth_cc(note.Channel, note.Controller, (int)note.Value);
                        break;

                    case MPTKCommand.PatchChange:
                        if (midiEvent.Channel != 9 || MPTK_EnablePresetDrum)
                        {
        ...
        public IEnumerator<float> ThreadClearAllSound(bool destroyAudioSource = false, int _idSession = -1)
        {
            ...
            if (MPTK_CorePlayer)
            {
                if (SpatialSynths != null && MPTK_SpatialSynthIndex == -1) // apply only for the MidiSynth reader
                {
                    //Debug.Log($"For Spatial ");
                    foreach (var mfp in SpatialSynths)
                        if (mfp.QueueSynthCommand != null)
                            mfp.QueueSynthCommand.Enqueue(new SynthCommand
                                { Command = SynthCommand.enCmd.NoteOffAll, IdSession = _idSession });
            ...
            else
            {
                if (ActiveVoices != null)
                    for (var i = 0; i < ActiveVoices.Count; i++)
                    {
                        ...
                        if (voice != null && (voice.status == fluid_voice_status.FLUID_VOICE_ON ||
            ...
            if (destroyAudioSource)
            {
                if (!MPTK_CorePlayer)
                    ...
                    yield return Routine.WaitUntilDone(
                        Routine.RunCoroutine(ThreadDestroyAllVoice(), Segment.RealtimeUpdate), false);
            }
        ...
        public IEnumerator<float> ThreadVoiceAudioSourceRemove(float wait, int _idSession = -1)
        {
            ...
            while (true)
            {
                var voiceAudioSources = FindObjectsByType<VoiceAudioSource>(FindObjectsSortMode.InstanceID);
        ...
        /// <returns></returns>
        public IEnumerator
            MPTK_WaitAllNotesOff(int _idSession = -1) // V2.84: new param idsession and CoRoutine compatible
        {
                ...
                while (count != 0 && (DateTime.Now - start).TotalMilliseconds < 3000d)
                {
                    ...
                    foreach (var voice in ActiveVoices)
                        if (voice != null &&
                            ...
                            (voice.status == fluid_voice_status.FLUID_VOICE_ON ||
                             voice.status == fluid_voice_status.FLUID_VOICE_SUSTAINED))
                            count++;
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (voiceAudioSources != null)
                        foreach (var voice in voiceAudioSources)
                            //Debug.Log($"    Destroy id:{voice.name} {(voice.Audiosource.clip != null ? voice.Audiosource.clip.name : "no clip")}");
                            if (voice.name !=
                                "VoiceAudioSourceTemplate") // Test in case template gas been enabled by error!
                                try
                                {
        ...
        public void MPTK_ResetStat()
        {
            ...
            if (Channels != null)
                foreach (var mptkChannel in Channels)
        ...
        {
            Debug.LogWarning(
                $"*** {old} is deprecated and will not works. Please investigate MPTK_Channels{newer} in place.");
        ...
        ///     Note: A single noteon event may create any number of voices, when the preset is layered. Typically 1 (mono) or 2
        ///     (stereo).
        /// </summary>
        ...
        public fluid_voice fluid_synth_alloc_voice_LOCAL(HiSample hiSample, MPTKEvent mptkEvent)
        {
            ...
            if (true)
                for (var indexVoice = 0; indexVoice < FreeVoices.Count;)
                {
                    var freeVoice = FreeVoices[indexVoice];
                    if (freeVoice.sample.Name ==
                        hiSample.Name /* v2.10.1 useless? freevoice is cleared when a new MIDI is played && _idSession == freeVoice.IdSession*/
                       )
                    {
                        voice = freeVoice;
                        voice.Reused = true;
            ...
            if (voice == null)
            {
                if (VerboseVoice)
                    Debug.Log(
                        $"Voice idSession:{mptkEvent.IdSession} idVoice:{fluid_voice.LastId} - Create - Sample:'{hiSample.Name}' Rate:{hiSample.SampleRate} hz");
                    ...
                    else
                    {
                        ...
                        //if (MidiPlayerGlobal.MPTK_LoadWaveAtStartup)
                        {
                            ...
                            if (voice.sample == null)
                                Debug.Log(
                                    $"<color=red>Sample {hiSample.Name} not loaded. Have you extracted samples from 'SoundFont Setup' menu?</color>");
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (clip == null)
                        {
                            var path = MidiPlayerGlobal.WavePath + "/" +
                                       Path.GetFileNameWithoutExtension(hiSample.Name);
                            ...
                            {
                                if (VerboseVoice)
                                    Debug.LogWarningFormat("fluid_synth_alloc_voice - Clip {0} not found",
                                        hiSample.Name);
                                return null;
                            }
                        ...
                        if (VerboseVoice)
                            Debug.Log(
                                $"Voice idSession:{mptkEvent.IdSession} idVoice:{fluid_voice.LastId} - Clip created - name:'{clip.name}' loadState:{clip.loadState}");
                        voice.sample = hiSample;
                    }
                    ...
                    if (clip == null)
                    {
                        Debug.LogWarningFormat(
                            "fluid_synth_alloc_voice - Clip {0} data not loaded - not compliant with WebGL",
                            hiSample.Name);
                    }
                    else
                    {
                        voice.VoiceAudio = Instantiate(AudiosourceTemplate);
                        ...
                        if (VerboseVoice)
                            Debug.Log(
                                $"Voice idSession:{mptkEvent.IdSession} idVoice:{fluid_voice.LastId} - AudioSource Instanciated - name:'{clip.name}' loadState:{clip.loadState}");
                    }
                }
            ...
            MPTK_StatVoiceRatioReused =
                MPTK_StatVoicePlayed > 0 ? MPTK_StatVoiceCountReused * 100 / MPTK_StatVoicePlayed : 0;
            return voice;
        }
        ...
        */
        ...
        public void fluid_synth_kill_by_exclusive_class(fluid_voice new_voice)
        {
            ...
            */
            ...
            //int i;
            var excl_class = (int)new_voice.gen[(int)fluid_gen_type.GEN_EXCLUSIVECLASS].Val;
            ...
            for (var i = 0; i < ActiveVoices.Count; i++)
            {
                ...
                if (!(voice.status == fluid_voice_status.FLUID_VOICE_ON) ||
                    voice.status == fluid_voice_status.FLUID_VOICE_SUSTAINED) continue;

                /* An exclusive class is valid for a whole channel (or preset). Is the voice on a different channel? Leave it alone. */
                if (voice.chan != new_voice.chan) continue;

                /* Existing voice has a different (or no) exclusive class? Leave it alone. */
                if ((int)voice.gen[(int)fluid_gen_type.GEN_EXCLUSIVECLASS].Val != excl_class) continue;
        ...
        private void fluid_synth_release_voice_on_same_note(int chan, int key)
        {
            for (var i = 0; i < ActiveVoices.Count; i++)
            {
                    ...
                    //&& (fluid_voice_get_id(voice) != synth->noteid))
                {
                    ...
                    //else
                    voice.fluid_rvoice_noteoff(true);
                    if (VerboseSpecialNoteOff)
                        Debug.Log(
                            $"Voice {voice.id} - Same note, send note-off {chan} key:{key} {voice.volenv_section} DurationTick:{voice.DurationTick} Name:{voice.sample.Name}");
                }
            }
        }
        ...
        public void fluid_synth_noteoff(int pchan, int pkey)
        {
            for (var i = 0; i < ActiveVoices.Count; i++)
            {
                ...
                if (voice.status == fluid_voice_status.FLUID_VOICE_ON &&
                    ...
                    (!keepPlayingNonLooped || voice.samplemode == fluid_loop.FLUID_LOOP_UNTIL_RELEASE ||
                     voice.samplemode == fluid_loop.FLUID_LOOP_DURING_RELEASE) &&
        ...
        /// <summary>
        ///     @brief
        ///     Tell all synthesis processes on this channel to update their synthesis parameters after an all control off message
        ///     (i.e. all controller have been reset to their default value).
        /// </summary>
        /// <param name="chan"></param>
        ...
        protected void PlayEvents(List<MPTKEvent> midievents, bool playNoteOff = true)
        {
            ...
            if (midievents != null)
                foreach (var note in midievents)
        ...
        protected void StopEvent(MPTKEvent midievent)
        {
            try
            {
                if (midievent != null && midievent.Voices != null)
                    for (var i = 0; i < midievent.Voices.Count; i++)
                    {
                        var voice = midievent.Voices[i];
            ...
            if (MPTK_SynthState == fluid_synth_status.FLUID_SYNTH_PLAYING)
            {
                ...
                if (MPTK_SynthState == fluid_synth_status.FLUID_SYNTH_PLAYING)
                {
                    ...
                    lock (this)
                    {
                        ...
                        watchPerfAudio.Stop();
                        StatProcessListMS =
 (float)watchPerfAudio.ElapsedTicks / ((float)System.Diagnostics.Stopwatch.Frequency / 1000f);
...
#endif
                            var vol = MPTK_Volume * volumeStartStop;
#if DEBUG_TRY_CATCH_AUDIODATA
                                try
                                {
#endif
                            for (var i = 0; i < FLUID_BUFSIZE; i++)
                            {
                                var j = (block + i) << 1;
                        ...
                        StatAudioFilterReadMS = watchSynth.ElapsedTicks / (Stopwatch.Frequency / 1000d);
                        ...
                        // 
                        // Build a load ratio
                        StatDspLoadPCT = (float)(DeltaTimeAudioCall > 0d
                            ? StatAudioFilterReadMS * 100d / DeltaTimeAudioCall
                            : 0d);
                        ...
                        if (StatAudioFilterReadMS > StatAudioFilterReadMAX) StatAudioFilterReadMAX =
 StatAudioFilterReadMS;
                        if (StatAudioFilterReadMS < StatAudioFilterReadMIN) StatAudioFilterReadMIN =
 StatAudioFilterReadMS;
 ...
 (float)watchPerfAudio.ElapsedTicks / ((float)System.Diagnostics.Stopwatch.Frequency / 1000f);
        ...
        private void ProcessQueueCommand()
        {
            try
            {
                ...
                {
                    var action = QueueSynthCommand.Dequeue();
                    if (action != null)
        ...
        {
            if (VerboseSynth)
                Debug.Log($"Start RoutineManageCacheVoice {IdSynth} {ActiveVoices?.Count} {FreeVoices?.Count}");
            while (MPTK_SynthState == fluid_synth_status.FLUID_SYNTH_PLAYING)
            {
                var ticks = DateTime.UtcNow.Ticks;
                ManageCacheVoice(ticks);
                yield return Routine.WaitForSeconds(0.2f);
            ...
            if (VerboseSynth)
                Debug.Log($"End RoutineManageCacheVoice {IdSynth} {ActiveVoices?.Count} {FreeVoices?.Count}");
        }
...
#else
            var firstToKill = false;
#endif
            var countActive = ActiveVoices.Count;
            for (var indexVoice = 0; indexVoice < ActiveVoices.Count;)
                try
                {
                    var voice = ActiveVoices[indexVoice];
#if DEBUG_STATUS_STAT
/*  FLUID_VOICE_CLEAN = 0
                        FLUID_VOICE_ON = 1
 ...
 /*  FLUID_VOICE_ENVDELAY = 0
                    ...
                    if (StatDspLoadPCT > MaxDspLoad)
                    {
                        if (VerboseOverload)
                            voice.DebugOverload(
                                $"OverLoad: DeltaTimeAudioCall:{DeltaTimeAudioCall:F2} StatAudioFilterReadMS:{StatAudioFilterReadMS:F2} ActiveVoice:{MPTK_StatVoiceCountActive}");
                        ...
                        if (voice.volenv_section == fluid_voice_envelope_index.FLUID_VOICE_ENVRELEASE)
                        {
                            ...
                            // reduce release time
                            float count = voice.volenv_data[(int)fluid_voice_envelope_index.FLUID_VOICE_ENVRELEASE]
                                .count;
                            ...
                            voice.volenv_data[(int)fluid_voice_envelope_index.FLUID_VOICE_ENVRELEASE].count =
                                (uint)count;
                        }

                        if (!firstToKill &&
                            DevicePerformance <=
                            25) // V2.82 Try to stop one older voice (the first in the list of active voice)
        ...
        {
            foreach (var v in ActiveVoices) Debug.LogFormat("", v.LastTimeWrite);
        }
        ...
        private void AutoCleanVoice(long ticks)
        {
            for (var indexVoice = 0; indexVoice < FreeVoices.Count;)
                try
                {
                    ...
                    {
                        var voice = FreeVoices[indexVoice];
                        ...
                        if ((ticks - voice.TimeAtStart) / fluid_voice.Nano100ToMilli > MPTK_AutoCleanVoiceTime ||
                            needClearingFreeVoices)
                        {
                            if (VerboseVoice)
                                Debug.Log(
                                    $"AutoCleanVoice:{FreeVoices.Count} id:{voice.id} start:{(ticks - voice.TimeAtStart) / fluid_voice.Nano100ToMilli}");
                            FreeVoices.RemoveAt(indexVoice);
        ...
        private void ThreadMidiPlayer()
        {
            if (VerboseSynth)
                Debug.Log(
                    $"START ThreadMidiPlayer IdSynth:{IdSynth} state:{MPTK_SynthState} ManagedThreadId:{Thread.CurrentThread.ManagedThreadId}");
            ...
            if (MPTK_SpatialSynthIndex < 0)
                while (MPTK_SynthState == fluid_synth_status.FLUID_SYNTH_PLAYING)
                {
                    ...
                    if (MPTK_ThreadMidiWait > 0)
                        Thread.Sleep(MPTK_ThreadMidiWait);
            ...
            if (VerboseSynth)
                Debug.Log(
                    $"STOP ThreadMidiPlayer IdSynth:{IdSynth} state:{MPTK_SynthState} ManagedThreadId:{Thread.CurrentThread.ManagedThreadId}");
            ...
            catch (Exception ex)
            {
                if (VerboseSynth)
                    Debug.LogWarning(
                        $"STOP ThreadMidiPlayer IdSynth:{IdSynth} state:{MPTK_SynthState} ManagedThreadId:{Thread.CurrentThread.ManagedThreadId} {ex}");
            }
            finally
            {
                midiThread = null;
        ...
        private void PlayMidi()
        {
            ...
            if (AudioThreadMidi)
                //Debug.Log($"{Thread.CurrentThread.ManagedThreadId} {Thread.CurrentThread.Name,15} {Thread.CurrentThread.IsThreadPoolThread} {Thread.CurrentThread.GetApartmentState()} {Thread.CurrentThread.Priority}");
                nowMs = watchMidi.ElapsedTicks / (Stopwatch.Frequency / 1000d);
            else
                //Debug.Log($"{Thread.CurrentThread.ManagedThreadId} {Thread.CurrentThread.Name,15} {Thread.CurrentThread.IsThreadPoolThread} {Thread.CurrentThread.GetApartmentState()} {Thread.CurrentThread.Priority}");
...
// Return false in case of error or stop playing detected
                            ...
                            var midievents = midiLoaded.fluid_player_callback((int)timeMidiFromStartPlay, IdSession);

#if DEBUG_PERF_MIDI
                            StatReadMidiMS =
 (float)watchPerfMidi.ElapsedTicks / ((float)System.Diagnostics.Stopwatch.Frequency / 1000f);
                            watchPerfMidi.Reset();
                            watchPerfMidi.Start();
#endif
 ...
 //    StatProcessMidiMAX = (StatProcessMidiMAX * 9f + StatProcessMidiMS) / 10f; // smooth regression 
        ...
        public void DebugPerf(string info, int mode = 1)
        {
            ...
            if (perfs != null)
            {
                ...
                float now =
 (float)watchPerfNoteOn.ElapsedTicks / ((float)System.Diagnostics.Stopwatch.Frequency / 1000f);
                perf_time_cumul = now;
                float delta = now - perf_time_last;
                perf_time_last = now;
                string perf =
 string.Format("{0,-30} \t\t delta:{1:F6} ms \t cumul:{2:F6} ms ", info, delta, perf_time_cumul);
                perfs.Add(perf);
            }
