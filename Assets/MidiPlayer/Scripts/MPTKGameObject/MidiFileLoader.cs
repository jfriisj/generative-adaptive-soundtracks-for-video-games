...
{
    /// <summary>
    ///     @Warning - will be soon deprecated, please rather use MidiFilePlayer prefab which have the same features +
    ///     eventually MIDI sequencer and MIDI synth!
    ...
    {
        [SerializeField] [HideInInspector] private string midiNameToPlay;
        ...
        ///     #MPTK_TickLast / #MPTK_DeltaTicksPerQuarterNote equal the duration time of a quarter-note regardless the defined
        ///     tempo.
        /// </summary>
        ...
        /// <summary>
        ///     @brief
        ///     From KeySignature event: Values between -7 and 7 and specifies the key signature in terms of number of flats (if
        ///     negative) or sharps (if positive)
        ...
        public float MPTK_DurationMS
        {
            get
            {
                try
                {
                    if (MPTK_MidiLoaded != null) return MPTK_MidiLoaded.MPTK_DurationMS;
                }
                catch (Exception ex)
                {
                    MidiPlayerGlobal.ErrorDetail(ex);
        ...
        /// </summary>
        public List<MPTKEvent> MPTK_MidiEvents => MPTK_MidiLoaded != null ? MPTK_MidiLoaded.MPTK_MidiEvents : null;

        /// <summary>
        ///     @brief
        ///     Get detailed information about the midi loaded.
        /// </summary>
        public MidiLoad MPTK_MidiLoaded { get; private set; }
        ...
        public bool MPTK_Load(byte[] midiBytesToLoad = null)
        {
            ...
            try
            {
                ...
                //if (MidiPlayerGlobal.ImSFCurrent != null && MidiPlayerGlobal.CurrentMidiSet != null && MidiPlayerGlobal.CurrentMidiSet.MidiFiles != null && MidiPlayerGlobal.CurrentMidiSet.MidiFiles.Count > 0)
                {
                    ...
                    var selectedMidi = MidiPlayerGlobal.CurrentMidiSet.MidiFiles.FindIndex(s => s == MPTK_MidiName);
                    if (selectedMidi < 0)
                    {
                        Debug.LogWarning("MidiFilePlayer - MidiFile " + MPTK_MidiName +
                                         " not found. Try with the first in list.");
                        selectedMidi = 0;
                        MPTK_MidiName = MidiPlayerGlobal.CurrentMidiSet.MidiFiles[0];
                    }
                    ...
                    try
                    {
                        MPTK_MidiLoaded = new MidiLoad();

                        // No midi byte array, try to load from MidiFile from resource
                        if (midiBytesToLoad == null || midiBytesToLoad.Length == 0)
                        {
                            var mididata =
                                Resources.Load<TextAsset>(Path.Combine(MidiPlayerGlobal.MidiFilesDB, MPTK_MidiName));
                            midiBytesToLoad = mididata.bytes;
                        }

                        MPTK_MidiLoaded.MPTK_LogLoadEvents = MPTK_LogLoadEvents;
                        MPTK_MidiLoaded.MPTK_KeepNoteOff = MPTK_KeepNoteOff;
                        MPTK_MidiLoaded.MPTK_KeepEndTrack = MPTK_KeepEndTrack;
                        MPTK_MidiLoaded.MPTK_LogLoadEvents = MPTK_LogLoadEvents;
                        MPTK_MidiLoaded.MPTK_EnableChangeTempo = true;
                        if (!MPTK_MidiLoaded.MPTK_Load(midiBytesToLoad))
                            return false;
                        SetAttributes();
                        result = true;
                    }
        ...
        {
            if (MPTK_MidiLoaded != null)
            {
                MPTK_InitialTempo = MPTK_MidiLoaded.MPTK_InitialTempo;
                MPTK_Duration = MPTK_MidiLoaded.MPTK_Duration;
                MPTK_TickLast = MPTK_MidiLoaded.MPTK_TickLast;
                MPTK_NumberBeatsMeasure = MPTK_MidiLoaded.MPTK_NumberBeatsMeasure;
                MPTK_NumberQuarterBeat = MPTK_MidiLoaded.MPTK_NumberQuarterBeat;
                MPTK_TimeSigNumerator = MPTK_MidiLoaded.MPTK_TimeSigNumerator;
                MPTK_TimeSigDenominator = MPTK_MidiLoaded.MPTK_TimeSigDenominator;
                MPTK_KeySigSharpsFlats = MPTK_MidiLoaded.MPTK_KeySigSharpsFlats;
                MPTK_KeySigMajorMinor = MPTK_MidiLoaded.MPTK_KeySigMajorMinor;
                MPTK_TicksInMetronomeClick = MPTK_MidiLoaded.MPTK_TicksInMetronomeClick;
                MPTK_No32ndNotesInQuarterNote = MPTK_MidiLoaded.MPTK_No32ndNotesInQuarterNote;
                MPTK_MicrosecondsPerQuarterNote = MPTK_MidiLoaded.MPTK_MicrosecondsPerQuarterNote;
                MPTK_DeltaTicksPerQuarterNote = MPTK_MidiLoaded.MPTK_DeltaTicksPerQuarterNote;
                MPTK_TrackCount = MPTK_MidiLoaded.MPTK_TrackCount;
            }
        }

        /// <summary>
        ///     @brief
        ///     Read the list of midi events available in the Midi file from a ticks position to an end position into a List of
        ///     MPTKEvent
        ///     @snippet TestMidiFileLoad.cs Example TheMostSimpleDemoForMidiLoader
        ...
        public List<MPTKEvent> MPTK_ReadMidiEvents(long fromTicks = 0, long toTicks = long.MaxValue)
        {
            ...
            MPTK_MidiLoaded.MPTK_LogLoadEvents = MPTK_LogLoadEvents;
            MPTK_MidiLoaded.MPTK_EnableChangeTempo = true;
            return MPTK_MidiLoaded.MPTK_ReadMidiEvents(fromTicks, toTicks);
        }
        ...
        {
            if (MPTK_MidiLoaded != null)
                return MPTK_MidiLoaded.NoteLength(note);
