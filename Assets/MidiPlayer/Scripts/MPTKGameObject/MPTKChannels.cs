...
namespace MidiPlayerTK
{
    ...
    public class MPTKChannels : IEnumerable<MPTKChannel>
    {
        /// <summary>
        ///     @brief
        ///     Enable to reset all channels when MIDI start playing (if true, Channels member is allocated at the synth reinit).
        ///     Default is true.\n
        ...
        private List<MPTKChannel> Channels { get; }
        ...
        public void ResetExtension(int channelNum = -1)
        {
            ...
            {
                Debug.LogWarning(
                    $"MPTK_ResetChannels: channel number is incorrect, must be between 0 and {Channels.Count}");
            }
        }
    }
    ...
    public class MPTKChannel
    {
        ...
        /**
         * < MIDI polyphonic key pressure from
         * [
         * 0
         * ;
         * 1
        ...
        private readonly double[] gens;

        public byte[] key_pressure;
        ...
        public HiPreset HiPreset { get; private set; }
        ...
        /// <summary>
        ///     @brief
        ///     Get or Set current preset number associated to the channel. Value must be between 0 and 127 (if you have a full GM
        ///     SoundFont).\n
        ...
        public int PresetNum
        {
            ...
            set
            {
                ...
                {
                    Debug.LogWarning(
                        $"MPTK_Channel[{Channel}].PresetNum out of range, must be between 0 and 127, found {value}");
                }
                else
                {
                    LastPreset = value;
        ...
        public int BankNum
        {
            ...
            set
            {
                if (value < 0 || value > 16383)
                    Debug.LogWarning(
                        $"MPTK_Channel[{Channel}].BankNum out of range, must be between 0 and 16383, found {value}");
                banknum = value;
            }
        }
        ...
        public string PresetName => HiPreset != null ? HiPreset.Name : "no preset defined";


        /// <summary>
        ///     @brief
        ///     Get or Set forced preset on the channel. MIDI will allways playing with this preset even if a MIDI Preset Change
        ///     message is received.\n
            ...
            set
            {
                ...
                else
                {
                    ...
                    if (synth.VerboseChannel)
                        Debug.Log($"ForcedPreset Channel:{Channel} Restore preset to:{LastPreset}");
        ...
        /// <summary>
        ///     @brief
        ///     Get or Set forced bank on the channel from 0 to 65635. MIDI will allways using this bank even if a bank change
        ///     message is received.\n
        ///     Set to -1 to disable this behavior.
        /// </summary>
        /// <returns>preset index, -1 if not set</returns>
        ...
        public int ForcedBank
        {
            ...
            set
            {
                ...
                else
                {
                    ...
                    {
                        var sFont = synth.MPTK_SoundFont.SoundFont;
        ...
        public override string ToString()
        {
            ...
            var sMuted = Enable ? "Enabled" : "Muted";
        ...
        private void fluid_channel_init()
        {
            ...
            {
                banknum = Channel == 9
                    ? synth.MPTK_SoundFont.SoundFont.DrumKitBankNumber
                    : synth.MPTK_SoundFont.SoundFont.DefaultBankNumber;
        ...
        private void fluid_channel_init_ctrl()
        {
            ...
            */
            ...
            for (var i = (int)MPTKController.SOUND_CTRL1; i <= (int)MPTKController.SOUND_CTRL10; i++) cc[i] = 64;
        ...
        public int Controller(MPTKController numController, int valueController = -1)
        {
            ...
            {
                Debug.LogWarning(
                    $"MPTK_Channel[{Channel}].Controller out of range, must be between 0 and 127, found {numController} {(int)numController}");
                return -1;
            }

            if (valueController < 0) return cc[(int)numController];
            ...
            if (synth.VerboseController || synth.VerboseChannel)
                Debug.Log(
                    $"MPTK_Channel[{Channel}].Controller Control:{numController} Value:{valueController} Previous:{previousValue}");
        ...
        public HiPreset fluid_synth_find_preset(int banknum, int prognum)
        {
            ...
            var preset_found = CheckBankAndPresetExist(banknum, prognum);
            ...
            if (banknum != 0)
            {
                    ...
                    sFont.Banks[banknum].defpresets[prognum] != null)
                {
                    if (synth.VerboseVoice || synth.VerboseChannel)
                        Debug.Log($"Select the HiPreset bank:{banknum} preset:{prognum}.");
                    return sFont.Banks[banknum].defpresets[prognum];
                }
            }

            // Not find, return the first available preset
            foreach (var bank in sFont.Banks)
                if (bank != null)
                    foreach (var preset in bank.defpresets)
                        if (preset != null)
                        {
                            if (synth.VerboseVoice || synth.VerboseChannel)
                                Debug.Log($"Select the HiPreset {preset.Num} in the bank {bank.BankNumber}.");
                            return preset;
                        }
        ...
        private HiPreset CheckBankAndPresetExist(int banknum, int prognum)
        {
            if (synth.MPTK_SoundFont.IsReady)
            {
                ...
                {
                    if (sfont.Banks[banknum].defpresets != null && prognum < sfont.Banks[banknum].defpresets.Length &&
                        sfont.Banks[banknum].defpresets[prognum] != null)
                        return sfont.Banks[banknum].defpresets[prognum];
        ...
        public void fluid_synth_program_change(int preset)
        {
            if (synth.MPTK_SoundFont.IsReady)
            {
                ...
                if (Channel != 9 || synth.MPTK_EnablePresetDrum) // V2.89.0
                {
                    if (ForcedPreset >= 0)
                        preset = ForcedPreset;
                    ...
                    if (synth.VerboseVoice || synth.VerboseChannel)
                        Debug.LogFormat("ProgramChange\tChannel:{0}\tBank:{1}\tPreset:{2}", Channel, banknum, preset);
