...
namespace MidiPlayerTK
{
    ...
    public class MPTKStat
    {
        ...
        private readonly int[,] CountByNoteByChannel = new int[16, 128];
        public int CountChannel;
        public int CountNote;
        private readonly int[] CountNotesByChannel = new int[16];
        ...
        public void Calculate(List<MPTKEvent> midiEvents)
        {
            try
            {
                ...
                foreach (var mptkEvent in midiEvents)
                {
                    CountAll++;
    ...
    ///     @note
    ///     @li The tempo map is automaticalled build when a MIDI file is loaded from the MIDI DB, from an external MIDI or
    ///     from a MIDI Writer instance.
    ///     @li The tempo map must be run by script on your MIDI events when created with MidiFileWriter2 with:\n
    ///     #MPTK_CalculateTempoMap
    ///     @li Each segments defined the tick start/end and the start real time of the segment and the pulse (duration in
    ///     millisecond of a MIDI tick) wich is constant all along the segment.
    /// </summary>
    public class MPTKTempo
    {
        ...
        /// <summary>
        ///     @brief
        ///     Duration in millisecond of a MIDI tick in this segment. The pulse length is the minimum time in millisecond between
        ///     two MIDI events.\n
        ///     @note
        ...
        ///     @note
        ///     - V2.12.0.1 Thanks to cihadturhan_unity for fixing the bug. See here:
        ///     https://forum.unity.com/threads/maestro-midi-player-tool-kit-good-news-for-your-rhythm-game.526741/page-15#post-9865116
        ...
        public static int FindSegment(List<MPTKTempo> tempoMap, float timeSearch, int fromIndex = 0)
        {
            ...
            {
                // calculate ToTime
                var ToTime = tempoMap[indexTempo].FromTime +
        ...
        public static void CalculateMap(int deltaTicksPerQuarterNote, List<MPTKEvent> mptkEvents,
            List<MPTKTempo> temposMap)
        {
            ...
            temposMap.Add(new MPTKTempo(0,
                microsecondsPerQuarterNote: MPTKEvent.BeatPerMinute2QuarterPerMicroSecond(120),
                pulse: MPTKEvent.BeatPerMinute2QuarterPerMicroSecond(120) / (double)DeltaTicksPerQuarterNote / 1000d));
            var previousTempo = temposMap.Last();

            mptkEvents.ForEach(mptkEvent =>
            {
                if (mptkEvent.Command == MPTKCommand.MetaEvent && mptkEvent.Meta == MPTKMeta.SetTempo)
                {
                    var microsecondsPerQuarterNote = mptkEvent.Value;
                    if (microsecondsPerQuarterNote <= 0)
                    {
                        Debug.LogWarning(
                            $"SetTempo with incorrect MicrosecondsPerQuarterNote at position {mptkEvent.Tick} . Force tempo to 120.");
                        microsecondsPerQuarterNote = MPTKEvent.BeatPerMinute2QuarterPerMicroSecond(120);
                    }

                    var pulse = microsecondsPerQuarterNote / (double)DeltaTicksPerQuarterNote / 1000d;
                    ...
                    else
                    {
                        ...
                        temposMap.Add(new MPTKTempo(
                            ...
                            mptkEvent.Tick,
                            pulse: pulse,
                            fromTime: previousTempo.FromTime +
                                      (mptkEvent.Tick - previousTempo.FromTick) * previousTempo.Pulse,
                            microsecondsPerQuarterNote: microsecondsPerQuarterNote
                        ));
        ...
        {
            var toTick = ToTick == long.MaxValue ? "End" : ToTick.ToString();
            return
                $"   {Index} Tick:{FromTick,-7:000000} to {toTick,-7:000000}  Pulse(ms):{Pulse:F2}  Time(ms) from:{FromTime,10:F2} BPM:{(int)(60000000f / MicrosecondsPerQuarterNote)} Quarter(ms):{(int)(MicrosecondsPerQuarterNote / 1000f)}";
        }
    }
    ...
    ///     @note
    ///     @li The signature map is automatically built when a MIDI file is loaded from the MIDI DB, from an external MIDI, or
    ///     from a MIDI Writer instance.
    ...
    public class MPTKSignature
    {
        ...
        [Preserve]
        public MPTKSignature(int index, long fromTick = 0, long toTick = long.MaxValue, int numberBeatsMeasure = 4,
        ...
        {
            var measure = int.MaxValue;
            if (tick < long.MaxValue)
                measure = FromMeasure + (int)((tick - FromTick) / (float)DeltaTicksPerQuarterNote /
                                              (NumberBeatsMeasure * 4 / NumberQuarterBeat));
        ...
        public static long MeasureToTick(List<MPTKSignature> temposMap, int measure)
        {
            long tick = -1;
            //long index = -1;
            foreach (var tempoMap in temposMap)
                if (measure >= tempoMap.FromMeasure && measure <= tempoMap.ToMeasure)
                {
                    //Debug.Log(tempoMap.ToString());
                    tick = tempoMap.FromTick + (measure - tempoMap.FromMeasure) *
                        (tempoMap.NumberBeatsMeasure * 4 / tempoMap.NumberQuarterBeat) * DeltaTicksPerQuarterNote;
        ...
        public int CalculateBeat(long tick, int measure)
        {
            ...
            if (tick < long.MaxValue && DeltaTicksPerQuarterNote > 0)
            {
                if (NumberBeatsMeasure > 0)
                    ...
                    beat = (int)((tick - FromTick) / (float)DeltaTicksPerQuarterNote) + 1 -
                           (measure - FromMeasure) * (NumberBeatsMeasure * 4 / NumberQuarterBeat);
            }
            else
            {
                beat = int.MaxValue;
            ...
            List<MPTKSignature> signaturesMap)
        {
            ...
            mptkEvents.ForEach(mptkEvent =>
            {
                ...
                {
                    var sign = new MPTKSignature(
                        signaturesMap.Count,
                        mptkEvent.Tick,
                        numberBeatsMeasure: MPTKEvent.ExtractFromInt((uint)mptkEvent.Value, 0),
                        numberQuarterBeat: Convert.ToInt32(Mathf.Pow(2f,
            ...
            if (signaturesMap.Count == 0)
                signaturesMap.Add(new MPTKSignature(0));
        }
        ...
        public static void CalculateMeasureBoundaries(List<MPTKSignature> signaturesMap)
        {
            ...
            {
                var numberBeatsMeasure = signMap.NumberBeatsMeasure * 4 / signMap.NumberQuarterBeat;
                if (signMap.Index == 0)
                {
                    // TBD try [MethodImpl(MethodImplOptions.AggressiveInlining)]
                    signMap.FromMeasure =
                        (int)(signMap.FromTick / (float)DeltaTicksPerQuarterNote / numberBeatsMeasure) + 1;
                    if (signMap.ToTick == long.MaxValue)
                        signMap.ToMeasure = int.MaxValue;
                    else
                        signMap.ToMeasure =
                            (int)(signMap.ToTick / (float)DeltaTicksPerQuarterNote / numberBeatsMeasure);
                }
                else
                {
                    ...
                    if (signMap.ToTick == long.MaxValue)
                        ...
                        signMap.ToMeasure = signMap.FromMeasure + (int)((signMap.ToTick - signMap.FromTick) /
                                                                        (float)DeltaTicksPerQuarterNote /
                                                                        numberBeatsMeasure) - 1;
                }
        ...
        {
            var toTick = ToTick == long.MaxValue ? "End" : ToTick.ToString();
            var toBar = ToMeasure == int.MaxValue ? "End" : ToMeasure.ToString();
            return
                $"   {Index} Tick:{FromTick,-7:000000} to {toTick,-7:000000} Bar: {FromMeasure} to {toBar} Beats/Measure:{NumberBeatsMeasure}  Quarter/Quarter:{NumberQuarterBeat}";
        }
    }
}