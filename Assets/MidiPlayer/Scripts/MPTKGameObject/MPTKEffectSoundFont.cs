...
namespace MidiPlayerTK
{
    ...
    ///     Furthermore, to get more liberty from SoundFont, Maestro can increase or decrease the impact of effects (from the
    ...
    {
        [HideInInspector] [SerializeField] private bool applySFReverb, applySFChorus, applySFFilter = true;
        ...
        {
            get => applySFReverb;
            set => applySFReverb = value;
        }

        /// <summary>
        ///     Apply chorus effect as defined in the SoundFont.\n
        ///     This effect is processed with the fluidsynth algo independently on each voices but with a small decrease of
        ///     performace.
        ///     @version Maestro Pro
        ///     @code
        ...
        public void Init(MidiSynth psynth)
        {
            ...
            fx_reverb = new float[psynth
                .FLUID_BUFSIZE]; // FLUID_MAX_BUFSIZE not supported, each frame must have the same length
            reverb = new fluid_revmodel(psynth.OutputRate,
                psynth.FLUID_BUFSIZE); // FLUID_MAX_BUFSIZE not supported, each frame must have the same length

            fx_chorus = new float[psynth
                .FLUID_BUFSIZE]; // FLUID_MAX_BUFSIZE not supported, each frame must have the same length
            /* allocate the chorus module */
            chorus = new fluid_chorus(psynth.OutputRate,
                psynth.FLUID_BUFSIZE); // FLUID_MAX_BUFSIZE not supported, each frame must have the same length
#if MPTK_PRO
            SetParamSfReverb();
            SetParamSfChorus();
        ...
        {
            /* send to reverb */
            if (EnableReverb && reverb_buf != null) reverb.fluid_revmodel_processmix(reverb_buf, left_buf, right_buf);

            /* send to chorus */
            if (EnableChorus && chorus_buf != null) chorus.fluid_chorus_processmix(chorus_buf, left_buf, right_buf);
        }
