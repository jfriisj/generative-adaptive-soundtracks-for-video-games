...
{
    /// <summary>
    ///     Build and Play Real Time Music in relation with user actions or algorithms. This class must be used with the prefab
    ///     MidiStreamPlayer.\n
    ///     @attention
    ...
    ///     No MIDI file is necessary, the notes are generated by your scripts from your own algorithms by using the methods
    ...
    ///     All the values must be set in MPTKEvent, command, note value, duration ... for more details look at the class
    ///     MPTKEvent.\n\n
    ///     A note-on must also be stopped, : if duration = -1 the note is infinite, it's the goal of MPTK_StopEvent() to stop
    ///     the note with a note-off.\n
    ...
    ///     This class inherits from MidiSynth so all properties, events, methods from MidiSynth are available in this
    ///     class.\n\n
    ///     @note
    ///     - A full example bellow.
    ///     - You can't directly copy paste this extract. But the full source code is available for free with Maestro MPTK
    ///     free.
    ...
    public class MidiStreamPlayer : MidiSynth
    {
        ...
        /// <summary>
        ///     @brief
        ///     Force starting of the MidiStream. May be useful from another gamecomponent which want send midi command at the
        ///     start of the application.
        ///     @code
        ///     // Find a MidiStreamPlayer Prefab from the scene
        ///     MidiStreamPlayer MidiStream = FindFirstObjectByType
        ///     <MidiStreamPlayer>
        ///         ();
                    ...
                    if (!MPTK_CorePlayer)
                        ...
                        lock (this) // V2.83
                        {
                            QueueSynthCommand.Enqueue(new SynthCommand
                                { Command = SynthCommand.enCmd.StartEvent, MidiEvent = mptkEvent });
                        }
                }
                else
                {
                    Debug.LogWarning(
                        $"SoundFont not yet loaded, MIDI Event cannot be processed Code:{mptkEvent.Command} Channel:{mptkEvent.Channel}");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (MPTK_SoundFont.IsReady)
                {
                    if (!MPTK_CorePlayer)
                        ...
                        lock (this) // V2.83
                        {
                            foreach (var evnt in mptkEvents)
                                QueueSynthCommand.Enqueue(new SynthCommand
        ...
        public void MPTK_StopEvent(MPTKEvent mptkEvent)
        {
            if (!MPTK_CorePlayer)
                ...
                {
                    QueueSynthCommand.Enqueue(new SynthCommand
                        { Command = SynthCommand.enCmd.StopEvent, MidiEvent = mptkEvent });
                }
        }
    }
}