...
namespace MidiPlayerTK
{
    ...
    {
        private readonly StringBuilder logSynthInfo;
        ...
        public StringBuilder MPTK_BuildInfoSynth(MidiSynth synth)
        {
            try
            {
                if (synth != null)
                {
                    ...
                    logSynthInfo.Append(
                        $" '{(synth.AudioEngine != null ? synth.AudioEngine : "No Audio Engine found")}'");

                    if (synth.AudioThreadMidi)
                        logSynthInfo.Append(" 'MidiOnAudioThread'");
                    else
                        logSynthInfo.Append(
                            $" 'MidiMptkThread'   Prio:{synth.ThreadMidiPriorityReal} Wait:{synth.MPTK_ThreadMidiWait}");
                    ...
                    if (synth.StatDeltaThreadMidiMA != null && synth.StatDeltaThreadMidiMA.Count > 0 &&
                        synth.StatDeltaThreadMidiMIN < double.MaxValue)
                    {
                        logSynthInfo.Append(
                            $"    Mini:{(synth.StatDeltaThreadMidiMIN < double.MaxValue ? (int)Math.Round(synth.StatDeltaThreadMidiMIN) : 0),-4}");
                    ...
                    logSynthInfo.Append(
                        $"  Reused:{synth.MPTK_StatVoiceCountReused,-3}  Ratio:{Mathf.RoundToInt(synth.MPTK_StatVoiceRatioReused),-2}%");
                    logSynthInfo.Append(
                        $"   In Cache:{synth.MPTK_StatVoiceCountFree,-3} Free:{synth.FreeVoices?.Count,-3}");
                    ...
                    {
                        logSynthInfo.Append(
                            $"  Mini:{(synth.StatAudioFilterReadMIN < double.MaxValue ? Math.Round(synth.StatAudioFilterReadMIN, 2) : 0),-5}");
                    ...
                    for (int i = 0; i < MaxLengthHisto; i++)
                    {
                        ...
                        {
                            if (audioHistoric[i].DeltaOnAudio < minDeltaOnAudio) minDeltaOnAudio =
 audioHistoric[i].DeltaOnAudio;
                            if (audioHistoric[i].DeltaOnAudio > maxDeltaOnAudio) maxDeltaOnAudio =
 audioHistoric[i].DeltaOnAudio;
                            if (audioHistoric[i].SizeBuffOnAudio < 64) countInf64++;
                            if (audioHistoric[i].SizeBuffOnAudio % 64 != 0) countNot64++;
                            if (audioHistoric[i].SizeBuffOnAudio < minSizeBuff) minSizeBuff =
 audioHistoric[i].SizeBuffOnAudio;
                            if (audioHistoric[i].SizeBuffOnAudio > maxSizeBuff) maxSizeBuff =
 audioHistoric[i].SizeBuffOnAudio;
                        }
                    }
                    ...
                    for (int i = 0; i < MaxLengthHisto; i++)
                    {
                        if (audioHistoric != null && audioHistoric[i] != null)
                        {
                            ...
                            string info =
 $"[{audioHistoric[i].SizeBuffOnAudio:000} {audioHistoric[i].DeltaOnAudio:0000} {audioHistoric[i].TimeSynthProcess:0000}]";
