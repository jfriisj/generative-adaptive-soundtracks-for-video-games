...
namespace MidiPlayerTK
{
    ...
    ///     It contains an instance of the class MidiLoad with all detailed information about the MIDI loaded and/or which are
    ///     playing. \n
    ...
    ///     MidiFilePlayer midiFilePlayer = FindFirstObjectByType
    ///     <MidiFilePlayer>
    ///         ();
    ...
    {
        /// <summary>
        ///     Defined the behavior of the MIDI player when playback is stopped with #MPTK_Stop or restarted when the last MIDI
        ///     events is reached and #MPTK_MidiAutoRestart is set to true.
        ...
        public static string[] ModeStopPlayLabel =
            { "Stop No Waiting", "Stop When All Voices Are Released", "Stop When All Voices Are Ended" };

        [SerializeField] [HideInInspector] protected string midiNameToPlay;
        ...
        [SerializeField] [HideInInspector] private int midiIndexToPlay;
        ...
        /// </summary>
        [HideInInspector] public LoadingStatusMidiEnum MPTK_StatusLastMidiLoaded;

        /// <summary>
        ///     @brief
        ///     Contains the error from the web request when loading MIDI from an URL
        /// </summary>
        [HideInInspector] public string MPTK_WebRequestError;

        /// <summary>
        ///     @brief
        ///     Method triggered for each MIDI event (or group of MIDI events) ready to be played by the MIDI synth. All these
        ///     events are on same MIDI tick\n.
        ...
        ///         midiFilePlayer = FindFirstObjectByType
        ///         <MidiFilePlayer>
        ///             ();
        ...
        ///                 Debug.Log($"Note on Time:{mptkEvent.RealTime} millisecond  Note:{mptkEvent.Value}
        ///                 Duration:{mptkEvent.Duration} millisecond  Velocity:{mptkEvent.Velocity}");
        ...
        ///     - Set #MPTK_KeepEndTrack or #MPTK_KeepNoteOff to true when loading the MIDI file to synchronise the end of playback
        ///     with the real end of the MIDI file.
        ///     @code
        ///     using MidiPlayerTK; // Add a reference to the MPTK namespace at the top of your script
        ///     using UnityEngine;
        ///     public class YourClass : MonoBehaviour
        ...
        [SerializeField] [HideInInspector] private int quantization;
        ...
        private bool playOnStart,
            ...
            keepEndTrack,
            needDelayToStart,
            needDelayToStop, /* V2.84 */
            rawSeek;

        [SerializeField] [HideInInspector] public bool nextMidi, prevMidi;

        /// <summary>
        ///     @brief
        ///     Should the MIDI playing must be paused when the application lost the focus?
        /// </summary>
        [HideInInspector] public bool MPTK_PauseOnFocusLoss;

        //[SerializeField]
        //[HideInInspector]
        //protected bool playPause = false;

        [Range(0, 100)] private readonly float delayMilliSeconde = 15f; // only with AudioSource mode (non core)

        private readonly float delayRampDnSecond = 0f;
        private readonly float delayRampUpSecond = 0f;
        ...
        private readonly float timeRampDnSecond = 0f;

        //private float delayToStopMilliseconds = 100f;

        private readonly float timeRampUpSecond = 0f;
        ...
        ///     By transmitting MIDI messages on their respective channels, you can alter the instrument, volume, pitch, and other
        ///     parameters. \n
        ///     Within the Maestro Midi Player Toolkit, MIDI channels are designated numerically from 0 to 15. Notably, channel 9
        ///     is set aside specifically for drum sounds.
        ///     @snippet TestMidiFilePlayerScripting.cs ExampleUsingChannelAPI_Full
        /// </summary>
        public MPTKChannels MPTK_Channels => Channels;
        ...
        ///     The Index of a MIDI file is displayed in the popup from the MidiFilePlayer inspector and in the window "Midi File
        ///     Setup" from the MPTK menu in the editor.\n
        ///     @code
        ...
        /// <summary>
        ///     @brief
        ///     @deprecated with 2.10.0 MPTK_Loop is deprecated. Please investigate #MPTK_MidiRestart or #MPTK_InnerLoop (Pro) for
        ///     a better looping accuracy.
        /// </summary>
        public bool MPTK_Loop
        {
            get
            {
                Debug.LogWarning("MPTK_Loop is deprecated. Please investigate MPTK_MidiAutoRestart.");
                return false;
            }
        ...
        /// <summary>
        ///     @brief
        ///     When the value is true, the current MIDI playing is restarted when it reaches the end of the MIDI file or
        ///     #MPTK_MidiLoaded.MPTK_TickEnd.\n
        ///     @note
        ///     - The MIDI file is not reloaded, the restart is quite immediate.
        ///     - The restart is processed by the main Unity thread, also one Unity frame or more is needed to restart the playing
        ///     of the MIDI.
        ///     - #MPTK_MidiLoaded.MPTK_TickStart and #MPTK_MidiLoaded.MPTK_TickEnd are useful to defined start and end playing
        ///     position.
        ...
        /// </summary>
        public bool MPTK_MidiAutoRestart
        {
            get => midiAutoRestart;
            set => midiAutoRestart = value;
        }
        ...
        public double MPTK_Tempo
        {
            get
            {
                if (midiLoaded != null) return midiLoaded.MPTK_CurrentTempo;
                return 0d;
            }
            set
            {
        ...
        /// </summary>
        public string MPTK_SequenceTrackName => midiLoaded != null ? midiLoaded.SequenceTrackName : "";
        ...
        ///     Can be used only when the MIDI is playing.
        /// </summary>
        public string MPTK_ProgramName => midiLoaded != null ? midiLoaded.ProgramName : "";
        ...
        ///     Can be used only when the MIDI is playing.
        /// </summary>
        public string MPTK_TrackInstrumentName => midiLoaded != null ? midiLoaded.TrackInstrumentName : "";
        ...
        ///     Can be used only when the MIDI is playing.
        /// </summary>
        public string MPTK_TextEvent => midiLoaded != null ? midiLoaded.TextEvent : "";
        ...
        ///     Can be used only when the MIDI is playing.
        /// </summary>
        public string MPTK_Copyright => midiLoaded != null ? midiLoaded.Copyright : "";

        /// <summary>
        ///     @brief
        ///     Percentage of the default playback speed. Range  0.1 (10% of the current BPM) to 10 (1000%). Default is 1 for
        ///     normal speed.\n
        ///     Speed also applied to the duration of the sample played at voice level (often multiple voices are played for one
        ///     note).
        ///     @note:
        ///     - MPTK_Pulse is modified, formula for pulse (60000000 /  MPTK_CurrentTempo) / MPTK_DeltaTicksPerQuarterNote / 1000
        ///     / Speed
        ...
        ///     When a new position is set, the corresponding tick in the MIDI event list is searched for by comparing it with the
        ///     MPTKEvent.RealTime value, and the current MIDI playback is moved to this new tick position.\n
        ///     @note
        ...
        ///     - When the MIDI is playing look at the inspector of the MidiFilePlayer prefab to read (or change) the current
        ///     position in seconds and find the position you want.
        ///     - see also :
        ///     - #MPTK_Load
        ...
        public double MPTK_Position
        {
            get =>
                ...
                MPTK_LastEventPlayed != null ? MPTK_LastEventPlayed.RealTime : 0;
            set
            {
                try
                {
        ...
        /// </summary>
        public bool MPTK_IsPaused => playPause;

        /// <summary>
        ///     @brief
        ///     Is MIDI file is playing ?
        /// </summary>
        public bool MPTK_IsPlaying => midiIsPlaying;

        /// <summary>
        ///     @brief
        ///     Get detailed information about the MIDI playing. This readonly properties is available only when a MIDI has been
        ///     loaded.
        /// </summary>
        public MidiLoad MPTK_MidiLoaded => midiLoaded;
        //! @endcond
        ...
        public TimeSpan MPTK_Duration
        {
            get
            {
                try
                {
                    if (midiLoaded != null) return midiLoaded.MPTK_Duration;
                }
                catch (Exception ex)
                {
                    MidiPlayerGlobal.ErrorDetail(ex);
        ...
        public float MPTK_DurationMS
        {
            get
            {
                try
                {
                    if (midiLoaded != null) return midiLoaded.MPTK_DurationMS;
                }
                catch (Exception ex)
                {
                    MidiPlayerGlobal.ErrorDetail(ex);
        ...
        /// <summary>
        ///     @brief
        ///     Last tick tick in Midi: it's the value of the tick for the last MIDI event in sequence expressed in number of
        ///     "ticks".\n
        /// </summary>
        public long MPTK_TickLast => midiLoaded != null ? midiLoaded.MPTK_TickLast : 0;
        ...
        /// </summary>
        public long MPTK_TickFirstNote => midiLoaded != null ? midiLoaded.MPTK_TickFirstNote : 0;
        ...
        /// </summary>
        public long MPTK_TickLastNote => midiLoaded != null ? midiLoaded.MPTK_TickLastNote : 0;
        ...
        /// </summary>
        public double MPTK_PositionFirstNote => midiLoaded != null ? midiLoaded.MPTK_PositionFirstNote : 0;
        ...
        ///     See also the #MPTK_DurationMS which provides the full time of all MIDI events including track-end, control at the
        ///     beginning and at the end, ....\n
        ///     See also #MPTK_TickLastNote
        /// </summary>
        public double MPTK_PositionLastNote => midiLoaded != null ? midiLoaded.MPTK_PositionLastNote : 0;
        ...
        /// </summary>
        public int MPTK_TrackCount => midiLoaded != null ? midiLoaded.MPTK_TrackCount : 0;
        ...
        ///     @details
        ///     MIDI tick is an easy way to identify a position in a song independently of the time which could vary with tempo
        ///     change event.\n
        ///     The count of ticks by quarter is constant all along a MIDI, it's a properties of the whole MIDI. see
        ...
        ///     - when the MIDI is playing look at the inspector of the MidiFilePlayer prefab to read (or change) the current tick
        ///     and find the tick you want.\n
        ...
        {
            get => midiLoaded != null ? midiLoaded.MPTK_TickCurrent : 0;
            set
            {
                try
                {
        ...
        ///     By default (false), when the playback position is changed, all events except "note-on" are replayed from the start
        ///     of the MIDI file up to the new position.
        ///     This ensures the synthesizer is correctly updated with the appropriate context (tempo, selected instruments,
        ///     controllers, etc.).\n
        ///     If set to true, the playback position is updated directly; consequently, the current context is preserved. \n
        ///     While this approach can lead to unintended (or amusing!) effects in some MIDI files, it allows for much faster
        ///     position changes.
        ///     @version 2.12.0
        ///     @note
        ///     - This value resets to false (default) each time a MIDI file is loaded. You can use #MPTK_Load() to load the MIDI,
        ///     adjust this parameter, and then play the MIDI using #MPTK_Play().
        /// </summary>
        public bool MPTK_RawSeek
        {
            get => midiLoaded != null ? midiLoaded.MPTK_RawSeek : rawSeek;
            set
            {
                rawSeek = value;
                if (midiLoaded != null)
        ...
        /// </summary>
        public MPTKEvent MPTK_LastEventPlayed => midiLoaded?.MPTK_LastEventPlayed;

        /// <summary>
        ///     @brief
        ///     Lenght in millisecond of a MIDI tick. The pulse length is the minimum time in millisecond between two MIDI
        ///     events.\n
        ///     It's like a definition of graphical resolution but for MIDI: the MIDI sequencer will not be able to play two
        ///     separate MIDI events in a time below this value.\n
        ///     @note
        ...
        public double MPTK_Pulse
        {
            get
            {
                try
                {
                    if (midiLoaded != null) return midiLoaded.MPTK_Pulse;
                }
                catch (Exception ex)
                {
                    MidiPlayerGlobal.ErrorDetail(ex);
        ...
        public double MPTK_PulseLenght
        {
            get
            {
                Debug.LogWarning("MPTK_PulseLenght has been deprecated, please investigate MPTK_Pulse in place");
                return 0d;
            }
        }
        //! @endcond
        ...
        /// <summary>
        ///     @brief
        ///     Real time in TimeSpan format from the beginning of play. It's an access to the MIDI timer used by the MIDI
        ///     sequencer.
        ///     @note
        ///     #MPTK_Tempo or #MPTK_Speed change have no direct impact on this value.
        /// </summary>
        public TimeSpan MPTK_PlayTime
        {
            get
            {
                try
                {
                    return TimeSpan.FromMilliseconds(timeMidiFromStartPlay);
                }
                catch (Exception ex)
                {
                    MidiPlayerGlobal.ErrorDetail(ex);
        ...
        ///     #MPTK_Tempo or #MPTK_Speed change have no direct impact on this value.
        /// </summary>
        public double MPTK_RealTime => timeMidiFromStartPlay;


        /// <summary>
        ///     @brief
        ///     A MIDI file is a kind of keyboard simulation: in general, a key pressed generates a 'note-on' and a key release
        ///     generates a 'note-off'.\n
        ///     But there is an other possibility in a MIDI file: create a 'note-on' with a velocity=0 wich must act as a
        ///     'midi-off'\n
        ...
        /// <summary>
        ///     @brief
        ///     If the value is true, text read from Text META (e.g. lyrics) will be read with UTF8 encoding. The default is
        ///     false.\n
        ///     The MIDI standard only allows ASCII characters for this META, but with this extension you will be able to read and
        ///     display\n
        ...
        /// </summary>
        public List<MPTKEvent> MPTK_MidiEvents => midiLoaded != null ? midiLoaded.MPTK_MidiEvents : null;
        ...
        public int MPTK_DeltaTicksPerQuarterNote
        {
            get
            {
                var DeltaTicksPerQuarterNote = 0;
                try
                {
        ...
        private new void Awake()
        {
            if (VerboseSynth)
                Debug.Log(
                    $"Awake MidiFilePlayer  midiIsPlaying:{midiIsPlaying} isPlaying:{Application.isPlaying} isEditor:{Application.isEditor}");
            AwakeMidiFilePlayer();
        }
        ...
        private new void Start()
        {
            if (VerboseSynth)
                Debug.Log(
                    $"Start MidiFilePlayer {name} isPlaying:{Application.isPlaying} midiIsPlaying:{midiIsPlaying} MPTK_PlayOnStart:{MPTK_PlayOnStart}");
        ...
        protected void StartMidiFilePlayer()
        {
            ...
            try
            {
                ...
                if (MPTK_PlayOnStart) Routine.RunCoroutine(TheadPlayIfReady(), Segment.RealtimeUpdate);
            }
            catch (Exception ex)
            {
                MidiPlayerGlobal.ErrorDetail(ex);
                ...
                if (MPTK_SoundFont.IsReady)
                {
                    ...
                    else if (!MPTK_IsPlaying)
                    {
                        ...
                        if (!alreadyLoaded)
                        {
                            ...
                            if (MidiPlayerGlobal.ImSFCurrent != null && MidiPlayerGlobal.CurrentMidiSet != null &&
                                ...
                                MidiPlayerGlobal.CurrentMidiSet.MidiFiles.Count > 0)
                            {
                                ...
                                var selectedMidi =
                                    MidiPlayerGlobal.CurrentMidiSet.MidiFiles.FindIndex(s => s == MPTK_MidiName);
                                if (selectedMidi < 0)
                                {
                                    Debug.LogWarning(
                                        $"MidiFilePlayer - MidiFile '{MPTK_MidiName}' not found. Trying with the first in list.");
                                    selectedMidi = 0;
                                    MPTK_MidiName = MidiPlayerGlobal.CurrentMidiSet.MidiFiles[0];
                                }
                            }
                        ...
                        if (MPTK_CorePlayer)
                        {
                            ...
                            if (Application.isPlaying)
                                Routine.RunCoroutine(
                                    ThreadCorePlay(alreadyLoaded: alreadyLoaded).CancelWith(gameObject),
                                    Segment.RealtimeUpdate);
                            else
                                Routine.RunCoroutine(ThreadCorePlay(alreadyLoaded: alreadyLoaded),
                                    Segment.EditorUpdate);
                        }
                        else
                        {
                            //Debug.Log($"MPTK_Play ThreadLegacyPlay");
                            Routine.RunCoroutine(ThreadLegacyPlay(alreadyLoaded: alreadyLoaded).CancelWith(gameObject),
                                Segment.RealtimeUpdate);
                        }
                    }
                }
        ...
        /// <summary>
        ///     @brief
        ///     Stops MIDI playback and cancels all sounds. This operation is performed in background, so MIDI may really stop
        ...
        ///     after this method returns.
        /// </summary>
        /// <param name="stopAllSound">
        ///     Set to true to stop all sounds (default), otherwise currently playing notes will continue
        ///     until they finish.
        /// </param>
        /// <param name="wait">
        ///     With Legacy mode and if greater than -1 (v2.16), waits until MIDI playback is fully stopped or the
        ///     specified wait time (in milliseconds) is reached. Otherwise, returns immediately.
        /// </param>
        public void MPTK_Stop(bool stopAllSound = true, float wait = -1f)
        {
            ...
            if (stopAllSound)
            {
                Routine.RunCoroutine(ThreadClearAllSound(true, IdSession),
                    Application.isPlaying ? Segment.RealtimeUpdate : Segment.EditorUpdate);
                if (!MPTK_CorePlayer)
                    Routine.RunCoroutine(ThreadVoiceAudioSourceRemove(wait, IdSession),
                        Application.isPlaying ? Segment.RealtimeUpdate : Segment.EditorUpdate);
            }
            //Debug.Log($"<<< MPTK_Stop");
        }
        ...
        /// <summary>
        ///     @brief
        ///     Load the midi file defined with MPTK_MidiName or MPTK_MidiIndex. It's an optional action before playing a midi file
        ///     with MPTK_Play()\n
        ...
        /// </summary>
        /// <returns>
        ///     MidiLoad to access all the properties of the midi loaded, null in case of error look at
        ...
        public MidiLoad MPTK_Load()
        {
            ...
            var mididata = Resources.Load<TextAsset>(Path.Combine(MidiPlayerGlobal.MidiFilesDB, MPTK_MidiName));
        ...
        public void MPTK_ResetTranspose()
        {
            if (midiLoaded == null || midiLoaded.MPTK_MidiEvents == null)
                ...
                foreach (var mPTKEvent in midiLoaded.MPTK_MidiEvents)
                    if (mPTKEvent.Command == MPTKCommand.NoteOn)
        ...
        /// <returns></returns>
        /*protected */
        public IEnumerator<float> ThreadLegacyPlay(byte[] midiBytesToPlay = null, string midiName = "",
            float fromPosition = 0, float toPosition = 0, bool alreadyLoaded = false)
        {
            double deltaTime = 0;
            midiIsPlaying = true;
            stopMidi = false;
            replayMidi = false;
            var first = true;
            var currentMidiName = midiName != null ? midiName : "";
            ...
            else
            {
                try
                {
                    ...
                    if (midiBytesToPlay == null || midiBytesToPlay.Length == 0)
                    {
                        ...
                        var mididata =
                            Resources.Load<TextAsset>(Path.Combine(MidiPlayerGlobal.MidiFilesDB, currentMidiName));
            ...
            if (midiLoaded != null && midiLoaded.MPTK_MidiEvents != null && midiLoaded.MPTK_MidiEvents.Count != 0)
            {
                ...
                timeAtStartMidi = DateTime.UtcNow.Ticks / 10000D;
                ResetMidiPlayer();
                ...
                do
                {
                    ...
                    if (MPTK_Spatialize)
                    {
                        distanceToListener = MidiPlayerGlobal.MPTK_DistanceToListener(transform);
                    ...
                    if (playPause)
                    {
                        ...
                        yield return Routine.WaitForSeconds(0.2f);
                        if (midiLoaded.EndMidiEvent || replayMidi || stopMidi) break;
                    ...
                    if (midiLoaded.EndMidiEvent || replayMidi ||
                        stopMidi /*|| (toPosition > 0 && toPosition > fromPosition && MPTK_Position > toPosition)*/
                       ) break;
                    ...
                    if (midievents != null && midievents.Count > 0)
                    {
                        ...
                        var beforePLay = Time.realtimeSinceStartup;
                    ...
                    {
                        var times = TimeSpan.FromMilliseconds(MPTK_Position);
                        playTimeEditorModeOnly = string.Format("{0:00}:{1:00}:{2:00}:{3:000}", times.Hours,
                            times.Minutes, times.Seconds, times.Milliseconds);
                        durationEditorModeOnly = string.Format("{0:00}:{1:00}:{2:00}:{3:000}", MPTK_Duration.Hours,
                            MPTK_Duration.Minutes, MPTK_Duration.Seconds, MPTK_Duration.Milliseconds);
                    }
            ...
            try
            {
                var reason = EventEndMidiEnum.MidiEnd;
                if (nextMidi)
                {
        ...
        public IEnumerator<float> ThreadCorePlay(byte[] midiBytesToPlay = null, float fromPosition = 0,
            float toPosition = 0, bool alreadyLoaded = false)
        {
            StartPlaying();
            var currentMidiName = MPTK_MidiName;
            if (alreadyLoaded)
            {
                //Debug.Log("Start playing same loaded MIDI");
            }
            ...
            if (Application.isPlaying)
                Routine.RunCoroutine(
                    ThreadInternalMidiPlaying(currentMidiName, fromPosition, toPosition, alreadyLoaded)
                        .CancelWith(gameObject), Segment.RealtimeUpdate);
            else
                Routine.RunCoroutine(
                    ThreadInternalMidiPlaying(currentMidiName, fromPosition, toPosition, alreadyLoaded),
                    Segment.EditorUpdate);
        ...
        protected IEnumerator<float> ThreadInternalMidiPlaying(string currentMidiName, float fromPosition = 0,
            float toPosition = 0, bool alreadyLoaded = false)
        {
            if (midiLoaded != null && midiLoaded.MPTK_MidiEvents != null && midiLoaded.MPTK_MidiEvents.Count != 0)
            {
                ...
                do
                {
                    ...
                    do
                    {
                        ...
                        {
                            distanceToListener = MidiPlayerGlobal.MPTK_DistanceToListener(transform);
                            var pause = distanceToListener > MPTK_MaxDistance;
                        ...
                        {
                            var pct = (timeRampUpSecond - Time.realtimeSinceStartup) / delayRampUpSecond;
                        ...
                        if (needDelayToStop)
                        {
                            var pct = (timeRampDnSecond - Time.realtimeSinceStartup) / delayRampDnSecond;
                            //Debug.Log($"{DateTime.UtcNow.ToLongTimeString()} {timeAtNeedToStopSecond - Time.realtimeSinceStartup} {delayNeedToStopSecond} {pct}");
                            if (pct > 0f)
                        ...
                        if (midiLoaded.EndMidiEvent)
                            if (MPTK_ModeStopVoice == ModeStopPlay.StopNoWaiting ||
                                (MPTK_ModeStopVoice == ModeStopPlay.StopWhenAllVoicesReleased &&
                                 MPTK_StatVoiceCountPlaying <= 0) ||
                                (MPTK_ModeStopVoice == ModeStopPlay.StopWhenAllVoicesEnded &&
                                 MPTK_StatVoiceCountActive <= 0))
                            {
        ...
        private void DequeueMidiEvents()
        {
            try
            {
                ...
                {
                    var midievents = QueueMidiEvents.Dequeue();
                    if (midievents != null && midievents.Count > 0)
                    {
