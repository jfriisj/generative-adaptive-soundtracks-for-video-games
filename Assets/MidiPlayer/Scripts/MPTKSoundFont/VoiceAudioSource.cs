...
namespace MidiPlayerTK
{
    public class VoiceAudioSource : MonoBehaviour
    {
        ...
        public void Awake()
        {
            ...
            if (Audiosource == null)
                Debug.LogWarning(
                    "No AudioSource attached to the VoiceAudioSource. Check VoiceAudioSource in your Hierarchy.");
        }
        ...
        protected IEnumerator<float> ThreadPlayNote()
        {
            ...
            if (Audiosource != null && Audiosource.gameObject.activeInHierarchy)
            {
                ...
                Audiosource.loop = fluidvoice.samplemode == fluid_loop.FLUID_LOOP_UNTIL_RELEASE ||
                                   fluidvoice.samplemode == fluid_loop.FLUID_LOOP_DURING_RELEASE;
                last_pan = -1; // Audiosource.panStereo = !synth.MPTK_EnablePanChange ? 0f : Mathf.Lerp(-1f, 1f, (fluidvoice.pan + 500f) / 1000f);
                last_pitch =
                    -1; // Audiosource.pitch = Mathf.Pow(fluid_voice._ratioHalfTone, (fluidvoice.pitch - fluidvoice.root_pitch) / 100f);

                if (synth.VerboseVoice)
                    Debug.LogFormat(
                        "   fluid_voice_start Audiosource volume:{0:0.000} loop:{1} pan:{2:0.000} pitch:{3:0.000} {4}",
                        Audiosource.volume, Audiosource.loop, Audiosource.panStereo, Audiosource.pitch,
                        synth.MPTK_WeakDevice ? "[WEAK]" : "[FULL]");
                    ...
                    if (!fluidvoice.weakDevice)
                    {
                        ...
                        if (last_pan != fluidvoice.pan)
                        {
                            ...
                            if (synth.VerboseCalcGen)
                                Debug.LogFormat(
                                    $"CalcGen - EnablePanChange={synth.MPTK_EnablePanChange} last_pan={last_pan:0.00} new pan={fluidvoice.pan:0.00} --> panStereo={Audiosource.panStereo:0.00}");
                        }
                        ...
                        if (last_pitch != fluidvoice.pitch)
                        {
                            ...
                            Audiosource.pitch = Mathf.Pow(fluid_voice._ratioHalfTone,
                                (fluidvoice.pitch - fluidvoice.root_pitch) / 100f);
                        }
        ...
        public void fluid_voice_audiosource_write(long ticks)
        {
            ...
            var env_data = fluidvoice.volenv_data[(int)fluidvoice.volenv_section];
            ...
            var x = fluidvoice.volenv_val;
            if (env_data.incr > 0)
                x += fluidvoice.DeltaTimeWrite / (float)env_data.count;
            ...
            if (!synth.AdsrSimplified && fluidvoice.volenv_section == fluid_voice_envelope_index.FLUID_VOICE_ENVATTACK)
            {
                ...
                if (synth.MPTK_ApplyModLfo)
                    amp = (float)(fluid_conv.fluid_cb2amp(fluidvoice.attenuation) *
                                  fluid_conv.fluid_cb2amp(fluidvoice.modlfo_val * -fluidvoice.modlfo_to_vol) *
                                  fluidvoice.volenv_val) * synth.MPTK_Volume;
                else
                    amp = (float)(fluid_conv.fluid_cb2amp(fluidvoice.attenuation) * fluidvoice.volenv_val) *
                          synth.MPTK_Volume;
            }
            else
            {
            ...
            if (!Mathf.Approximately(Audiosource.volume, amp))
            {
                ...
                else
                {
                    var velocity = 0f;
                    Audiosource.volume =
                        Mathf.SmoothDamp(Audiosource.volume, amp, ref velocity, synth.DampVolume * 0.001f);
                }

                if (synth.VerboseVolume)
                    Debug.LogFormat(
                        "Volume [Id:{0}] {1} TimeFromStart:{2} Delta:{3} Att:{4,0:F2} modlfo_val:{5,0:F2} modlfo_to_vol:{6,0:F2} volenv_val:{7,0:F2} modenv_val:{8,0:F2} Amp:{9,0:F3} --> volume:{10,0:F3} {11}",
                        fluidvoice.id, "", fluidvoice.TimeFromStart / fluid_voice.Nano100ToMilli,
                        fluidvoice.DeltaTimeWrite / fluid_voice.Nano100ToMilli,
                        fluidvoice.attenuation, fluidvoice.modlfo_val, fluidvoice.modlfo_to_vol, fluidvoice.volenv_val,
                        fluidvoice.modenv_val, amp, Audiosource.volume, fluidvoice.volenv_section);
            }
        ...
        public void fluid_weakvoice_write(long ticks)
        {
            ...
            Audiosource.volume =
                Mathf.Lerp(Audiosource.volume, 0f, fluidvoice.volenv_count / synth.MPTK_ReleaseTimeMin);

            if (synth.VerboseEnvVolume)
                Debug.LogFormat(
                    "[Weak] Volume - [{0,4}] TimeFromStart:{1:0.000} Delta:{2:0.000} synth.ReleaseTimeMin:{3:0.000} volenv_count:{4:0.000} Volume:{5:0.0000}",
                    fluidvoice.id, fluidvoice.TimeFromStart / fluid_voice.Nano100ToMilli,
                    fluidvoice.DeltaTimeWrite / fluid_voice.Nano100ToMilli,
                    synth.MPTK_ReleaseTimeMin / fluid_voice.Nano100ToMilli, fluidvoice.volenv_count,
                    Audiosource.volume);
