...
namespace MidiPlayerTK
{
    public class SFFile
    {
        ...
        public const int FLUID_SAMPLETYPE_OGG_VORBIS = 0x10;

        /**
         * < Flag used for Ogg Vorbis compressed samples ( non-standard compliant extension) as found in the program "sftools"
         *     developed by Werner Schweer from MuseScore @ since
         * 1
         * .
         * 1
         * .
         ...
         */
        public const int
            SF_SAMPLETYPE_ROM = 0x8000; // 32768 Flag that indicates ROM samples, causing the sample to be ignored
        ...
        {
            var conv = name;
            foreach (var i in Path.GetInvalidFileNameChars()) conv = conv.Replace(i, '_');
            conv = conv.Replace('#', 'd');
            conv = conv.Replace('.', '-');
            conv = conv.Replace(' ', '-');
            return conv;
        ...
        public static void DumpSFToFile(SFData sf, string filename)
        {
            using (var writer = File.CreateText(filename))
            {
                foreach (var p in sf.preset)
                    if (p != null)
                    {
                        writer.WriteLine("Preset itemid:{0} bank:{1} prenum:{2} '{3}'", p.ItemId, p.Bank, p.Num,
                            p.Name);
                        foreach (var z in p.Zone)
                        {
                            if (z.Index < 0)
                                ...
                                writer.WriteLine("   PZone itemid:{0} index:{1} instrument:'{2}'", z.ItemId, z.Index,
                                    sf.inst[z.Index].Name);
                            else
                                writer.WriteLine("   PZone itemid:{0} index:{1} *** index not in instrument range ***",
                                    z.ItemId, z.Index);

                            if (z.gens != null)
                                foreach (var g in z.gens)
                                    if (g.Amount != null)
                                        writer.WriteLine("      Gen Id:{0,-20} lo:{1:000} hi:{2:000} sword:{3}", g.type,
                                            g.Amount.Lo, g.Amount.Hi, g.Amount.Sword);
                                    else
                                        writer.WriteLine("      Gen Id:{0,-20} amount is null", g.type);
                            if (z.mods != null)
                                foreach (var m in z.mods)
                                    writer.WriteLine("      Mod amount:{0} {1} {2} {3} {4} ", m.SfSrc, m.dest,
                                        m.SfAmtSrc, m.SfTrans, m.amount);
                        }
                    }

                writer.WriteLine("------------- Instrument -------------");

                foreach (var p in sf.inst)
                    //if (p.name.Contains("Mellow"))
                {
                    ...
                    foreach (var z in p.Zone)
                    {
                        if (z.Index < 0)
                            ...
                            writer.WriteLine("   IZone itemid:{0} index:{1} sample:'{2}'", z.ItemId, z.Index,
                                sf.Samples[z.Index].Name);
                        else
                            writer.WriteLine("   IZone itemid:{0} index:{1} *** index not in sample range ***",
                                z.ItemId, z.Index);
                ...
                foreach (var s in sf.Samples)
                    writer.WriteLine(
                        "itemid:{0} '{1,-20}' start:{2} end:{3} loopstart:{4} loopend:{5} origpitch:{6} pitchadj:{7} samplerate:{8}",
                        s.ItemId, s.Name, s.Start, s.End, s.LoopStart, s.LoopEnd, s.OrigPitch, s.PitchAdj,
                        s.SampleRate);
            }
        }
        ...
        {
            Array.Sort(sfdata.preset,
                delegate(HiPreset sp1, HiPreset sp2)
                {
                    return (sp1.Bank * 256 + sp1.Num).CompareTo(sp2.Bank * 256 + sp2.Num);
                });
