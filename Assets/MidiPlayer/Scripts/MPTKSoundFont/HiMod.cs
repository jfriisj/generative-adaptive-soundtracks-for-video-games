...
namespace MidiPlayerTK
{
    ...
    public class HiMod
    {
        ...
        public static int FLUID_NUM_MOD = 64;
        public float amount;
         ...
         */
        private static float fluid_mod_get_source_value(byte mod_src, byte mod_flags, ref float range,
            fluid_voice voice)
        {
         ...
         */
        private static float fluid_mod_transform_source_value(float val, byte mod_flags, float range)
        {
            /* normalized value, i.e. usually in the range [0;1] */
            var val_norm = val / range;
            ...
            {
                case (byte)fluid_mod_flags.FLUID_MOD_LINEAR | (byte)fluid_mod_flags.FLUID_MOD_UNIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_POSITIVE: /* =0 */
                    val = val_norm;
                    break;

                case (byte)fluid_mod_flags.FLUID_MOD_LINEAR | (byte)fluid_mod_flags.FLUID_MOD_UNIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_NEGATIVE: /* =1 */
                    val = 1.0f - val_norm;
                    break;

                case (byte)fluid_mod_flags.FLUID_MOD_LINEAR | (byte)fluid_mod_flags.FLUID_MOD_BIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_POSITIVE: /* =2 */
                    val = -1.0f + 2.0f * val_norm;
                    break;

                case (byte)fluid_mod_flags.FLUID_MOD_LINEAR | (byte)fluid_mod_flags.FLUID_MOD_BIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_NEGATIVE: /* =3 */
                    val = 1.0f - 2.0f * val_norm;
                    break;

                case (byte)fluid_mod_flags.FLUID_MOD_CONCAVE | (byte)fluid_mod_flags.FLUID_MOD_UNIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_POSITIVE: /* =4 */
                    val = fluid_conv.fluid_concave(127 * val_norm);
                    break;

                case (byte)fluid_mod_flags.FLUID_MOD_CONCAVE | (byte)fluid_mod_flags.FLUID_MOD_UNIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_NEGATIVE: /* =5 */
                    val = fluid_conv.fluid_concave(127 * (1.0f - val_norm));
                    break;

                case (byte)fluid_mod_flags.FLUID_MOD_CONCAVE | (byte)fluid_mod_flags.FLUID_MOD_BIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_POSITIVE: /* =6 */
                    val = val_norm > 0.5f
                        ? fluid_conv.fluid_concave(127 * 2 * (val_norm - 0.5f))
                        : -fluid_conv.fluid_concave(127 * 2 * (0.5f - val_norm));
                    break;

                case (byte)fluid_mod_flags.FLUID_MOD_CONCAVE | (byte)fluid_mod_flags.FLUID_MOD_BIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_NEGATIVE: /* =7 */
                    val = val_norm > 0.5f
                        ? -fluid_conv.fluid_concave(127 * 2 * (val_norm - 0.5f))
                        : fluid_conv.fluid_concave(127 * 2 * (0.5f - val_norm));
                    break;

                case (byte)fluid_mod_flags.FLUID_MOD_CONVEX | (byte)fluid_mod_flags.FLUID_MOD_UNIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_POSITIVE: /* =8 */
                    val = fluid_conv.fluid_convex(127 * val_norm);
                    break;

                case (byte)fluid_mod_flags.FLUID_MOD_CONVEX | (byte)fluid_mod_flags.FLUID_MOD_UNIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_NEGATIVE: /* =9 */
                    val = fluid_conv.fluid_convex(127 * (1.0f - val_norm));
                    break;

                case (byte)fluid_mod_flags.FLUID_MOD_CONVEX | (byte)fluid_mod_flags.FLUID_MOD_BIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_POSITIVE: /* =10 */
                    val = val_norm > 0.5f
                        ? fluid_conv.fluid_convex(127 * 2 * (val_norm - 0.5f))
                        : -fluid_conv.fluid_convex(127 * 2 * (0.5f - val_norm));
                    break;

                case (byte)fluid_mod_flags.FLUID_MOD_CONVEX | (byte)fluid_mod_flags.FLUID_MOD_BIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_NEGATIVE: /* =11 */
                    val = val_norm > 0.5f
                        ? -fluid_conv.fluid_convex(127 * 2 * (val_norm - 0.5f))
                        : fluid_conv.fluid_convex(127 * 2 * (0.5f - val_norm));
                    break;

                case (byte)fluid_mod_flags.FLUID_MOD_SWITCH | (byte)fluid_mod_flags.FLUID_MOD_UNIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_POSITIVE: /* =12 */
                    val = val_norm >= 0.5f ? 1.0f : 0.0f;
                    break;

                case (byte)fluid_mod_flags.FLUID_MOD_SWITCH | (byte)fluid_mod_flags.FLUID_MOD_UNIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_NEGATIVE: /* =13 */
                    val = val_norm >= 0.5f ? 0.0f : 1.0f;
                    break;

                case (byte)fluid_mod_flags.FLUID_MOD_SWITCH | (byte)fluid_mod_flags.FLUID_MOD_BIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_POSITIVE: /* =14 */
                    val = val_norm >= 0.5f ? 1.0f : -1.0f;
                    break;

                case (byte)fluid_mod_flags.FLUID_MOD_SWITCH | (byte)fluid_mod_flags.FLUID_MOD_BIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_NEGATIVE: /* =15 */
                    val = val_norm >= 0.5f ? -1.0f : 1.0f;
                    break;
                 ...
                 * is close enough.
                 */
                case (byte)fluid_mod_flags.FLUID_MOD_SIN | (byte)fluid_mod_flags.FLUID_MOD_UNIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_POSITIVE: /* custom sin(x) */
                    val = Mathf.Sin(Mathf.PI / 2.0f * 0.87f * val_norm);
                    break;

                case (byte)fluid_mod_flags.FLUID_MOD_SIN | (byte)fluid_mod_flags.FLUID_MOD_UNIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_NEGATIVE: /* custom */
                    val = Mathf.Sin(Mathf.PI / 2.0f * 0.87f * (1.0f - val_norm));
                    break;

                case (byte)fluid_mod_flags.FLUID_MOD_SIN | (byte)fluid_mod_flags.FLUID_MOD_BIPOLAR |
                     (byte)fluid_mod_flags.FLUID_MOD_POSITIVE: /* custom */
                    val = val_norm > 0.5f
                        ? Mathf.Sin(Mathf.PI * (val_norm - 0.5f))
         ...
         */
        private bool fluid_mod_check_non_cc_source(int src1_select)
        {
            byte flags, src;
         ...
         */
        private bool fluid_mod_check_cc_source(int src1_select)
        {
            byte flags, src;
        ...
        {
            return string.Format("Mod amount:{0} src1:{1} flags1:{2} src2:{3} flags2:{4} dest:{5}", amount, src1,
                flags1, src2, flags2, (fluid_gen_type)dest);
        }
        ...
        public void fluid_dump_modulator(float valMod)
        {
            ...
            dump.Append(
                $" amount:{amount,-10:#######.##} flags:{flags1,-2:##} src2:{src2} flags2:{flags2,-2:##} incGenerator:{valMod,-10:#######.##}\n");
            Debug.Log(dump);
        }
    }
}