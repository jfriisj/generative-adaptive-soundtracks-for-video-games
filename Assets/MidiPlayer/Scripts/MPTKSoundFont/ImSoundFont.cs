...
namespace MidiPlayerTK
{
    ...
    public class ImSoundFont
    {
        ...
        [XmlIgnore] public bool LiveSF;

        [XmlIgnore] public bool[] BankSelected;

        [XmlIgnore] public float[] SamplesData;

        /// <summary>
        ///     @brief
        ///     List  of banks of the sound font
        /// </summary>
        [XmlIgnore] public ImBank[] Banks;

        [XmlIgnore] public SFData HiSf;
        ...
        {
            for (var b = 0; b < BankSelected.Length; b++)
                BankSelected[b] = true;
        }
        ...
        public static ImSoundFont LoadMPTKSoundFont(string path, string name)
        {
            ...
            try
            {
                ...
                var sfxml = Resources.Load<TextAsset>(path + "/" + name);
                ...
                else
                {
                    if (sfxml != null && !string.IsNullOrEmpty(sfxml.text))
                    {
                        ...
                        else
                        {
                            ...
                            var sfbin = Resources.Load<TextAsset>(path + "/" + name + "_data");
                            if (sfbin == null)
                            {
                            ...
                            else
                            {
                                ...
                                var load = new SFLoad(sfbin.bytes, SFFile.SfSource.MPTK);
                                if (load == null || load.SfData == null)
                                {
                                    Debug.LogWarningFormat("Error when decoding SoundFont data {0} from {1}", name,
                                        path);
                                }
                                else
                                {
        ...
        public static void LoadBanks(ImSoundFont imsf)
        {
            ...
            for (var b = 0; b < imsf.BankSelected.Length; b++)
                imsf.BankSelected[b] = false;

            if (imsf.StrBankSelected != null)
            {
                var sbanks = imsf.StrBankSelected.Split(',');
                if (sbanks != null)
                    foreach (var sbank in sbanks)
                        if (!string.IsNullOrEmpty(sbank))
                        {
                            var ibank = Convert.ToInt32(sbank);
            ...
            foreach (var p in imsf.HiSf.preset)
                if (p != null)
                {
