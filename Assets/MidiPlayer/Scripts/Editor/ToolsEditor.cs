...
namespace MidiPlayerTK
{
    ...
    public class ToolsEditor
    {
        ...
        public static void CheckMidiSet()
        {
            ...
            try
            {
                var folder = Path.Combine(Application.dataPath + "/", MidiPlayerGlobal.PathToSoundfonts);
                //Debug.Log($"CheckMidiSet {MidiPlayerGlobal.PathToSoundfonts}");
                if (!Directory.Exists(folder))
                    Directory.CreateDirectory(folder);

                var tobesaved = false;
                var fileEntries = Directory.GetFiles(folder, "*" + MidiPlayerGlobal.ExtensionSoundFileDot,
                ...
                if (MidiPlayerGlobal.CurrentMidiSet != null && MidiPlayerGlobal.ImSFCurrent != null)
                    if (MidiPlayerGlobal.CurrentMidiSet.SoundFonts != null &&
                        MidiPlayerGlobal.CurrentMidiSet.SoundFonts.Find(s =>
                            s.Name == MidiPlayerGlobal.ImSFCurrent.SoundFontName) == null)
                    {
            ...
            try
            {
                var pathMidi = Path.Combine(Application.dataPath, MidiPlayerGlobal.PathToMidiFile);
                ...
                //
                foreach (var pathmidifile in midiFiles)
                {
                    var filename = Path.GetFileNameWithoutExtension(pathmidifile);
        ...
        public static void LoadMidiSet()
        {
            try
            {
                MidiPlayerGlobal.CurrentMidiSet =
                    MidiSet.Load(Application.dataPath + "/" + MidiPlayerGlobal.PathToMidiSet);
        ...
        public static string ReadTextFile(string filePath)
        {
            ...
            try
            {
                ...
                while (!stream.EndOfStream) content += stream.ReadLine() + "\n";
                stream.Close();
            }
            catch (Exception)
            {
        ...
        private static List<string> GetMidiFilePath()
        {
            ...
            try
            {
                var folder = Application.dataPath + "/" + MidiPlayerGlobal.PathToMidiFile;
                if (Directory.Exists(folder))
                    try
                    {
                        var fileEntries = Directory.GetFiles(folder, "*" + MidiPlayerGlobal.ExtensionMidiFile,
                            SearchOption.AllDirectories);
                        if (fileEntries.Length > 0) paths = new List<string>(fileEntries);
                    }
                    catch (Exception ex)
                    {
        ...
        private static void RenameExtFileFromMidToBytes()
        {
            try
            {
                ...
                if (Directory.Exists(folder))
                {
                    var fileEntries = Directory.GetFiles(folder, "*.mid", SearchOption.AllDirectories);
                    foreach (var filename in fileEntries)
                        try
                        {
                            var target = Path.ChangeExtension(filename, MidiPlayerGlobal.ExtensionMidiFile);
        ...
        public static void RenumMidiFile()
        {
            try
            {
                ...
                if (Directory.Exists(folder))
                {
                    var fileEntries = new List<string>(Directory.GetFiles(folder,
                        "*" + MidiPlayerGlobal.ExtensionMidiFile, SearchOption.AllDirectories));
                    fileEntries.Sort();
                    var index = 1;
                    foreach (var filename in fileEntries)
                        try
                        {
                            if (!Path.GetFileName(filename).StartsWith("MPTK_Example"))
                            {
                                ...
                                do
                                {
                                    target = Path.Combine(Path.GetDirectoryName(filename),
                                                 string.Format("MPTK_Example {0:0000}", index++)) +
                                             MidiPlayerGlobal.ExtensionMidiFile;
