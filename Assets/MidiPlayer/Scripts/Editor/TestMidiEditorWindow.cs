...
namespace MidiPlayerTK
{
    ...
    {
        private static TestMidiEditorWindow window;
        ...
        private void OnGUI()
        {
            try
            {
                ...
                if (GUILayout.Button("Stop", GUILayout.Width(100))) MidiPlayerEditor.MidiPlayer.MPTK_Stop();
        ...
        public static void InitSequencer()
        {
            ...
            try
            {
                ...
                {
                    window = CreateInstance(typeof(TestMidiEditorWindow)) as TestMidiEditorWindow;
                    window.Show();
            ...
            foreach (var midiEvent in midiEvents)
                switch (midiEvent.Command)
                {
                    ...
                    case MPTKCommand.NoteOn:
                        Debug.LogFormat(
                            $"NoteOn Channel:{midiEvent.Channel} {midiEvent.Value} Velocity:{midiEvent.Velocity} Duration:{midiEvent.Duration}");
                        break;
                    ...
                    case MPTKCommand.MetaEvent:
                        switch (midiEvent.Meta)
                        {
                            ...
                            case MPTKMeta.SequenceTrackName:
                                Debug.LogFormat(
                                    $"MetaEvent Channel:{midiEvent.Channel} Meta:{midiEvent.Meta} Info:{midiEvent.Info}");
                                break;
                        }
