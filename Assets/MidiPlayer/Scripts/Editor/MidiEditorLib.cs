...
namespace MidiPlayerTK
{
    public class MidiEditorLib
    {
        ...
        private readonly bool logSoundFontLoaded;
        public MidiFileEditorPlayer MidiPlayer;
        private readonly string nameComponent;
        ...
        private void LoadPlayer()
        {
            ...
            if (logDebug)
                Debug.Log(">>> Load Editor Player ..." + nameComponent + " Application.isPlaying:" +
                          Application.isPlaying);

            var oldGo = GameObject.Find(nameComponent);
            if (oldGo != null)
            {
                if (logDebug)
                    Debug.Log($"{(DateTime.Now - startLoad).TotalSeconds:F3} Delete previous " + nameComponent);
            ...
            var goMidiGlobal = GameObject.Find("MidiPlayerGlobal");
            if (goMidiGlobal == null)
            {
                ...
                var objectMidiGlobal = new GameObject();
            ...
            else
            {
                if (logDebug)
                {
                    ...
                    var parent = goMidiGlobal.transform.parent;
                    if (parent == null)
            ...
            if (logDebug)
                Debug.Log($"{(DateTime.Now - startLoad).TotalSeconds:F3} <<< Load Editor Player ..." + nameComponent);
        }
        ...
        private AudioClip Create()
        {
            ...
            var sampleChannel = 1;
            //float frequency = 440;
            var myClip = AudioClip.Create("blank", sampleCount, sampleChannel, sampleRate, false);
            var samples = new float[sampleCount * sampleChannel];
            for (var i = 0;
                 i < samples.Length;
                 ++i) samples[i] = 0f; // Mathf.Sin(2 * Mathf.PI * frequency * i / samplerate);
        ...
        public void DestroyMidiObject()
        {
            ...
            var instanceHome = GameObject.Find(Routine.InstanceName);
            //Debug.Log("instanceHome: " + instanceHome ?? "null");
            if (instanceHome != null)
            {
                if (Application.isPlaying)
                    Object.Destroy(instanceHome);
                else
                    Object.DestroyImmediate(instanceHome, true);
            }
