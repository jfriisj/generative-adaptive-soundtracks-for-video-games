...
namespace MidiPlayerTK
{
    ...
    {
        private const int MAX_EVENT_PAGE = 250;
        private const int WIDTH_BUTTON_PLAYER = 100;

        private const int HEIGHT_PLAYER_CMD = 32;

        //const int HEIGHT_SETTING_DISPLAY = 60;
        private const int HEIGHT_TITLE = 25;
        private const int AREA_SPACE = 4;
        private List<MPTKEvent> AllMidiEvents;
        private List<MPTKGui.StyleItem> ColumnEvents;
        private int CurrentPage;
        private int DisplayTime;
        private bool FollowEvent = true;
        private MPTKEvent LastEventPlayed;
        private List<List<MPTKEvent>> MidiEventsLoaded;

        private MPTKGui.PopupList PopupDisplayTime;
        private List<MPTKGui.StyleItem> PopupFiltersChannel;
        private List<MPTKGui.StyleItem> PopupFiltersCommand;
        private List<MPTKGui.StyleItem> PopupFiltersDisplay;
        private List<MPTKGui.StyleItem> PopupFiltersTrack;
        private Vector2 ScrollerMidiPlayer = Vector2.zero;
        ...
        private void InitGUI()
        {
            ...
            if (PopupFiltersTrack == null)
            {
                ...
                for (var track = 0; track < midiPlayerLib.MidiPlayer.MPTK_TrackCount; track++)
            ...
            if (PopupFiltersCommand == null)
                ...
                {
                    new("Note On", true, true),
                    new("Note Off", true, true),
                    new("Control Change", true, true),
                    new("Preset Change", true, true),
                    new("Meta", true, true),
                    new("Touch", true, true),
                    new("Others", true, true)
                };
            ...
            if (ColumnEvents == null)
                ColumnEvents = new List<MPTKGui.StyleItem>
                {
                    new()
                    {
                        Width = 60, Caption = "Index",
                        Tooltip =
                            "Unique index of the MIDI event. Some index are missing because by default, MPTK removes NotOff Events."
                    },
                    new()
                    {
                        Width = 70, Caption = "Tick",
                        Tooltip =
                            "Time in MIDI Tick (part of a Quarter) of the Event since the start of playing the midi file. This time is independent of the Tempo or Speed."
                    },
                    new()
                    {
                        Width = 70, Caption = "Time (s)", Hidden = true,
                        Tooltip =
                            "Real time in seconds of this event from the start of the midi depending the tempo change."
                    },
                    new()
                    {
                        Width = 120, Caption = "Time", Hidden = true,
                        Tooltip =
                            "Real time in seconds of this event from the start of the midi depending the tempo change."
                    },
                    new()
                    {
                        Width = 70, Caption = "Track{*}", Offset = 25, ItemPopupContent = PopupFiltersTrack,
                        ItemPopupWidth = 100,
                        Tooltip =
                            "Track index of the event in the midi. It's just a cool way to regroup MIDI events in a ... track. There is any impact on music played. Track 0 is the first track read from the midi file."
                    },
                    new()
                    {
                        Width = 70, Caption = "Channel{*}", Offset = 25, ItemPopupContent = PopupFiltersChannel,
                        ItemPopupWidth = 100,
                        Tooltip =
                            "MIDI channel fom 0 to 15 (9 for drum). Only one instrument can be selected at a time for a channel."
                    },
                    new()
                    {
                        Width = 200, Caption = "Command {Count}", Offset = 10, ItemPopupContent = PopupFiltersCommand,
                        Tooltip = "MIDI Command. Defined the MIDI action to be done by the synthesizer."
                    },
                    new()
                    {
                        Width = 180, Caption = "Value",
                        Tooltip =
                            "Contains a value in relation with the Command: Note pitch, instrument selected, control change value, text for Meta command. "
                    },
                    new() { Width = 70, Caption = "Velocity", Tooltip = "Velocity between 0 and 127." },
                    new() { Width = 50, Caption = "Duration", Tooltip = "Duration of the note in ticks." }, //9
                    new()
                    {
                        Width = 70, Caption = "Duration (s)", Hidden = true, Tooltip = "Duration of the note in second."
                    }, //10
                    new() { Width = 120, Caption = "Duration", Hidden = true, Tooltip = "Duration of the note." }, //11
                    new() { Width = 120, Caption = "Measure", Tooltip = "Measure/Beat" } //12
                };
        }
        ...
        {
            if (PopupFiltersChannel != null)
                foreach (var item in PopupFiltersChannel)
                    item.Selected = true;
        }
        ...
        private void LoadMidiFileSelected(int indexEditItem, bool forceChannelEnabled = false)
        {
            try
            {
                ...
                if (indexEditItem >= 0 && indexEditItem < MidiPlayerGlobal.CurrentMidiSet.MidiFiles.Count)
                {
                    ...
                    foreach (var midiEvent in AllMidiEvents)
                    {
            ...
            try
            {
                ...
                if (FollowEvent)
                {
                    ...
                    if (LastEventPlayed.Tick < MidiEventsLoaded[CurrentPage][0].Tick ||
                        LastEventPlayed.Tick >
                        MidiEventsLoaded[CurrentPage][MidiEventsLoaded[CurrentPage].Count - 1].Tick)
                        // Last pLayed events is on another page
                        for (var page = 0; page < MidiEventsLoaded.Count; page++)
                        {
                            var eventsPage = MidiEventsLoaded[page];
                            if (LastEventPlayed.Tick >= eventsPage[0].Tick &&
                                LastEventPlayed.Tick <= eventsPage[eventsPage.Count - 1].Tick)
                            {
        ...
        private void ShowMidiPlayerCommand()
        {
            try // Command ----- Second line -----
            {
                ...
                midiPlayerLib.MidiPlayer.MPTK_MidiAutoRestart = GUILayout.Toggle(
                    midiPlayerLib.MidiPlayer.MPTK_MidiAutoRestart, "Restart MIDI", MPTKGui.styleToggle,
                    GUILayout.Width(100)); // Vertical alignment with MIDI name

                var styleButtonPlayAndPause =
                    midiPlayerLib.MidiPlayer.MPTK_IsPlaying && !midiPlayerLib.MidiPlayer.MPTK_IsPaused
                        ? MPTKGui.ButtonHighLight
                        : MPTKGui.Button;
                if (GUILayout.Button("Play", styleButtonPlayAndPause, GUILayout.Width(WIDTH_BUTTON_PLAYER)))
                {
                    ...
                    {
                        EditorUtility.DisplayDialog("MIDI Player ",
                            "Can't play MIDI in non core mode. WebGL build enabled?", "Ok");
                    }
                    else
                    {
                ...
                styleButtonPlayAndPause =
                    midiPlayerLib.MidiPlayer.MPTK_IsPaused ? MPTKGui.ButtonHighLight : MPTKGui.Button;
            ...
            {
                GUILayout.BeginHorizontal();
                FollowEvent = GUILayout.Toggle(FollowEvent, "Follow", MPTKGui.styleToggle,
                    GUILayout.ExpandWidth(false) /*, GUILayout.Height(alignHorizontal)*/);

                if (DisplayTime == 0)
                {
                    var tickCurrent = midiPlayerLib.MidiPlayer.MPTK_IsPlaying
                        ? midiPlayerLib.MidiPlayer.MPTK_TickCurrent
                        : 0;
                    GUILayout.Label($"Tick: {tickCurrent:000000} / {midiPlayerLib.MidiPlayer.MPTK_TickLast:000000}",
                        MPTKGui.Label, GUILayout.Width(160));
                    var tick = (long)GUILayout.HorizontalSlider(tickCurrent, 0f, midiPlayerLib.MidiPlayer.MPTK_TickLast,
                        MPTKGui.HorizontalSlider, MPTKGui.HorizontalThumb, GUILayout.MinWidth(160));
                    if (tick != tickCurrent)
                ...
                {
                    var currentPosition = midiPlayerLib.MidiPlayer.MPTK_IsPlaying
                        ? Math.Round(midiPlayerLib.MidiPlayer.MPTK_Position / 1000d, 3)
                        : 0;
                    var lastPosition = midiPlayerLib.MidiPlayer.MPTK_PositionLastNote / 1000d;
                    if (DisplayTime == 1)
                    {
                        GUILayout.Label($"Time: {currentPosition:F2} / {lastPosition:F2} sec.", MPTKGui.Label,
                            GUILayout.Width(180));
                    }
                    else if (DisplayTime == 2)
                    {
                        var timePos = TimeSpan.FromSeconds(currentPosition);
                        var playTime = string.Format("{0:00}:{1:00}:{2:00}:{3:000}", timePos.Hours, timePos.Minutes,
                            timePos.Seconds, timePos.Milliseconds);
                        var lastPos = TimeSpan.FromSeconds(lastPosition);
                        var lastTime = string.Format("{0:00}:{1:00}:{2:00}:{3:000}", lastPos.Hours, lastPos.Minutes,
                            lastPos.Seconds, lastPos.Milliseconds);
                        GUILayout.Label($"Time: {playTime} / {lastTime}", MPTKGui.Label, GUILayout.Width(220));
                    }

                    var newPosition = Math.Round(GUILayout.HorizontalSlider((float)currentPosition, 0f,
                        (float)midiPlayerLib.MidiPlayer.MPTK_Duration.TotalSeconds /*, GUILayout.Width(150)*/,
                ...
                var volume = midiPlayerLib.MidiPlayer.MPTK_Volume;
                // Button to restore Volume to 0.5 with label style
                if (GUILayout.Button("Volume: " + volume.ToString("F2"), MPTKGui.Label, GUILayout.Width(80)))
                    volume = 0.5f;
                midiPlayerLib.MidiPlayer.MPTK_Volume = GUILayout.HorizontalSlider(volume, 0f, Constant.MAX_VOLUME,
                    MPTKGui.HorizontalSlider, MPTKGui.HorizontalThumb, GUILayout.Width(100));

                var speed = midiPlayerLib.MidiPlayer.MPTK_Speed;
                ...
                midiPlayerLib.MidiPlayer.MPTK_Speed = GUILayout.HorizontalSlider(speed, Constant.MIN_SPEED,
                    Constant.MAX_SPEED, MPTKGui.HorizontalSlider, MPTKGui.HorizontalThumb, GUILayout.Width(100));
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"{ex}");
            ...
            try
            {
                ...
                if (MidiEventsLoaded != null)
                {
                    ...
                    GUILayout.Label($"Page {CurrentPage} / {MidiEventsLoaded.Count - 1}", MPTKGui.styleLabelCenter,
                        GUILayout.Height(alignHorizontal));
        ...
        private void ShowMidiPlayerEvent(float startX, float width, float height, float nextAreaY)
        {
            try // Begin area title list
            {
                ...
                GUILayout.BeginHorizontal();
                var style = MPTKGui.LabelListNormal; // get a ref to the style
                style.contentOffset = new Vector2(-ScrollerMidiPlayer.x, style.contentOffset.y);
                foreach (var column in ColumnEvents)
                    //try
                    //{
                    if (!column.Hidden)
                        if (column.ItemPopupContent == null)
                            GUILayout.Label(new GUIContent(column.Caption, column.Tooltip), style,
                                    ...
                                    if (column.Caption.Contains("Channel"))
                                        for (var channel = 0; channel < 16; channel++)
                                            if (index == -2)
                                                ...
                                                midiPlayerLib.MidiPlayer.MPTK_Channels[channel].Enable =
                                                    column.ItemPopupContent[channel].Selected;
            ...
            if (MidiEventsLoaded != null)
                try // Begin area MIDI events list
                {
                    ...
                    var lineIndex = 0;
                    foreach (var mptkEvent in MidiEventsLoaded[CurrentPage])
                    {
                        // Filters MIDI events
                        switch (mptkEvent.Command)
                        {
                            case MPTKCommand.NoteOn:
                                if (!PopupFiltersCommand[0].Selected) continue;
                                break;
                            case MPTKCommand.NoteOff:
                                if (!PopupFiltersCommand[1].Selected) continue;
                                break;
                            case MPTKCommand.ControlChange:
                                if (!PopupFiltersCommand[2].Selected) continue;
                                break;
                            case MPTKCommand.PatchChange:
                                if (!PopupFiltersCommand[3].Selected) continue;
                                break;
                            case MPTKCommand.MetaEvent:
                                if (!PopupFiltersCommand[4].Selected) continue;
                                break;
                            case MPTKCommand.ChannelAfterTouch:
                            case MPTKCommand.KeyAfterTouch:
                                if (!PopupFiltersCommand[5].Selected) continue;
                                break;
                            default:
                                if (!PopupFiltersCommand[6].Selected) continue;
                                break;
                        }
                        ...
                        try
                        {
                            ...
                            var displayMoreColumns = false;
                            // Command column
                            switch (mptkEvent.Command)
                            {
                                case MPTKCommand.ControlChange:
                                    text = "CC - " + mptkEvent.Controller;
                                    break;
                                case MPTKCommand.MetaEvent:
                                    text = "Meta - " + mptkEvent.Meta;
                                    break;
                                default:
                            ...
                            switch (mptkEvent.Command)
                            {
                                case MPTKCommand.NoteOn:
                                    ...
                                    text = mptkEvent.Value.ToString("000") + " - " +
                                           HelperNoteLabel.LabelFromMidi(mptkEvent.Value);
                                    break;
                                case MPTKCommand.NoteOff:
                                    text = mptkEvent.Value.ToString("000") + " - " +
                                           HelperNoteLabel.LabelFromMidi(mptkEvent.Value);
                                ...
                                case MPTKCommand.MetaEvent:
                                    switch (mptkEvent.Meta)
                                    {
                                        case MPTKMeta.KeySignature:
                                            text =
                                                $"SharpsFlats:{MPTKEvent.ExtractFromInt((uint)mptkEvent.Value, 0)} MajorMinor:{MPTKEvent.ExtractFromInt((uint)mptkEvent.Value, 1)}";
                                            break;
                                        case MPTKMeta.TimeSignature:
                                            text =
                                                $"Numerator:{MPTKEvent.ExtractFromInt((uint)mptkEvent.Value, 0)} Denominator:{MPTKEvent.ExtractFromInt((uint)mptkEvent.Value, 1)}";
                                            break;
                                        case MPTKMeta.SetTempo:
                                            text =
                                                $"�seconds:{mptkEvent.Value} Tempo:{MPTKEvent.QuarterPerMicroSecond2BeatPerMinute(mptkEvent.Value):F0}";
                                            break;
                            ...
                            AddCell(mptkEvent, ColumnEvents[column++],
                                displayMoreColumns ? mptkEvent.Velocity.ToString() : "", styleRow);
                            AddCell(mptkEvent, ColumnEvents[column++],
                                displayMoreColumns ? mptkEvent.Length.ToString() : "", styleRow);
                            AddCell(mptkEvent, ColumnEvents[column++], displayMoreColumns ? mptkEvent.Duration : -1,
                                styleRow);
                            AddCell(mptkEvent, ColumnEvents[column++], displayMoreColumns ? mptkEvent.Duration : -1,
                                styleRow); // 11 time hh:mm:ss

                            AddCell(mptkEvent, ColumnEvents[column++], $"{mptkEvent.Measure}/{mptkEvent.Beat}",
                                styleRow);

                            // make events played visible in the schroll
                            lineIndex++;

                            if (Event.current.type == EventType.Repaint && !midiPlayerLib.MidiPlayer.MPTK_IsPaused &&
                                midiPlayerLib.MidiPlayer.MPTK_IsPlaying)
                            {
                                var lastEventDraw = GUILayoutUtility.GetLastRect();
        ...
        private void AddCell(MPTKEvent midiEvent, MPTKGui.StyleItem item, float timeMs, GUIStyle styleRow)
        {
            if (!item.Hidden)
            {
                ...
                else if (timeMs > 0)
                {
                    ...
                    {
                        var timePos = TimeSpan.FromMilliseconds(timeMs);
                        text = string.Format("{0:00}:{1:00}:{2:00}:{3:000}", timePos.Hours, timePos.Minutes,
                            timePos.Seconds, timePos.Milliseconds);
                    }
                }
        ...
        private void AddCell(MPTKEvent midiEvent, MPTKGui.StyleItem item, string text, GUIStyle styleRow = null)
        {
            if (!item.Hidden)
            {
                var style = styleRow == null ? item.Style : styleRow;
                // Align content of the column 
                if (item.Offset != 0)
                    style.contentOffset = item.OffsetV; /* Debug.Log($"{ item.Caption} {item.Offset}");*/
                ;
