...
namespace MidiPlayerTK
{
    ...
    {
        private static List<string> infoStats = new();
        ...
        private static void CalculateStat()
        {
            ...
            {
                var pathMidiFile = MidiPlayerGlobal.CurrentMidiSet.MidiFiles[IndexEditItem];
                ...
                if (midiLoad != null)
                {
                    infoStats.Add($"MIDI File Format:         {midiLoad.midifile.FileFormat}" +
                                  (midiLoad.midifile.FileFormat == 0 ? " (converted to format 1)" : ""));
                    ...
                    infoStats.Add(
                        $"MPTK_Duration:            {midiLoad.MPTK_Duration.TotalMilliseconds} ms ({midiLoad.MPTK_Duration})");
                    ...
                    infoStats.Add(
                        $"MPTK_NumberQuarterBeat:   {midiLoad.MPTK_NumberQuarterBeat} (2 pow timesig.Denominator)");
                    ...
                    infoStats.Add(
                        $"MPTK_MicrosecondsPerQuarterNote: {midiLoad.MPTK_MicrosecondsPerQuarterNote} �seconds per quarter");
                    infoStats.Add($"MPTK_No32ndNotesInQuarterNote:   {midiLoad.MPTK_No32ndNotesInQuarterNote}");
                    infoStats.Add(
                        $"MPTK_KeySigSharpsFlats:          {midiLoad.MPTK_KeySigSharpsFlats}  number of flats (if negative) or sharps (if positive).");
                    infoStats.Add("MPTK_KeySigMajorMinor:           " +
                                  (midiLoad.MPTK_KeySigMajorMinor == 0 ? "Major" : "Minor"));
                    infoStats.Add("");

                    infoStats.Add("Tempo Change");
                    foreach (var tempo in midiLoad.MPTK_TempoMap)
                        infoStats.Add("  " + tempo);
                    infoStats.Add("");

                    infoStats.Add("Signature Change");
                    foreach (var sign in midiLoad.MPTK_SignMap)
                        infoStats.Add("  " + sign);
                    infoStats.Add("");
                    ...
                    var stat_note = new int[16, 128];
                    var stat_channel = new int[16];

                    try
                    {
                        // Calculate notes count by channel
                        foreach (var mptkEvent in midiLoad.MPTK_MidiEvents)
