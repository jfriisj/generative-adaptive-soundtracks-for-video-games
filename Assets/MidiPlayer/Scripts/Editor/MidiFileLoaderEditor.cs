    ...
    public class MidiFileLoaderEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            try
            {
                ...
                if (commonEditor == null) commonEditor = CreateInstance<MidiCommonEditor>();
                ...
                {
                    commonEditor.DrawCaption(null, "MIDI File Loader - Load a MIDI and read the MIDI message.",
                        "https://paxstellar.fr/prefab-midifileloader/",
                        "df/d2e/class_midi_player_t_k_1_1_midi_file_loader.html#details");
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField(new GUIContent("Select MIDI ", "Select MIDI File to load"),
                        GUILayout.Width(150));

                    if (GUILayout.Button(
                            new GUIContent(instance.MPTK_MidiIndex + " - " + instance.MPTK_MidiName,
                                "Selected MIDI File to load"), GUILayout.Height(30)))
                    ...
                    instance.MPTK_LogLoadEvents = EditorGUILayout.Toggle(
                        new GUIContent("Log MIDI Events", "Log information about each MIDI events read."),
                        instance.MPTK_LogLoadEvents);
                    if (instance.MPTK_LogLoadEvents)
                        EditorGUILayout.LabelField(
                            "Setting 'Log MIDI Events' to true could increase greatly the load time. To be used only for debug.",
                            MPTKGui.styleAlertRed);
