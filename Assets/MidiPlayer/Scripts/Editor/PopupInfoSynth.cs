    ...
    public class PopupInfoSynth : EditorWindow
    {
        ...
        private void OnGUI()
        {
            try
            {
                ...
                {
                    var SepBorder1 = new RectOffset(1, 1, 1, 1);
                    TextFieldMultiCourier = MPTKGui.BuildStyle(MPTKGui.TextField, 14, textAnchor: TextAnchor.UpperLeft);
                    ...
                    MPTKGui.ColorStyle
                    (
                        TextFieldMultiCourier,
                        Color.green,
                        MPTKGui.MakeTex(10, 10, new Color(0.2f, 0.2f, 0.2f, 1f), SepBorder1,
                ...
                */
                ...
                // -----------------------
                var infoSynth = MidiSynth.synthInfo.MPTK_BuildInfoSynth(MidiSynth);

                //string info;
                GUILayout.Label(infoSynth.ToString(), TextFieldMultiCourier);

                if (GUI.Button(new Rect(position.width - btResetStatSize.x - 15f, 20f, 80f, 20f), btResetStatContent,
                        MPTKGui.Button)) MidiSynth.MPTK_ResetStat();
            }
            catch (Exception ex)
            {
