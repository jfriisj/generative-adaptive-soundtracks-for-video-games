...
namespace MidiPlayerTK
{
    ...
    {
        public static SelectMidiWindow winSelectMidi;
        ...
        private List<MPTKListItem> filteredList;
        public List<MPTKListItem> midiList;
        public Action<object, int> OnSelect;
        private Rect rectClear;
        private int selectedInFilterList;
        public object Tag;

        private void OnDisable()
        {
            //Debug.Log($"OnDisable SelectMidiWindow {this.GetInstanceID()} {this.}");
            ...
            try
            {
                ...
                try
                {
                    if (MidiPlayerGlobal.CurrentMidiSet != null && MidiPlayerGlobal.CurrentMidiSet.MidiFiles != null)
                        foreach (var midiname in MidiPlayerGlobal.CurrentMidiSet.MidiFiles)
                            midiList.Add(new MPTKListItem
                                { Label = midiList.Count + " - " + midiname, Index = midiList.Count });
                }
                catch (Exception ex)
                {
                    Debug.Log(ex);
                ...
                var newFilter = GUILayout.TextField(midiFilter, MPTKGui.TextField, GUILayout.MinWidth(200),
                ...
                if (midiList != null && midiList.Count > 0 && SelectedIndexMidi >= 0 &&
                    SelectedIndexMidi < midiList.Count)
                {
                    var midiName = midiList[SelectedIndexMidi].Label;
                    if (GUILayout.Button("Reload " + midiName, MPTKGui.Button, GUILayout.ExpandWidth(true),
                            GUILayout.Height(30)))
                    {
                ...
                try
                {
                    ...
                    {
                        if (item != null && (string.IsNullOrEmpty(midiFilter) ||
                                             item.Label.ToUpper().Contains(midiFilter.ToUpper())))
                        {
                    ...
                    {
                        var listLabel = new GUIContent[filteredList.Count];
                        var i = 0;
                        var maxLen = 10f;
                        ...
                        var colCount = Mathf.Clamp(Convert.ToInt32(position.size.x / maxLen + 0.5f), 1, 15) - 1;
                        if (colCount <= 0) colCount = 1;
                        var selected = GUILayout.SelectionGrid(-1, listLabel, colCount /*, MPTKGui.Button*/);
