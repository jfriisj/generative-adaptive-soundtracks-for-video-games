...
namespace MPTK.NAudio.Midi
{
    ...
    public class MidiEvent : ICloneable
    {
        ...
        protected bool extendedText = false; // TBN
        ...
        public static MidiEvent ReadNextEvent(BinaryReader br, MidiEvent previous, bool extendedText = false) // TBN
        {
            ...
            switch (commandCode)
            {
                ...
                case MidiCommandCode.Sysex:
                    ...
                    break;
                case MidiCommandCode.MetaEvent:
                    me = MetaEvent.ReadMetaEvent(br, extendedText);
                    break;
                default:
        ...
        public static bool IsEndTrack(MidiEvent midiEvent)
        {
            if (midiEvent != null)
            {
                ...
                if (me != null) return me.MetaEventType == MetaEventType.EndTrack;
            }
        ...
        {
            if (value < 0) throw new ArgumentOutOfRangeException("value", value, "Cannot write a negative Var Int");
            if (value > 0x0FFFFFFF)
        ...
        {
            if (this.AbsoluteTime < absoluteTime) throw new FormatException("Can't export unsorted MIDI events");
            WriteVarInt(writer, (int)(this.AbsoluteTime - absoluteTime));
            absoluteTime = this.AbsoluteTime;
            var output = (int)commandCode;
            if (commandCode != MidiCommandCode.MetaEvent) output += channel - 1;
