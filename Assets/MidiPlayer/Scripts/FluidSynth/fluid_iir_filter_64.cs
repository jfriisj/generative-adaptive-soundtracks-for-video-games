...
namespace MidiPlayerTK
{
    ...
    {
        private double a1; /* a0 / a0 */
        private double a1_incr;
        private double a2; /* a1 / a0 */
        private double a2_incr;

        /* filter coefficients */
        /* The coefficients are normalized to a0. */
        /* b0 and b2 are identical => b02 */
        private double b02; /* b0 / a0 */

        private double b02_incr;
        private double b1; /* b1 / a0 */
        private double b1_incr;
        private bool compensate_incr; /* Flag: If set, must compensate history */
        private int filter_coeff_incr_count;
        private double filter_gain; /* Gain correction factor, depends on q */
        ...
        private double fres; /* the resonance frequency, in cents (not absolute cents) */
        private double hist1, hist2; /* Sample history for the IIR filter */

        private double last_fres; /* Current resonance frequency of the IIR filter */

        /* Serves as a flag: A deviation between fres and last_fres */
        /* indicates, that the filter has to be recalculated. */
        private double q_lin; /* the q-factor on a linear scale */
                ...
                if (dsp_filter_coeff_incr_count > 0)
                {
                    ...
                    for (dsp_i = 0; dsp_i < count; dsp_i++)
                    {
                        ...
                        if (dsp_filter_coeff_incr_count-- > 0)
                        {
                            ...
                            if (compensate_incr && Math.Abs(dsp_b02) > 0.001d)
                            {
        ...
        private static double fluid_iir_filter_q_from_dB(double q_dB, double offset /*MPTK specific*/)
        {
            ...
            q_dB = Math.Clamp(q_dB + offset, 0d,
                96d); // replaced by the following line to be compliant with Unity 2019.4
        ...
        private void fluid_iir_filter_calculate_coefficients(int transition_samples, float output_rate)
        {
            ...
            else
            {
                ...
                var omega = 2d * Math.PI * (last_fres / output_rate);
                var sin_coeff = Math.Sin(omega);
                var cos_coeff = Math.Cos(omega);
                var alpha_coeff = sin_coeff / (2d * q_lin);
                var a0_inv = 1d / (1d + alpha_coeff);
                ...
                var a1_temp = -2d * cos_coeff * a0_inv;
                var a2_temp = (1d - alpha_coeff) * a0_inv;
        ...
        public override void fluid_iir_filter_calc(float output_rate, float fres_mod, float offset /*MPTK specific*/)
        {
            ...
            else if (fres < 5d) fres = 5d;

            /* if filter enabled and there is a significant frequency change.. */
            if (type != fluid_iir_filter_type.FLUID_IIR_DISABLED && Math.Abs(fres - last_fres) > 0.01d)
            {
