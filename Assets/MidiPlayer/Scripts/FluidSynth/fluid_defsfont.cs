...
namespace MidiPlayerTK
{
    ...
    public partial class MidiSynth : MonoBehaviour
    {
        ...
        public void fluid_defpreset_noteon(MPTKEvent note)
        {
            ...
            {
                if (MPTK_LogWave)
                    Debug.LogWarningFormat("No preset associated to this channel {0}, set first preset, note: {1}",
                ...
                if (defpreset == null)
                {
                    Debug.LogWarningFormat("No preset associated to this channel {0}, cancel playing note: {1}",
                        note.Channel, note.Value);
                    return;
                }
            }
            ...
            var soundFont = MPTK_SoundFont.SoundFont;
            note.Voices = new List<fluid_voice>();

            var global_preset_zone = defpreset.GlobalZone;

            // run thru all the zones of this preset 
            foreach (var preset_zone in defpreset.Zone)
                ...
                if (preset_zone.KeyLo <= note.Value &&
                    ...
                    {
                        var inst = soundFont.HiSf.inst[preset_zone.Index];
                        var global_inst_zone = inst.GlobalZone;
                        ...
                        if (global_preset_zone != null && global_preset_zone.genE == null)
                        {
                            ...
                            foreach (var gen in global_preset_zone.gens)
                        ...
                        foreach (var voice_zone in inst.Zone)
                        {
                                ...
                                voice_zone.VelHi >= note.Velocity)
                            {
                                ...
                                if (midiLoaded != null)
                                    voice.DurationTick = note.Duration >= 0
                                        ? (long)(note.Duration * fluid_voice.Nano100ToMilli / midiLoaded.Speed)
                                        : -1;
                                else
                                    // No midi loaded. Synth used as a realtime player without MIDI loaded
                                    voice.DurationTick = note.Duration >= 0
                                        ? (long)(double)(note.Duration * fluid_voice.Nano100ToMilli)
                                        : -1;
                                ...
                                //if (!voice.Reused) // For v2.12.2 try to optimize
                                {
                                    ...
                                    if (inst_zone.genE == null)
                                    {
                                        ...
                                        foreach (var gen in inst_zone.gens)
                                            inst_zone.genE[(byte)gen.type] = gen;
                                    }

                                    for (var i = 0; i < (int)fluid_gen_type.GEN_LAST; i++)
                                    {
                                        var inst_zone_genE = inst_zone.genE[i];
                                        ...
                                        //if (i == 52)
                                        //    Debug.Log("FINETUNE");
                                        if (inst_zone_genE != null &&
                                            inst_zone_genE.flags != fluid_gen_flags.GEN_UNUSED)
                                        {
                                        ...
                                        else if (global_inst_zone != null && global_inst_zone.genE[i] != null &&
                                                 global_inst_zone.genE[i].flags != fluid_gen_flags.GEN_UNUSED)
                                        {
                                    ...
                                    if (preset_zone.genE == null)
                                    {
                                        ...
                                        foreach (var gen in preset_zone.gens)
                                    ...
                                    for (var i = 0; i < (int)fluid_gen_type.GEN_LAST; i++)
                                    {
                                        ...
                                        */
                                        ...
                                        var preset_zone_genE = preset_zone.genE[i];

                                        if (preset_zone_genE != null &&
                                            preset_zone_genE.flags != fluid_gen_flags.GEN_UNUSED)
                                        {
                                            //fluid_voice_gen_incr(voice, i, inst_zone->gen[i].val);
                                            voice.gen[i].Val += preset_zone_genE.Val;
                                            voice.gen[i].flags = fluid_gen_flags.GEN_SET;
                                        }
                                        else if (global_preset_zone != null && global_preset_zone.genE[i] != null &&
                                                 global_preset_zone.genE[i].flags != fluid_gen_flags.GEN_UNUSED)
                                        {
                                ...
                                {
                                    sLogSampleUse.Clear();
                                    sLogSampleUse.Append(
                                        $"Channel:{note.Channel:00} Bank:{Channels[note.Channel].BankNum:000} Preset:{Channels[note.Channel].PresetNum:000} ");
                                    sLogSampleUse.Append(
                                        $"{defpreset.Name,-21} Key:{note.Value,-3}({HelperNoteLabel.LabelFromMidi(note.Value),-3}) ");
                                    sLogSampleUse.Append(
                                        $"Transp.:{(note.OriginalValue == -1 ? 0 : note.Value - note.OriginalValue),-3} ");
                                    sLogSampleUse.Append($"Vel.:{note.Velocity,-3} ");
                                    sLogSampleUse.Append(note.Duration >= 0
                                        ? $"Duration:{note.Duration,6} ms {voice.DurationTick,9} ticks "
                                        : "Inf.  ");
                                    sLogSampleUse.Append(
                                        $"Instr:{inst.Name,-21} Sample:{soundFont.HiSf.Samples[voice_zone.Index].Name,-21} ");
                                    sLogSampleUse.Append(
                                        $"Atten:{fluid_conv.fluid_cb2amp(voice.attenuation):F2} Pan:{voice.pan:F2}");
                                    //,Channels[note.Channel].cc[(int)MPTKController.VOLUME_MSB]  {12}
                                    Debug.Log(sLogSampleUse);
                                }

                                if (VerboseGenerator)
                                    foreach (var gen in voice.gen)
                                        if (gen != null && gen.flags > 0)
                                            Debug.LogFormat(
                                                "Gen Id:{1,-50}\t{0}\tValue:{2:0.00}\tMod:{3:0.00}\tflags:{4,-50}",
                                                (int)gen.type, gen.type, gen.Val, gen.Mod, gen.flags);
            ...
            if (MPTK_LogWave && note.Voices.Count == 0)
                Debug.LogFormat(
                    "NoteOn [{0:00} {1:000} {2:000}]\t{3,-21}\tKey:{4,-3}\tVel:{5,-3}\tDuration:{6:0.000}\tInstr:{7,-21}",
                    note.Channel, Channels[note.Channel].BankNum, Channels[note.Channel].PresetNum, defpreset.Name,
                    note.Value, note.Velocity, note.Duration, "*** no wave found ***");
        }
         ...
         */
        private void fluid_defpreset_noteon_add_mod_to_voice(fluid_voice voice,
            ...
            fluid_voice_addorover_mod mode)
        {
            ...
            if (local_mod != null)
                foreach (var l_mod in local_mod)
