...
namespace MidiPlayerTK
{
    public class fluid_revmodel
    {
...
#else
        private const int NBR_DELAYS = 8; /* default or 12*/
#endif
        ...
        */
        ...
        private const float FIXED_GAIN = 0.1f; /* input gain */

        /* SCALE_WET is adjusted to 5.0 to get internal output level equivalent to freeverb */
        private const float SCALE_WET = 5.0f; /* scale output gain */
        ...
        */
        ...
        private const float MIN_DC_REV_TIME = 0.7f; /* minimum T60DC reverb time: seconds */
        private const float MAX_DC_REV_TIME = 12.5f; /* maximumm T60DC time in seconds */
        private const float RANGE_REV_TIME = MAX_DC_REV_TIME - MIN_DC_REV_TIME;
        ...
        */
        ...
        private const float MOD_PHASE = 360f / NBR_DELAYS;

        private const float FLUID_M_LN10 = 2.3025850929940456840179914546844f;
        ...
        */
        private const float FDN_MATRIX_FACTOR = -2f / NBR_DELAYS;
        ...
        public class delay_line
        {
            ...
            public void clear_delay_line()
            {
                ...
                for (i = 0; i < size; i++) line[i] = DC_OFFSET;
            }
        }
        ...
        public class sinus_modulator
        {
            ...
            {
                var w = 2f * Mathf.PI * freq / sample_rate; /* initial angle */
                float a;
        ...
        public class mod_delay_line
        {
            ...
            private float center_pos_mod; /* center output position modulated by modulator */
            ...
            private float frac_pos_mod; /* fractional position part between samples) */

            /*-------------------------*/
            /* variable rate control of center output position */
            private int index_rate; /* index rate to know when to update center_pos_mod */

            /*---------------------------*/
            /* Sinusoidal modulator member */
            public sinus_modulator mod; /* sinus modulator */
            private int mod_depth; /* modulation depth (in samples) */
            private int mod_rate; /* rate at which center_pos_mod is updated */
            ...
            public bool set_mod_delay_line(int delay_length, int pmod_depth, int pmod_rate)
            {
                /*-----------------------------------------------------------------------*/
                /* checks parameter */
                if (delay_length < 1) return false;
            ...
            public float get_mod_delay()
            {
                ...
                if (++index_rate >= mod_rate)
                {
                    ...
                    if (out_index >= 0f)
                    {
                        ...
                        if ((dl.line_out = int_out_index) >= dl.size) dl.line_out -= dl.size;
                    }
                    else /* negative */
                    {
                    ...
                    if ((center_pos_mod += mod_rate) >= dl.size) center_pos_mod -= dl.size;
                }
                ...
                /* updates line_out to the next sample.
                   Boundary check and circular motion as needed */
                if (++dl.line_out >= dl.size) dl.line_out -= dl.size;
...
-----------------------------------------------------------------------------*/
        ...
        /* reverb parameters */
        private float roomsize; /* acting on reverb time */
        private float damp; /* acting on frequency dependent reverb time */
        private float level, wet1, wet2; /* output level */
        private float width; /* width stereo separation */
        ...
        public class fluid_late
        {
            ...
            {
                int i;
                var sample_period = 1 / samplerate; /* Sampling period */
            ...
            public bool create_mod_delay_lines(float sample_rate)
            {
...
#else
                delay_length = new[] { 601, 691, 773, 839, 919, 997, 1061, /* v1.9 1093*/ 1129 };
#endif
                int i;
                ...
                */
                var length_factor = 2f;
                float mod_depth = MOD_DEPTH;
                if (sample_rate > 44100f)
                {
                    var sample_rate_factor = sample_rate / 44100f;
                ...
                for (i = 0; i < NBR_DELAYS; i++) /* for each delay line */
                {
                    ...
                    if (!mod_delay_lines[i]
                            .set_mod_delay_line((int)(delay_length[i] * length_factor), (int)mod_depth, MOD_RATE))
                        return false;

                    /* Sets local Modulators parameters: frequency and phase
                     Each modulateur are shifted of MOD_PHASE degree
                    */
                    mod_delay_lines[i].mod.set_mod_frequency(MOD_FREQ * MOD_RATE, samplerate, MOD_PHASE * i);
                }
            ...
            public bool create_fluid_rev_late(float psample_rate)
            {
                ...
                */
                ...
                if (!create_mod_delay_lines(psample_rate)) return false;
        ...
        */
        private void fluid_revmodel_init()
        {
            ...
            for (i = 0; i < NBR_DELAYS; i++) late.mod_delay_lines[i].dl.clear_delay_line();
        }
        ...
        */
        private void fluid_revmodel_update()
        {
            ...
            var wet = level * SCALE_WET / (1f + width * SCALE_WET_WIDTH);
        ...
        {
            /*-----------------------------------*/
            if ((set & (int)fluid_revmodel_set_t.FLUID_REVMODEL_SET_ROOMSIZE) != 0) Roomsize = proomsize;

            /*-----------------------------------*/
            if ((set & (int)fluid_revmodel_set_t.FLUID_REVMODEL_SET_DAMPING) != 0) Damp = pdamping;

            /*-----------------------------------*/
            if ((set & (int)fluid_revmodel_set_t.FLUID_REVMODEL_SET_WIDTH) != 0) width = pwidth;

            /*-----------------------------------*/
            if ((set & (int)fluid_revmodel_set_t.FLUID_REVMODEL_SET_LEVEL) != 0) Level = plevel;
        ...
        private bool fluid_revmodel_samplerate_change(float psample_rate)
        {
            ...
            if (!late.create_mod_delay_lines(psample_rate)) return false; /* memory error */
        ...
        private readonly float[] delay_out = new float[NBR_DELAYS]; /* Line output + damper output */
        ...
        public void fluid_revmodel_processmix(float[] inp, float[] left_out, float[] right_out)
        {
            ...
            for (k = 0; k < FLUID_BUFSIZE; k++)
            {
                ...
                */
                ...
                for (i = 0; i < NBR_DELAYS; i++)
                {
                    var mdl = late.mod_delay_lines[i];
                    /* get current modulated output */
                ...
                */
                ...
                {
                    /* delay_in[i-1] = delay_out[i] + matrix_factor */
                    var dl = late.mod_delay_lines[i - 1].dl;
                ...
                /* delay_in[0] = delay_out[NB_DELAY -1] + matrix_factor */
                {
                    var dl = late.mod_delay_lines[NBR_DELAYS - 1].dl;
