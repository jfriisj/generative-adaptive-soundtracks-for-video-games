...
namespace MidiPlayerTK
{
    ...
    public class fluid_voice
    {
        ...
        private fluid_interp interp_method; // MPTK used at synth level
        public fluid_loop samplemode; // Set at SF instrument level. 0: no loop, 1: loop when released

        public byte
            check_sample_sanity_flag; /* Flag that initiates, that sample-related parameters have to be checked. */

        // To be closer FS
        private const byte FLUID_SAMPLESANITY_CHECK = 1 << 0;
        private const byte FLUID_SAMPLESANITY_STARTUP = 1 << 1;
        ...
        private float
            prev_attenuation; /* the previous attenuation in centibels used by fluid_rvoice_multi_retrigger_attack() */

        public float
            min_attenuation_cB; /* Estimate on the smallest possible attenuation during the lifetime of the voice */
        ...
        * the current sample), the lower 32 bits the fractional part.
        ...
        private float reverb_send;

        /* chorus */
        //public float chorus_send;
        private float chorus_send;
        ...
        private static StringBuilder logSynthInfo;
        ...
        public override string ToString()
        {
            try
            {
                ...
                logSynthInfo.Append(
                    $"ID:[{MptkEvent.IdSession} {MptkEvent.Index}] TCV:[{MptkEvent.Track:00} {MptkEvent.Channel:00} {MptkEvent.Value:000}] TT:[{MptkEvent.RealTime / 1000f:F3} {MptkEvent.Tick,-7:0000000}] ");
                logSynthInfo.Append(
                    $"MB[{MptkEvent.Measure}/{MptkEvent.Beat}] DD:[{MptkEvent.Duration / 1000f:F2} {MptkEvent.Length}] VS:[{volenv_val:F2} {volenv_section}] ");
                logSynthInfo.AppendLine(
                    $"PR:['{synth.Channels[MptkEvent.Channel].HiPreset.Num} {synth.Channels[MptkEvent.Channel].HiPreset.Name}' '{sample.Name}']");
            }
            catch (Exception ex)
            {
                Debug.LogException(ex);
                logSynthInfo.Append(ex);
            }

            return logSynthInfo.ToString();
        }

        private static readonly int[] list_of_generators_to_initialize =
        {
            (int)fluid_gen_type
                .GEN_STARTADDROFS, /* SF2.01 page 48 #0  - Unity load wave from wave file, no real time change possible on wave attribute */
            (int)fluid_gen_type
                .GEN_ENDADDROFS, /*                #1  - Unity load wave from wave file, no real time change possible on wave attribute */
            (int)fluid_gen_type
                .GEN_STARTLOOPADDROFS, /*                #2  - Unity load wave from wave file, no real time change possible on wave attribute */
            (int)fluid_gen_type
                .GEN_ENDLOOPADDROFS, /*                #3  - Unity load wave from wave file, no real time change possible on wave attribute */
        ...
        private static readonly int[] list_of_non_core_generators_to_initialize =
        {
            (int)fluid_gen_type
                .GEN_STARTADDROFS, /* SF2.01 page 48 #0  - Unity load wave from wave file, no real time change possible on wave attribute */
            (int)fluid_gen_type
        ...
        private static readonly int[] list_of_weakgenerators_to_initialize =
        {
            (int)fluid_gen_type
                .GEN_STARTADDROFS, /* SF2.01 page 48 #0  - Unity load wave from wave file, no real time change possible on wave attribute */
            (int)fluid_gen_type
        ...
        {
            return ticks / Nano100ToMilli;
        }

        public static float TicksToMilliF(long ticks)
        {
            return ticks / (float)Nano100ToMilli;
        }
        ...
        public fluid_voice(MidiSynth psynth)
        {
            ...
            volenv_data[(int)fluid_voice_envelope_index.FLUID_VOICE_ENVSUSTAIN].count =
                0xffffffff; // infiny until note off or duration is over
            volenv_data[(int)fluid_voice_envelope_index.FLUID_VOICE_ENVSUSTAIN].coeff = 1;
            volenv_data[(int)fluid_voice_envelope_index.FLUID_VOICE_ENVSUSTAIN].incr =
                0; // Volume remmains constant during sustain phase
            volenv_data[(int)fluid_voice_envelope_index.FLUID_VOICE_ENVSUSTAIN].min =
                -1; // not used for sustain (constant volume)
            volenv_data[(int)fluid_voice_envelope_index.FLUID_VOICE_ENVSUSTAIN].max =
                2; //1;     // not used for sustain (constant volume)
            ...
            modenv_data[(int)fluid_voice_envelope_index.FLUID_VOICE_ENVSUSTAIN].max =
                2; //1; fluidsythn original value=2
            ...
            resonant_filter.fluid_iir_filter_init(fluid_iir_filter_type.FLUID_IIR_LOWPASS,
                fluid_iir_filter_flags.FLUID_IIR_NOFLAGS);
...

        {
            ...
            has_looped =
                false; // from voice->dsp.has_looped - Will be set during voice_write when the 2nd loop point is reached 
        ...
        public void fluid_voice_add_mod_local(HiMod mod, fluid_voice_addorover_mod mode, int check_limit_count)
        {
            ...
            if (check_limit_count > mods.Count) check_limit_count = mods.Count;
        ...
        public void fluid_voice_start()
        {
            ...
            if (!weakDevice)
            {
                if (synth.VerboseEnvVolume)
                    for (var i = 0; i < (int)fluid_voice_envelope_index.FLUID_VOICE_ENVLAST; i++)
                        Debug.LogFormat("Volume Env. {0} {1,24} {2}", i, (fluid_voice_envelope_index)i, volenv_data[i]);

                if (synth.VerboseEnvModulation)
                    for (var i = 0; i < (int)fluid_voice_envelope_index.FLUID_VOICE_ENVLAST; i++)
                        Debug.LogFormat("Modulation Env. {0} {1,24} {2}", i, (fluid_voice_envelope_index)i,
                            modenv_data[i]);
            ...
            StartVolume = synth.MPTK_Volume * (float)(fluid_conv.fluid_cb2amp(attenuation) *
                                                      fluid_conv.fluid_cb2amp(960.0f * (1f - volenv_val)));
            ...
            // A single tick represents one hundred nanoseconds or one ten-millionth of a second.
            // There are 10,000 ticks in a millisecond, or 10 million ticks in a second. 
            TimeAtStart = MptkEvent.Delay * Nano100ToMilli + DateTime.UtcNow.Ticks;
        ...
        /// <summary>
        ///     @brief
        ///     in this function we calculate the values of all the parameters. the parameters are converted to their most useful
        ///     unit for the DSP algorithm,
        ...
        private void fluid_voice_calculate_runtime_synthesis_parameters()
        {
            ...
            foreach (var mod in mods)
            {
                ...
                var valMod = mod.fluid_mod_get_value(this);
            ...
            if (synth.VerboseCalcGen)
            {
                ...
                for (var n = 0; n < list_of_generators.Length; n++)
                {
                    var g = list_of_generators[n];
                    Debug.Log(
                        $"  ({g,-2:00}) {(fluid_gen_type)g,-25} {(int)gen[g].flags} val:{gen[g].Val,10:F2} mod:{gen[g].Mod,10:F2}");
                }

                Debug.Log("fluid_voice_update_param:");
            }

            foreach (var igen in list_of_generators)
                try
                {
                ...
                {
                    Debug.LogError(
                        $"Channel:{chan} Sample:{sample.Name} Generator:[{igen} {(fluid_gen_type)igen}] {ex.Message}");
                }
        ...
        private float fluid_voice_get_lower_boundary_for_attenuation()
        {
            ...
            foreach (var m in mods)
                ...
                if (m.dest == (int)fluid_gen_type.GEN_ATTENUATION)
                    if ((m.flags1 & (byte)fluid_mod_flags.FLUID_MOD_CC) > 0 ||
                        (m.flags2 & (byte)fluid_mod_flags.FLUID_MOD_CC) > 0 ||
                        m.src1 == (byte)fluid_mod_src.FLUID_MOD_CHANNELPRESSURE ||
                        m.src1 == (byte)fluid_mod_src.FLUID_MOD_KEYPRESSURE ||
                        m.src1 == (byte)fluid_mod_src.FLUID_MOD_PITCHWHEEL ||
                        m.src2 == (byte)fluid_mod_src.FLUID_MOD_CHANNELPRESSURE ||
                        m.src2 == (byte)fluid_mod_src.FLUID_MOD_KEYPRESSURE ||
                        m.src2 == (byte)fluid_mod_src.FLUID_MOD_PITCHWHEEL)
                    {
                        var current_val = m.fluid_mod_get_value(this);
                        var min_val = Mathf.Abs(m.amount);
                        ...
                        if (synth.VerboseCalcMod)
                            Debug.Log(
                                $"fluid_voice_get_lower_boundary_for_attenuation {m.dest} current_val:{current_val} min_val: {min_val}");
                        if (current_val > min_val) possible_att_reduction_cB += current_val - min_val;
                    }
            ...
            if (synth.VerboseCalcMod)
                Debug.Log(
                    $"fluid_voice_get_lower_boundary_for_attenuation possible_att_reduction_cB:{possible_att_reduction_cB} lower_bound:{lower_bound} ");

            /* SF2.01 specs do not allow negative attenuation */
            if (lower_bound < 0f) lower_bound = 0f;
            return lower_bound;
        }

        /// <summary>
        ///     @brief
        ///     The value of a generator (gen) has changed.  (The different generators are listed in fluidsynth.h, or in SF2.01
        ///     page 48-49). Now the dependent 'voice' parameters are calculated.
        ///     fluid_voice_update_param can be called during the setup of the  voice (to calculate the initial value for a voice
        ///     parameter), or
        ///     during its operation (a generator has been changed due to real-time parameter modifications like pitch-bend).
        ///     Note: The generator holds three values: The base value .val, an offset caused by modulators .mod, and an offset
        ///     caused by the
        ///     NRPN system. _GEN(voice, generator_enumerator) returns the sum of all three.
        ///     From fluid_midi_send_event NOTE_ON -. synth_noteon -. fluid_voice_start -.
        ///     fluid_voice_calculate_runtime_synthesis_parameters
        ///     From fluid_midi_send_event CONTROL_CHANGE -. fluid_synth_cc -. fluid_channel_cc Default      -.
        ///     fluid_synth_modulate_voices     -. fluid_voice_modulate
        ///     From fluid_midi_send_event CONTROL_CHANGE -. fluid_synth_cc -. fluid_channel_cc ALL_CTRL_OFF -.
        ///     fluid_synth_modulate_voices_all -. fluid_voice_modulate_all
        /// </summary>
        /// <param name="igen"></param>
        public void fluid_voice_update_param(int igen)
        {
            //Debug.Log("fluid_voice_update_param " + (fluid_gen_type)igen);
            var genVal = CalculateGeneratorValue(igen);
            string header = null;
            ...
            switch (igen)
            {
                case (int)fluid_gen_type.GEN_PAN:
                    ...
                    if (synth.MPTK_EnablePanChange)
                        if (synth.MPTK_CorePlayer)
                        {
                            var left = fluid_conv.fluid_pan(pan, true); // from 0 to 1
                            var right = fluid_conv.fluid_pan(pan, false); // from 0 to 1 
                            amp_left *= left;
                            amp_right *= right;
                            if (synth.VerboseCalcGen)
                                Debug.LogFormat(
                                    $"{header} EnablePanChange={synth.MPTK_EnablePanChange} synth.gain={synth.gain:0.00} pan={pan:0.00} amp_left={left:0.00} amp_right={right:0.00} mptkChannel.volume={channel.Volume}");
                        }
                ...
                case (int)fluid_gen_type.GEN_ATTENUATION:
                    ...
                    if (synth.VerboseCalcGen)
                        Debug.LogFormat($"{header} Val={gen[igen].Val} Mod={gen[igen].Mod} attenuation:{genVal}");
                    break;
                ...
                {
                    float x = 0;
                    var val = gen[(int)fluid_gen_type.GEN_OVERRIDEROOTKEY].Val;
                    ...
                    if (synth.MPTK_CorePlayer)
                        /*was root_pitch*/
                        x = (float)(fluid_conv.fluid_ct2hz(root_pitch) * (output_rate / (double)sample.SampleRate));
                    ...
                    if (synth.VerboseCalcGen)
                        Debug.LogFormat(
                            $"{header} key:{fluid_voice_get_actual_key()} pitch.val:{gen[(int)fluid_gen_type.GEN_PITCH].Val} root_pitch:{root_pitch} PitchAdj:{sample.PitchAdj}");
                }
                    break;
                ...
                {
                    var x = genVal < -12000f ? -12000f : genVal > 5000f ? 5000f : genVal;
                    modlfo_delay = (uint)(output_rate * fluid_conv.fluid_tc2sec_delay(x));
                    if (synth.VerboseCalcGen)
                        Debug.LogFormat($"{header} time:{fluid_conv.fluid_tc2sec_delay(x):F4} s.");
                }
                    break;
                ...
                case (int)fluid_gen_type.GEN_MODLFOFREQ:
                {
                    ...
                    var x = genVal < -16000.0f ? -16000.0f : genVal > 4500.0f ? 4500.0f : genVal;
                ...
                case (int)fluid_gen_type.GEN_STARTADDRCOARSEOFS: /* SF2.01 section 8.1.3 # 4 */
                    if (sample != null)
                    {
                        start = (int)(sample.Start
                                      + (int)gen[(int)fluid_gen_type.GEN_STARTADDROFS].Val +
                                      gen[(int)fluid_gen_type.GEN_STARTADDROFS]
                                          .Mod /*+ gens[(int)fluid_gen_type.GEN_STARTADDROFS].nrpn*/
                                      + 32768 * (int)gen[(int)fluid_gen_type.GEN_STARTADDRCOARSEOFS].Val +
                                      gen[(int)fluid_gen_type.GEN_STARTADDRCOARSEOFS]
                                          .Mod /*+ gens[(int)fluid_gen_type.GEN_STARTADDRCOARSEOFS].nrpn*/);
                ...
                case (int)fluid_gen_type.GEN_ENDADDRCOARSEOFS: /* SF2.01 section 8.1.3 # 12 */
                    if (sample != null)
                    {
                                    ...
                                    + (int)gen[(int)fluid_gen_type.GEN_ENDADDROFS].Val +
                                    gen[(int)fluid_gen_type.GEN_ENDADDROFS]
                                        .Mod /*+ gens[(int)fluid_gen_type.GEN_ENDADDROFS].nrpn*/
                                    + 32768 * (int)gen[(int)fluid_gen_type.GEN_ENDADDRCOARSEOFS].Val +
                                    gen[(int)fluid_gen_type.GEN_ENDADDRCOARSEOFS]
                                        .Mod /*+ gens[(int)fluid_gen_type.GEN_ENDADDRCOARSEOFS].nrpn*/);
                    ...
                    if (sample != null)
                    {
                        loopstart =
                            (int)(sample.LoopStart +
                                  ...
                                  gen[(int)fluid_gen_type.GEN_STARTLOOPADDRCOARSEOFS]
                                      .Mod /*+ gens[(int)fluid_gen_type.GEN_STARTLOOPADDRCOARSEOFS].nrpn*/);
                ...
                case (int)fluid_gen_type.GEN_VOLENVDELAY: /* SF2.01 section 8.1.3 # 33 */
                {
                    ...
                    var count = synth.MPTK_CorePlayer
                        ?
                        // NUM_BUFFERS_DELAY
                    ...
                    {
                        var delaySec = (float)fluid_conv.fluid_tc2sec_delay(x);
                ...
                {
                    var x = genVal < -12000f ? -12000f : genVal > 8000f ? 8000f : genVal;
                    ...
                    if (synth.VerboseCalcVolADSR)
                    {
                        var attackSec = (float)fluid_conv.fluid_tc2sec_attack(x);
                ...
                {
                    var count = synth.MPTK_CorePlayer
                        ? calculate_hold_decay_buffers((int)fluid_gen_type.GEN_VOLENVHOLD,
                            (int)fluid_gen_type.GEN_KEYTOVOLENVHOLD, false)
                        : /* 0 means: hold */
                        (uint)(Nano100ToMilli * calculate_hold_decay_ms((int)fluid_gen_type.GEN_VOLENVHOLD,
                            (int)fluid_gen_type.GEN_KEYTOVOLENVHOLD, false)); /* 0 means: hold */
                    ...
                    if (synth.VerboseCalcVolADSR)
                    {
                        var holdSec = calculate_hold_decay_buffers_seconds(
                ...
                case (int)fluid_gen_type.GEN_KEYTOVOLENVDECAY: /* SF2.01 section 8.1.3 # 40 */
                {
                    ...
                    var genv = CalculateGeneratorValue((int)fluid_gen_type.GEN_VOLENVSUSTAIN);
                    var x = 1f - 0.001f * genv;
                    x = x < 0f ? 0f : x > 1f ? 1f : x;

                    var count = synth.MPTK_CorePlayer
                        ? calculate_hold_decay_buffers((int)fluid_gen_type.GEN_VOLENVDECAY,
                            (int)fluid_gen_type.GEN_KEYTOVOLENVDECAY, true)
                        : (uint)(Nano100ToMilli * calculate_hold_decay_ms((int)fluid_gen_type.GEN_VOLENVDECAY,
                            (int)fluid_gen_type.GEN_KEYTOVOLENVDECAY, true));
                    ...
                    if (synth.VerboseCalcVolADSR)
                    {
                        var decaySec = calculate_hold_decay_buffers_seconds((int)fluid_gen_type.GEN_VOLENVDECAY,
                            (int)fluid_gen_type.GEN_KEYTOVOLENVDECAY, true);
                ...
                {
                    var x = genVal < FLUID_MIN_VOLENVRELEASE ? FLUID_MIN_VOLENVRELEASE :
                        genVal > 8000.0f ? 8000.0f : genVal;
                    uint count;
                    if (synth.MPTK_CorePlayer)
                    {
                        //NUM_BUFFERS_RELEASE
                        count = 1 + (uint)(output_rate * fluid_conv.fluid_tc2sec_release(x) *
                            synth.MPTK_ReleaseTimeMod / synth.FLUID_BUFSIZE);
                    }
                    else
                    {
                        //uint count = 1 + (uint)(output_rate * fluid_conv.fluid_tc2sec_release(x) / fluid_synth_t.FLUID_BUFSIZE);
                        var rt = (uint)(Nano100ToMilli * fluid_conv.fluid_tc2sec_release(x) * 1000f);
                    ...
                    if (synth.VerboseCalcVolADSR)
                    {
                        var releaseSec = (float)fluid_conv.fluid_tc2sec_release(x);
                ...
                case (int)fluid_gen_type.GEN_MODENVDELAY: /* SF2.01 section 8.1.3 # 33 */
                {
                    ...
                    if (synth.VerboseCalcModADSR)
                        Debug.Log(string.Format(
                            $"{header} count:{count,-5} time:{fluid_conv.fluid_tc2sec_delay(x):F4} s."));
                }
                    break;
                ...
                case (int)fluid_gen_type.GEN_MODENVATTACK: /* SF2.01 section 8.1.3 # 34 */
                {
                    ...
                    if (synth.VerboseCalcModADSR)
                        Debug.Log(string.Format(
                            $"{header} count:{count,-5} time:{fluid_conv.fluid_tc2sec_attack(x):F4} s."));
                }
                    break;
                ...
                {
                    var count = synth.MPTK_CorePlayer
                        ? calculate_hold_decay_buffers((int)fluid_gen_type.GEN_MODENVHOLD,
                            (int)fluid_gen_type.GEN_KEYTOMODENVHOLD, false)
                        : /* 0 means: hold */
                        (uint)(Nano100ToMilli * calculate_hold_decay_ms((int)fluid_gen_type.GEN_MODENVHOLD,
                            (int)fluid_gen_type.GEN_KEYTOMODENVHOLD, false)); /* 0 means: hold */
                    ...
                    if (synth.VerboseCalcModADSR)
                    {
                        var delayHold = calculate_hold_decay_buffers_seconds((int)fluid_gen_type.GEN_MODENVHOLD,
                            (int)fluid_gen_type.GEN_KEYTOMODENVHOLD, false);
                ...
                case (int)fluid_gen_type.GEN_KEYTOMODENVDECAY: /* SF2.01 section 8.1.3 # 40 */
                {
                    var count = synth.MPTK_CorePlayer
                        ? calculate_hold_decay_buffers(
                            ...
                            (int)fluid_gen_type.GEN_KEYTOMODENVDECAY, true)
                        : /* 1 for decay */
                        (uint)(Nano100ToMilli * calculate_hold_decay_ms((int)fluid_gen_type.GEN_MODENVDECAY,
                            (int)fluid_gen_type.GEN_KEYTOMODENVDECAY, true)); /* 1 for decay */

                    var x = 1f - 0.001f * CalculateGeneratorValue((int)fluid_gen_type.GEN_MODENVSUSTAIN);
                    x = x < 0f ? 0f : x > 1f ? 1f : x;
                    ...
                    if (synth.VerboseCalcModADSR)
                    {
                        var decaySec = calculate_hold_decay_buffers_seconds(
                            (int)fluid_gen_type.GEN_MODENVDECAY,
                            (int)fluid_gen_type.GEN_KEYTOVOLENVDECAY, true);
                ...
                {
                    var x = genVal < -12000f ? -12000f : genVal > 8000.0f ? 8000.0f : genVal;
                    uint count;
                    if (synth.MPTK_CorePlayer)
                    {
                        //NUM_BUFFERS_RELEASE
                    ...
                    if (synth.VerboseCalcModADSR)
                        Debug.Log(string.Format(
                            $"{header} count:{count,-5} time:{fluid_conv.fluid_tc2sec_release(x):F4} s."));
                }
                    break;
            }
        }
        ...
        public int fluid_voice_get_actual_key()
        {
            ...
            var x = gen[(int)fluid_gen_type.GEN_KEYNUM].Val +
                    gen[(int)fluid_gen_type.GEN_KEYNUM].Mod; //+ gens[igen].nrpn 
         ...
         * @since 1.1.7
         */
        private int fluid_voice_get_key()
        {
            return key;
        }
        ...
        */
        private float fluid_voice_calculate_pitch(int key)
        {
         ...
         */
        private float fluid_channel_get_key_pitch(int key)
        {
        ...
        public int fluid_voice_get_actual_velocity()
        {
            ...
            var x = gen[(int)fluid_gen_type.GEN_VELOCITY].Val +
                    gen[(int)fluid_gen_type.GEN_VELOCITY]
                        .Mod; //+ gens[igen].nrpn // fluid_voice_gen_value(voice, GEN_KEYNUM);

            if (x > 0f) return (int)x;
        ...
        private float calculate_hold_decay_buffers_seconds(int gen_base, int gen_key2base, bool is_decay)
        {
            /* Purpose:
             *
             ...
             */
            var keysteps = 60f - fluid_channel_get_key_pitch(fluid_voice_get_actual_key()) / 100f;
            var timecents = CalculateGeneratorValue(gen_base) + CalculateGeneratorValue(gen_key2base) * keysteps;

            /* Range checking */
            if (is_decay)
            {
                /* SF 2.01 section 8.1.3 # 28, 36 */
                if (timecents > 8000f) timecents = 8000f;
            }
            else
            {
                /* SF 2.01 section 8.1.3 # 27, 35 */
                if (timecents > 5000f) timecents = 5000f;
                /* SF 2.01 section 8.1.2 # 27, 35:
                 * The most negative number indicates no hold time
                 */
                if (timecents <= -32768f) return 0;
            }

            /* SF 2.01 section 8.1.3 # 27, 28, 35, 36 */
            if (timecents < -12000f) timecents = -12000f;
        ...
        private uint calculate_hold_decay_buffers(int gen_base, int gen_key2base, bool is_decay)
        {
            ...
            return (uint)(output_rate * calculate_hold_decay_buffers_seconds(gen_base, gen_key2base, is_decay) /
                synth.FLUID_BUFSIZE + 0.5f);
        }

        /// <summary>
        ///     @brief
        ///     Returns the number of DSP loops, that correspond to the hold (is_decay=0) or decay (is_decay=1) time.
        ///     gen_base=GEN_VOLENVHOLD, GEN_VOLENVDECAY, GEN_MODENVHOLD, GEN_MODENVDECAY gen_key2base=GEN_KEYTOVOLENVHOLD,
        ///     GEN_KEYTOVOLENVDECAY, GEN_KEYTOMODENVHOLD, GEN_KEYTOMODENVDECAY
        /// </summary>
        ...
        private float calculate_hold_decay_ms(int gen_base, int gen_key2base, bool is_decay)
        {
            ...
            var timecents = CalculateGeneratorValue(gen_base) + CalculateGeneratorValue(gen_key2base) * (60f - key);
            ...
            var seconds = Mathf.Pow(2f, timecents / 1200f);
        ...
        private const int NBR_BIT_BY_VAR_LN2 = 5; /* for 32 bits variables */
        private const int NBR_BIT_BY_VAR = 1 << NBR_BIT_BY_VAR_LN2;
        private const int NBR_BIT_BY_VAR_ANDMASK = NBR_BIT_BY_VAR - 1;

        private const int SIZE_UPDATED_GEN_BIT =
            ((byte)fluid_gen_type.GEN_LAST + NBR_BIT_BY_VAR_ANDMASK) / NBR_BIT_BY_VAR;
                    ...
                    mod.src2 == ctrl && (mod.flags2 & (byte)fluid_mod_flags.FLUID_MOD_CC) == 0 && cc == 0)
                {
                    ...
                    //if ((updated_gen_bit[igen >> NBR_BIT_BY_VAR_LN2] & (1 << (igen & NBR_BIT_BY_VAR_ANDMASK)))!=0)
                    {
                        ...
                        if (synth.VerboseController)
                            Debug.Log(
                                $"Modulate Generator Channel:{channel.Channel} Controller:{(fluid_mod_src)ctrl} Continue={cc} Gen:{(fluid_gen_type)gen} Value:{modval:F2}");
         ...
         */
        private void fluid_rvoice_check_sample_sanity()
        {
            //Debug.Log("fluid_rvoice_check_sample_sanity");
            var min_index_nonloop = (int)sample.Start;
            var max_index_nonloop = (int)sample.End;

            /* make sure we have enough samples surrounding the loop */
            var min_index_loop = (int)sample.Start + FLUID_MIN_LOOP_PAD;
            /* 'end' is last valid sample, loopend can be + 1 */
            var max_index_loop = (int)sample.End - FLUID_MIN_LOOP_PAD + 1;
            ...
            if (samplemode == fluid_loop.FLUID_LOOP_UNTIL_RELEASE || samplemode == fluid_loop.FLUID_LOOP_DURING_RELEASE)
            {
                ...
                if (loopend < loopstart + FLUID_MIN_LOOP_SIZE) samplemode = fluid_loop.FLUID_UNLOOPED;
        ...
        #define fluid_phase_index(_x) ((uint)((_x) >> 32))
                ...
                */
                ...
                var index_in_sample = (int)phase >> 32;
            ...
            float[] dsp_reverb_buf, float[] dsp_chorus_buf)
        {
            ...
            if (DurationTick >= 0 && ticks > TimeAtEnd)
                    ...
                    volenv_section < fluid_voice_envelope_index.FLUID_VOICE_ENVRELEASE)
                {
                    ...
                    if (synth.keepPlayingNonLooped && samplemode == fluid_loop.FLUID_UNLOOPED)
                    {
                        ...
                        {
                            var title =
                                $"Ignore note-off Channel:{chan} key:{key} ADSR:{volenv_section} Ignore:{synth.keepPlayingNonLooped} DurationTick:{DurationTick} Name:{sample.Name}";
                            DebugVolEnv(title);
                        }
                    }
                    else
            ...
            if (volenv_section == fluid_voice_envelope_index.FLUID_VOICE_ENVATTACK)
            {
                ...
                target_amp = (float)fluid_conv.fluid_cb2amp(attenuation) *
                             (float)fluid_conv.fluid_cb2amp(modlfo_val * -modlfo_to_vol) * volenv_val;
                ...
                if (synth.VerboseVolume)
                    DebugVolume($"amp_max:no_max target_amp:{target_amp:F5} amp:{amp:F5} CutOff:no_cutoff");
            }
            else
            {
                ...
                // MPTK 2.11.3 update related to FS 2.3
                target_amp = (float)fluid_conv.fluid_cb2amp(attenuation) *
                             (float)fluid_conv.fluid_cb2amp(fluid_conv.FLUID_PEAK_ATTENUATION * (1f - volenv_val) +
                ...
                if (synth.VerboseVolume)
                    DebugVolume(
                        $"amp_max:{amp_max:F5} target_amp:{target_amp:F5} amp:{amp:F5} CutOff:{synth.MPTK_CutOffVolume:F6}");
            }
...
#else
                resonant_filter.fluid_iir_filter_calc(output_rate,
                    modlfo_val * modlfo_to_fc + modenv_val * modenv_to_fc, 0);
#endif
         ...
         */
        private void fluid_voice_effects(int count, float[] dsp_left_buf, float[] dsp_right_buf, float[] dsp_reverb_buf,
            float[] dsp_chorus_buf)
        {
            int dsp_i;
            float v;
...
#else
            /* reverb send. Buffer may be NULL. */
            var levelReverb = reverb_send;
            /* chorus send. Buffer may be NULL. */
            var levelChorus = chorus_send;
#endif
            if (levelReverb > 1f)
                levelReverb = 1f;
            if (levelChorus > 1f)
        ...
        public void fluid_rvoice_noteoff(bool force = false)
        {
            if (status == fluid_voice_status.FLUID_VOICE_ON || status == fluid_voice_status.FLUID_VOICE_SUSTAINED)
            {
                if (!weakDevice)
                {
                    ...
                    {
                        if (synth.VerboseController)
                            if (status != fluid_voice_status.FLUID_VOICE_SUSTAINED)
                                Debug.Log(
                                    $"fluid_voice_noteoff Channel:{chan} key:{key} set status to FLUID_VOICE_SUSTAINED");
                        ...
                        if (volenv_section == fluid_voice_envelope_index.FLUID_VOICE_ENVATTACK)
                        {
                            ...
                            if (volenv_val > 0)
                            {
                                ...
                                if (synth.MPTK_ApplyModLfo)
                                {
                                    ...
                                    */
                                    var lfo = modlfo_val * -modlfo_to_vol;
                                    var amp = volenv_val * (float)fluid_conv.fluid_cb2amp(lfo);
                                    env_value = -((-200f / fluid_conv.M_LN10 * Mathf.Log(amp) - lfo) /
                                        fluid_conv.FLUID_PEAK_ATTENUATION - 1);
                                    // before 2.14.1
                                ...
                                {
                                    env_value = -(-200f / fluid_conv.M_LN10 * Mathf.Log(amp) /
                                        fluid_conv.FLUID_PEAK_ATTENUATION - 1);
        ...
        {
            if (!(status == fluid_voice_status.FLUID_VOICE_ON) ||
                status == fluid_voice_status.FLUID_VOICE_SUSTAINED) return 0;
            ...
            if (synth.VerboseSpecialNoteOff)
                DebugKillByExclusive($"Release sample with class {gen[(int)fluid_gen_type.GEN_EXCLUSIVECLASS].Val}");
        ...
        public void fluid_voice_off()
        {
            ...
            volenv_count = 0;
            if (VoiceAudio != null && VoiceAudio.Audiosource != null) VoiceAudio.Audiosource.volume = 0;
        ...
        {
            Debug.LogFormat(
                $"KillByExclusive - [{id,4}] TimeFromStart:{TicksToMilli(ticks - TimeAtStart)} ms Delta:{TicksToMilliF(DeltaTimeWrite):F2} ms {sample.Name} {info}");
        }

        public void DebugSynth(string info)
        {
            Debug.LogFormat(
                $"Synth - [{id,4}] TimeFromStart:{TicksToMilli(ticks - TimeAtStart)} ms Delta:{TicksToMilliF(DeltaTimeWrite):F2} ms {info}");
        }

        public void DebugOverload(string info)
        {
            Debug.LogFormat(
                $"Overload - [{id,4}] TimeFromStart:{TicksToMilli(ticks - TimeAtStart)} ms Delta:{TicksToMilliF(DeltaTimeWrite):F2} ms {info}");
        }

        public void DebugVolume(string info)
        {
            Debug.LogFormat(
                $"Volume - [{id,4}] TimeFromStart:{TicksToMilli(ticks - TimeAtStart)} ms Delta:{TicksToMilliF(DeltaTimeWrite):F2} ms {volenv_section} {info}");
        }

        public void DebugVolEnv(string info)
        {
            Debug.LogFormat(
                "VolEnv - [{0,4}] {1,-25} TimeFromStart:{2} ms Delta:{3:F2} ms section:{4} volenv_val:{5:0.000} volenv_count:{6} incr:{7:0.0000} coeff:{8:0.00}",
                id, info, TicksToMilli(ticks - TimeAtStart), TicksToMilliF(DeltaTimeWrite),
                volenv_section, volenv_val, volenv_data[(int)volenv_section].count,
                volenv_data[(int)volenv_section].incr, volenv_data[(int)volenv_section].coeff);
        }

        public void DebugModEnv(string info)
        {
            Debug.LogFormat(
                "ModEnv - [{0,4}] {1,-15} TimeFromStart:{2:0.000} Delta:{3:0.000} section:{4} modenv_val:{5:0.000} modenv_count:{6} incr:{7:0.0000}",
                id, info, TicksToMilli(ticks - TimeAtStart), TicksToMilliF(DeltaTimeWrite),
                modenv_section, modenv_val, modenv_data[(int)modenv_section].count,
                modenv_data[(int)modenv_section].incr);
        }

        public void DebugLFO(string info)
        {
            Debug.LogFormat(
                "[{0,4}] {1,-15} TimeFromStart:{2:00000.000} Delta:{3:0.000} modlfo_delay:{4} modlfo_incr:{5:0.000} modlfo_val:{6:0.000} modlfo_to_vol:{7:0.000}",
                id, info, TicksToMilli(ticks - TimeAtStart), TicksToMilliF(DeltaTimeWrite), modlfo_delay, modlfo_incr,
                modlfo_val, modlfo_to_vol);
        }
        ...
        {
            Debug.LogFormat(
                "[{0,4}] {1,-15} TimeFromStart:{2:00000.000} Delta:{3:0.000} viblfo_delay:{4} viblfo_incr:{5:0.000} viblfo_val:{6:0.000} viblfo_to_pitch:{7:0.000} -. pitch mod:{8:0.000}",
                id, info, TicksToMilli(ticks - TimeAtStart), TicksToMilliF(DeltaTimeWrite), viblfo_delay, viblfo_incr,
                viblfo_val, viblfo_to_pitch, (float)(1d + viblfo_val * viblfo_to_pitch / 1000d));
        }
    }
}