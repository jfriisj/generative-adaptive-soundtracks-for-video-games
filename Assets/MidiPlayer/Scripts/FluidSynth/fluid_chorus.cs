...
namespace MidiPlayerTK
{
   ...
   --------------------------------------------------------------------------------------*/
        ...
        private const int MAX_CHORUS = 99; /* number maximum of block */
        private const int MAX_LEVEL = 10; /* max output level */
        private const float MIN_SPEED_HZ = 0.1f; /* min lfo frequency (Hz) */
        private const int MAX_SPEED_HZ = 5; /* max lfo frequency (Hz) */
        ...
        */
        //const int WIDTH = 10; now a setting in MPTK
        ...
        private const float SCALE_WET = 1.0f;

        private const int MAX_SAMPLES = 2048; /* delay length in sample (46.4 ms at sample rate: 44100Hz).*/
        private const int LOW_MOD_DEPTH = 176; /* low mod_depth/2 in samples */
        private const float HIGH_MOD_DEPTH = MAX_SAMPLES / 2; /* high mod_depth in sample */
        private const float RANGE_MOD_DEPTH = HIGH_MOD_DEPTH - LOW_MOD_DEPTH;
        ...
        */
        ...
        private const int LOW_MOD_RATE = 5; /* MOD_RATE acceptable for low modulation depth (8 ms) */

        private const int HIGH_MOD_RATE = 4; /* MOD_RATE acceptable for max modulation depth (46.6 ms) */

        /* and max lfo speed (5 Hz) */
        private const int RANGE_MOD_RATE = HIGH_MOD_RATE - LOW_MOD_RATE;
        ...
        */
        ...
        public const fluid_chorus_mod FLUID_CHORUS_DEFAULT_TYPE = fluid_chorus_mod.FLUID_CHORUS_MOD_SINE;
        ...
        private float[] line; /* buffer line */

        private int line_in; /* line in position */


        /* modulator member */
        private readonly modulator[] mod; /* sinus/triangle modulator */
        ...
        private readonly float sample_rate;
        private int size; /* effective internal size (in samples) */
    ...
    ------------------------------------------------------------------------------*/
        ...
        private float get_mod_delay(modulator mod)
        {
            ...
            if (index_rate >= mod_rate)
            {
                ...
                if (out_index >= 0f)
                {
                    ...
                    if ((mod.line_out = int_out_index) >= size) mod.line_out -= size;
                }
                else /* negative */
                {
            ...
            /* updates line_out to the next sample.
               Boundary check and circular motion as needed */
            if (++mod.line_out >= size) mod.line_out -= size;
        ...
        private void set_center_position()
        {
            ...
            */
            ...
            {
                var delta_mod_depth = mod_depth - LOW_MOD_DEPTH;
         ...
         */
        public void fluid_chorus_set(int set, int nr, float plevel, float speed, float pdepth_ms,
            fluid_chorus_mod ptype, float pwidth)
        {
            int i;

            if ((set & (int)fluid_chorus_set_t.FLUID_CHORUS_SET_NR) != 0) /* number of block */ number_blocks = nr;

            if ((set & (int)fluid_chorus_set_t.FLUID_CHORUS_SET_LEVEL) != 0) /* output level */ level = plevel;
            ...
            */
            width = pwidth; // set as a setting in MPTK
        ...
        public void fluid_chorus_processmix(float[] inp, float[] left_out, float[] right_out)
        {
            ...
            for (sample_index = 0; sample_index < FLUID_BUFSIZE; sample_index++)
            {
                ...
                if (index_rate >= mod_rate)
                {
                    ...
                    if ((center_pos_mod += mod_rate) >= size) center_pos_mod -= size;
                }
