...
{
    public class fluid_iir_filter_32 : fluid_iir_filter
    {
        private float a1; /* a0 / a0 */
        private float a1_incr;
        private float a2; /* a1 / a0 */
        private float a2_incr;

        /* filter coefficients */
        /* The coefficients are normalized to a0. */
        /* b0 and b2 are identical => b02 */
        private float b02; /* b0 / a0 */

        private float b02_incr;
        private float b1; /* b1 / a0 */
        private float b1_incr;
        private bool compensate_incr; /* Flag: If set, must compensate history */
        private int filter_coeff_incr_count;
        private float filter_gain; /* Gain correction factor, depends on q */

        private bool filter_startup; /* Flag: If set, the filter will be set directly.
                       Else it changes smoothly. */

        private fluid_iir_filter_flags flags; /* additional flags to customize this filter */

        private readonly int FLUID_BUFSIZE;

        private float fres; /* the resonance frequency, in cents (not absolute cents) */
        private float hist1, hist2; /* Sample history for the IIR filter */

        private float last_fres; /* Current resonance frequency of the IIR filter */

        /* Serves as a flag: A deviation between fres and last_fres */
        /* indicates, that the filter has to be recalculated. */
        private float q_lin; /* the q-factor on a linear scale */
        private fluid_iir_filter_type type; /* specifies the type of this filter */

        public fluid_iir_filter_32(int bufsize)
        {
            //Debug.Log("Init iir filter 32");
            FLUID_BUFSIZE = bufsize;
        }

        public override void fluid_iir_filter_apply(float[] dsp_buf, int count)
        {
            //Debug.Log($"fluid_iir_filter_32 {count} {q_lin}");
            ...
            else
            {
                ...
                var dsp_b02 = b02;
                var dsp_b1 = b1;
                var dsp_filter_coeff_incr_count = filter_coeff_incr_count;
                ...
                {
                    var dsp_a1_incr = a1_incr;
                    var dsp_a2_incr = a2_incr;
                    var dsp_b02_incr = b02_incr;
                    var dsp_b1_incr = b1_incr;
                    ...
                    for (dsp_i = 0; dsp_i < count; dsp_i++)
                    {
                        ...
                        if (dsp_filter_coeff_incr_count-- > 0)
                        {
                            ...
                            {
                                var compensate = old_b02 / dsp_b02;
        ...
        public override void fluid_iir_filter_init(fluid_iir_filter_type ptype, fluid_iir_filter_flags pflags)
        {
            ...
            if (type != fluid_iir_filter_type.FLUID_IIR_DISABLED) fluid_iir_filter_reset();
        }
        ...
        public override void fluid_iir_filter_set_q(float pq, float offset /*MPTK Specific*/)
        {
            ...
            if (((int)flags & (int)fluid_iir_filter_flags.FLUID_IIR_Q_ZERO_OFF) != 0 && q <= 0f)
                q = 0;
        ...
        private static float fluid_iir_filter_q_from_dB(float q_dB, float offset /*MPTK specific*/)
        {
        ...
        private void fluid_iir_filter_calculate_coefficients(int transition_samples, float output_rate)
        {
            ...
            else
            {
                ...
                var omega = 2f * Mathf.PI * (last_fres / output_rate);
                var sin_coeff = Mathf.Sin(omega);
                var cos_coeff = Mathf.Cos(omega);
                var alpha_coeff = sin_coeff / (2f * q_lin);
                var a0_inv = 1f / (1f + alpha_coeff);
                ...
                var a1_temp = -2f * cos_coeff * a0_inv;
                var a2_temp = (1f - alpha_coeff) * a0_inv;
                ...
                {
                    //Debug.Log($"Setting transition filter coefficients {transition_samples}.\n");
        ...
        public override void fluid_iir_filter_calc(float output_rate, float fres_mod, float offset /*MPTK specific*/)
        {
            ...
            fres = (float)fluid_conv.fluid_ct2hz(this.fres + fres_mod);
