...
namespace MidiPlayerTK
{
    public class fluid_dsp_float
    {
        ...
        private const float FLUID_FRACT_MAX = 4294967296f;

        /* Interpolation (find a value between two samples of the original waveform) */

        /* Linear interpolation table (2 coefficients centered on 1st) */
        private static float[][] interp_coeff_linear; //[FLUID_INTERP_MAX,2];
        ...
        public static void fluid_dsp_float_config()
        {
            ...
            for (i2 = 0; i2 < FLUID_INTERP_MAX; i2++)
            {
                ...
                i_shifted = i - SINC_INTERP_ORDER / 2f + i2 / (float)FLUID_INTERP_MAX;
                ...
                if (Mathf.Abs(i_shifted) > 0.000001f)
                {
                    ...
                    /* Hamming window */
                    v *= 0.5f * (1f + Mathf.Cos(2f * fluid_voice.M_PI * i_shifted / SINC_INTERP_ORDER));
                }
                else
                {
                    v = 1f;
        ...
        public static int fluid_dsp_float_interpolate_none(fluid_voice voice)
        {
            ...
            var dsp_amp = voice.amp;
            var dsp_amp_incr = voice.amp_incr;
            uint dsp_i = 0;
