    ...
    public class fluid_dsp_float_64
    {
        ...
        public static void fluid_dsp_float_config()
        {
            ...
            for (i2 = 0; i2 < FLUID_INTERP_MAX; i2++)
            {
                ...
                i_shifted = i - SINC_INTERP_ORDER / 2d + i2 / (double)FLUID_INTERP_MAX;

                /* sinc(0) cannot be calculated straightforward (limit needed for 0/0) */
                if (Math.Abs(i_shifted) > 0.000001d)
                {
                    var arg = fluid_voice.M_PId * i_shifted;
                    v = Math.Sin(arg) / arg;
                    /* Hamming window */
                    v *= 0.5d * (1d + Math.Cos(2d * arg / SINC_INTERP_ORDER));
                }
                else
                {
                    v = 1f;
        ...
        {
            var dsp_phase = voice.phase;
            ulong dsp_phase_incr;
            var dsp_data = voice.sample.Data;
            var dsp_buf = voice.dsp_buf;
            double dsp_amp = voice.amp;
            double dsp_amp_incr = voice.amp_incr;
            uint dsp_i = 0;
            uint dsp_phase_index;
            uint end_index;
            var fluid_bufsize = voice.synth.FLUID_BUFSIZE;
            ...
            */
            dsp_phase_incr = ((ulong)voice.phase_incr << 32) |
                             (uint)(((double)voice.phase_incr - (int)voice.phase_incr) * FLUID_FRACT_MAX);
        ...
        public static int fluid_dsp_float_interpolate_linear(fluid_voice voice)
        {
            ...
            point = voice.is_looping
                ? dsp_data[voice.loopstart]
                : dsp_data[voice.end]; /* duplicate end for samples no longer looping */
                ...
                */
                ...
                for (; dsp_i < fluid_bufsize && dsp_phase_index <= end_index; dsp_i++)
                {
                    ...
                    */
                    ...
                    var tablerow = ((uint)(dsp_phase & 0xFFFFFFFF) & FLUID_INTERP_BITS_MASK) >> FLUID_INTERP_BITS_SHIFT;

                    coeffs = interp_coeff_linear[tablerow];

                    dsp_buf[dsp_i] = (float)(dsp_amp * (coeffs[0] * dsp_data[dsp_phase_index] +
            ...
            while (true)
            {
                ...
                for (; dsp_phase_index == start_index && dsp_i < fluid_bufsize; dsp_i++)
                {
                    ...
                    dsp_buf[dsp_i] = (float)(dsp_amp
                                             * (coeffs[0] * start_points2
                                                + coeffs[1] * start_points1
                                                + coeffs[2] * start_points0
                                                + coeffs[3] * dsp_data[dsp_phase_index]
                                                + coeffs[4] * dsp_data[dsp_phase_index + 1]
                                                + coeffs[5] * dsp_data[dsp_phase_index + 2]
                                                + coeffs[6] * dsp_data[dsp_phase_index + 3]));
                ...
                for (; dsp_phase_index == start_index && dsp_i < fluid_bufsize; dsp_i++)
                {
                    ...
                    dsp_buf[dsp_i] = (float)(dsp_amp
                                             * (coeffs[0] * start_points1
                                                + coeffs[1] * start_points0
                                                + coeffs[2] * dsp_data[dsp_phase_index - 1]
                                                + coeffs[3] * dsp_data[dsp_phase_index]
                                                + coeffs[4] * dsp_data[dsp_phase_index + 1]
                                                + coeffs[5] * dsp_data[dsp_phase_index + 2]
                ...
                for (; dsp_phase_index == start_index && dsp_i < fluid_bufsize; dsp_i++)
                {
                    ...
                    dsp_buf[dsp_i] = (float)(dsp_amp
                                             * (coeffs[0] * start_points0
                                                + coeffs[1] * dsp_data[dsp_phase_index - 2]
                                                + coeffs[2] * dsp_data[dsp_phase_index - 1]
                                                + coeffs[3] * dsp_data[dsp_phase_index]
                                                + coeffs[4] * dsp_data[dsp_phase_index + 1]
                                                + coeffs[5] * dsp_data[dsp_phase_index + 2]
                ...
                for (; dsp_i < fluid_bufsize && dsp_phase_index <= end_index; dsp_i++)
                {
                    ...
                    dsp_buf[dsp_i] = (float)(dsp_amp
                                             * (coeffs[0] * dsp_data[dsp_phase_index - 3]
                                                + coeffs[1] * dsp_data[dsp_phase_index - 2]
                                                + coeffs[2] * dsp_data[dsp_phase_index - 1]
                                                + coeffs[3] * dsp_data[dsp_phase_index]
                                                + coeffs[4] * dsp_data[dsp_phase_index + 1]
                ...
                for (; dsp_phase_index <= end_index && dsp_i < fluid_bufsize; dsp_i++)
                {
                    ...
                    dsp_buf[dsp_i] = (float)(dsp_amp
                                             * (coeffs[0] * dsp_data[dsp_phase_index - 3]
                                                ...
                                                + coeffs[3] * dsp_data[dsp_phase_index]
                                                + coeffs[4] * dsp_data[dsp_phase_index + 1]
                                                + coeffs[5] * end_points0
                                                + coeffs[6] * end_points1));
                ...
                for (; dsp_phase_index <= end_index && dsp_i < fluid_bufsize; dsp_i++)
                {
                    ...
                    dsp_buf[dsp_i] = (float)(dsp_amp
                                             * (coeffs[0] * dsp_data[dsp_phase_index - 3]
                                                ...
                                                + coeffs[2] * dsp_data[dsp_phase_index - 1]
                                                + coeffs[3] * dsp_data[dsp_phase_index]
                                                + coeffs[4] * end_points0
                                                + coeffs[5] * end_points1
                                                + coeffs[6] * end_points2));
