...
public class DemoScript : MonoBehaviour
{
    ...
    private void InitializeOutputDevice()
    {
        ...
        if (!allOutputDevices.Any(d => d.Name == OutputDeviceName))
        {
            ...
            Debug.Log(
                $"There is no [{OutputDeviceName}] device presented in the system. Here the list of all device:{Environment.NewLine}{allDevicesList}");
            return;
        }
    ...
    private MidiFile CreateTestFile()
    {
        ...
        foreach (var noteNumber in SevenBitNumber.Values) patternBuilder.Note(Note.Get(noteNumber));
